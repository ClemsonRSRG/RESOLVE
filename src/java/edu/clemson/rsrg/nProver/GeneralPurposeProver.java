...
/**
 * <p>
 * This class is a general purpose prover that attempts to prove the {@code VCs}
 * generated by {@link VCGenerator}.
 * </p>
 *
 * @author Yu-Shan Sun
...
public class GeneralPurposeProver {
	...
	/**
	 * <p>
	 * The current job's compilation environment that stores all necessary objects
	 * and flags.
	 * </p>
	 */
	 ...
	 *                           The current job's compilation environment that
	 *                           stores all necessary objects and flags.
	 */
	public GeneralPurposeProver(List<VerificationCondition> vcs, ModuleScope moduleScope,
		...
		if (myCompileEnvironment.flags.isFlagSet(FLAG_TIMEOUT)) {
			myTimeout = Long.parseLong(
					myCompileEnvironment.flags.getFlagArgument(FLAG_TIMEOUT, "milliseconds"));
		} else {
			myTimeout = 5000;
		}
		...
		if (myCompileEnvironment.flags.isFlagSet(FLAG_NUMTRIES)) {
			myNumTriesBeforeHalting = Integer
					.parseInt(myCompileEnvironment.flags.getFlagArgument(FLAG_NUMTRIES,
							"numtries"));
		} else {
		...
		if (moduleDec instanceof ConceptModuleDec) {
			header = mySTGroup.getInstanceOf("outputConceptHeader").add("conceptName",
			...
			header = mySTGroup.getInstanceOf("outputConceptRealizHeader")
					...
					.add("conceptName", ((ConceptRealizModuleDec) moduleDec).getConceptName()
							.getName());
			...
			header = mySTGroup.getInstanceOf("outputEnhancementHeader")
					...
					.add("conceptName",
							((EnhancementModuleDec) moduleDec).getConceptName().getName());
		} else if (moduleDec instanceof EnhancementRealizModuleDec) {
			header = mySTGroup.getInstanceOf("outputEnhancementRealizHeader")
					.add("realizName", moduleDec.getName().getName())
					.add("enhancementName",
							((EnhancementRealizModuleDec) moduleDec).getEnhancementName()
									.getName())
					.add("conceptName", ((EnhancementRealizModuleDec) moduleDec).getConceptName()
							.getName());
		} else {
			header = mySTGroup.getInstanceOf("outputFacilityHeader").add("facilityName",
					moduleDec.getName().getName());
		}
	...
	/**
	 * <p>
	 * This method returns the prover setting for how many unproved {@code VCs} we
	 * allow before halting.
	 * </p>
	 *
	 * @return The number of tries with -1 indicating that we attempt to prove all
	 *         VCs.
	 */
	...
	/**
	 * <p>
	 * This method returns the prover setting for the maximum amount of time we can
	 * spend proving each {@code VC}.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * This method returns the total elapsed time that we spent proving {@code VCs}
	 * in this {@link ModuleDec}.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * This method returns the verbose mode output with how we attempted to prove
	 * each {@code VCs} in this
	 * {@link ModuleDec}.
	 * </p>
	 *
	...
	public void proveVCs() {
		...
		for (VerificationCondition vc : myVerificationConditions) {
			...
			CongruenceClassRegistry<Integer, String, String, String> registry = new CongruenceClassRegistry<>(
					1000,
					1000, 1000, 1000);
			...
			for (TheoremEntry te : relevantTheorems) {
				 ...
				 * e:te.getAssertion().getSubExpressions()){
				 * System.out.println(e.getSubExpressions()); for(Exp
			...
			/*
			 * for(ElaborationRule eR : rules.getMyElaborationRules()){
			 * System.out.println("Precursors" +
			 * eR.getPrecursorClauses()); System.out.println("==>" +
			 * eR.getResultantClause());
			 *
			 * }
			 *
			 */
	 ...
	 *
	 * @param vc
	 *                  The {@link VerificationCondition} we have attempted to
	 *                  prove.
	 * @param result
	 *                  The prover results.
	 * @param registry
	 ...
	 * @param expLabels
	 *                  The expression labels assigned to the expressions in this
	 *                  {@code VC}.
	 */
	private void storeVCProofVerboseDetail(VerificationCondition vc, String result,
			CongruenceClassRegistry<Integer, String, String, String> registry,
