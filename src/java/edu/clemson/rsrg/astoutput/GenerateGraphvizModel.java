 ...
 */
package edu.clemson.rsrg.astoutput;
...
import edu.clemson.rsrg.absyn.statements.MemoryStmt;
...
/**
 * <p>
 * This class generates a Graphviz model file object using the provided RESOLVE abstract syntax tree. This visitor logic
 * is implemented as a a {@link TreeWalkerStackVisitor}.
 * </p>
 *
 * @author Chuck Cook
 * @author Yu-Shan Sun
...
public class GenerateGraphvizModel extends TreeWalkerStackVisitor {
    ...
    /**
     * <p>
     * This creates an object that overrides methods to generate a Graphviz model file.
     * </p>
     *
     * @param stGroup
     *            The string template file.
     * @param model
     *            The model we are going be generating.
     */
    ...
    /**
     * <p>
     * For all nodes that have a parent, add an edge from its parent node to itself.
     * </p>
     *
     * @param data
     *            Current {@link ResolveConceptualElement} we are visiting.
     */
    @Override
    public void preAnyStack(ResolveConceptualElement data) {
        ...
        if (parent != null) {
            ...
            ST edge = mySTGroup.getInstanceOf("outputGraphvizEdges").add("parentNodeNum", parentNum).add("nodeNum",
                    myNodeNum);
    ...
    public void postAnyStack(ResolveConceptualElement data) {
        ...
        if (data instanceof VirtualListNode) {
            ST node = createNode(myElementToNodeNumMap.get(data), data.getClass().getSimpleName(), true);

            node.add("nodeData", ((VirtualListNode) data).getListType().getSimpleName());
    ...
    /**
     * <p>
     * For all {@link Dec} nodes, create a new node and add the declaration's name field.
     * </p>
     *
     * @param e
     *            Current {@link Dec} we are visiting.
     */
    @Override
    public void postDec(Dec e) {
        // Create the new node
        ST node = createNode(myElementToNodeNumMap.get(e), e.getClass().getSimpleName(), true);
        String data;

        if (e instanceof MathAssertionDec) {
            data = ((MathAssertionDec) e).getAssertionType().name() + " " + e.getName().getName();
    ...
    /**
     * <p>
     * For all {@link DefinitionBodyItem} nodes, create a new node and add the "=" as the operator.
     * </p>
     *
     * @param e
     *            Current {@link DefinitionBodyItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link UsesItem} nodes, create a new node and add the name of the imported module.
     * </p>
     *
     * @param e
     *            Current {@link UsesItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link PerformanceSpecInitFinalItem} nodes, create a new node and add the item type.
     * </p>
     *
     * @param e
     *            Current {@link PerformanceSpecInitFinalItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link SpecInitFinalItem} nodes, create a new node and add the item type.
     * </p>
     *
     * @param e
     *            Current {@link SpecInitFinalItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link FacilityInitFinalItem} nodes, create a new node and add the item type.
     * </p>
     *
     * @param e
    ...
    /**
     * <p>
     * For all {@link RealizInitFinalItem} nodes, create a new node and add the item type.
     * </p>
     *
     * @param e
    ...
    /**
     * <p>
     * For all {@link LoopVerificationItem} nodes, we create a new node with its simple class name.
     * </p>
     *
     * @param e
    ...
    public void postLoopVerificationItem(LoopVerificationItem e) {
        myModel.add("nodes", createNode(myElementToNodeNumMap.get(e), e.getClass().getSimpleName(), false));
    }
    ...
    /**
     * <p>
     * For all {@link ModuleArgumentItem} nodes, we create a new node with its simple class name.
     * </p>
     *
     * @param e
     *            Current {@link ModuleArgumentItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link EnhancementSpecItem} nodes, we create a new node with its simple class name and the name of the
     * concept enhancement.
     * </p>
     *
     * @param e
     *            Current {@link EnhancementSpecItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link EnhancementSpecRealizItem} nodes, we create a new node with its simple class name, the name of the
    ...
    /**
     * <p>
     * For all {@link Ty} nodes, create a new node. If this node is a {@link NameTy}, we add in the name.
     * </p>
     *
     * @param e
     *            Current {@link Ty} we are visiting.
     */
    @Override
    public void postTy(Ty e) {
        ...
        if (e instanceof NameTy) {
            ...
            node = createNode(myElementToNodeNumMap.get(e), e.getClass().getSimpleName(), true);
            node.add("nodeData", data);
        } else {
            node = createNode(myElementToNodeNumMap.get(e), e.getClass().getSimpleName(), false);
        }
    ...
    /**
     * <p>
     * For all {@link AssertionClause} nodes, we create a new node with its simple class name and indicates what type of
     * clause this is and if it has an {@code which_entails}.
     * </p>
     *
     * @param e
     *            Current {@link AssertionClause} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link Statement} nodes, create a new node and add the simple name. If this node is a
     * {@link ConfirmStmt}, we add the simplify flag ({code true}/{code false}) to the data. If this node is a
     * {@link MemoryStmt}, we add the statement type to the data.
     * </p>
     *
     * @param e
     *            Current {@link Statement} we are visiting.
     */
    @Override
    public void postStatement(Statement e) {
        ...
        if (e instanceof ConfirmStmt) {
            ...
            node.add("nodeData", "SIMPLIFY: " + ((ConfirmStmt) e).getSimplify());
    ...
    /**
     * <p>
     * For all {@link CrossTypeExp} nodes, create a new node and add all the variables in this expression.
     * </p>
     *
     * @param e
     *            Current {@link CrossTypeExp} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link FunctionExp} nodes, create a new node and add the function's name (with qualifier if it is not
     * {@code null} and/or with the carat expression if it is not {@code null}).
     * </p>
     *
     * @param e
     *            Current {@link FunctionExp} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link InfixExp} nodes, create a new node and add the operator's name (with qualifier if it is not
     * {@code null}).
     * </p>
     *
     * @param e
     *            Current {@link InfixExp} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link OutfixExp} nodes, create a new node and add the operator as a string.
     * </p>
     *
     * @param e
     *            Current {@link OutfixExp} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link QuantExp} nodes, create a new node and add the appropriate quantification.
     * </p>
     *
     * @param e
     *            Current {@link QuantExp} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link TypeAssertionExp} nodes, create a new node and add its operator as a string.
     * </p>
     *
     * @param e
     *            Current {@link TypeAssertionExp} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * For all {@link MathExp} nodes that we didn't add special override logic, we create a new node with its simple
     * class name and the string retrieved from {@link MathExp#asString}.
     * </p>
     *
     * @param e
     *            Current {@link MathExp} we are visiting.
     */
    @Override
    public void postMathExp(MathExp e) {
        if (!(e instanceof InfixExp) && !(e instanceof TypeAssertionExp) && !(e instanceof QuantExp)
                && !(e instanceof FunctionExp) && !(e instanceof OutfixExp) && !(e instanceof CrossTypeExp)) {
            // Create the new node
            ST node = createNode(myElementToNodeNumMap.get(e), e.getClass().getSimpleName(), true);

            node.add("nodeData", escapeSpecialChars(e.asString(0, 0).toCharArray()));
            myModel.add("nodes", node);
        }
    }
    ...
    /**
     * <p>
     * For all {@link ProgramExp} nodes, we create a new node with its simple class name and the string retrieved from
     * {@link ProgramExp#asString}.
     * </p>
     *
     * @param e
     *            Current {@link ProgramExp} we are visiting.
     */
    @Override
     ...
     * @param nodeName
     *            The node's simple class name.
     * @param hasData
     *            {@code true} if the node has data, {@code false} otherwise.
     *
     * @return The newly created ST for "node".
     */
    private ST createNode(int nodeNum, String nodeName, boolean hasData) {
        return mySTGroup.getInstanceOf("outputGraphvizNodes").add("nodeNum", nodeNum).add("nodeName", nodeName)
