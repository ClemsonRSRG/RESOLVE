...
/**
 * <p>
 * This class contains the logic for applying facility initialization logic for the {@link FacilityDec} stored inside a
...
public class FacilityInitStmtRule extends AbstractProofRuleApplication implements ProofRuleApplication {
    ...
    /**
     * <p>
     * This creates an application rule that deals with {@link FacilityInitStmt}.
     * </p>
     *
     * @param facilityInitStmt
     *            The {@link FacilityInitStmt} we are applying the rule to.
     ...
     */
    public FacilityInitStmtRule(FacilityInitStmt facilityInitStmt, MathSymbolTableBuilder symbolTableBuilder,
    ...
    public final void applyRule() {
        ...
        InstantiatedFacilityDecl instantiatedFacilityDecl = myCurrentVerificationContext
                .getProcessedInstFacilityDecl(myFacilityInitStmt.getInstantiatedFacilityDec());

        // Extract the shared variable initialization for each block
        Exp initializationEnsuresExp = VarExp.getTrueVarExp(myFacilityInitStmt.getLocation(), myTypeGraph);
        PosSymbol facilityName = instantiatedFacilityDecl.getInstantiatedFacilityName();
        for (SharedStateDec dec : instantiatedFacilityDecl.getConceptSharedStates()) {
            Exp decInitEnsures = getSharedVariableInitEnsures(facilityName, dec, instantiatedFacilityDecl);
            if (VarExp.isLiteralTrue(initializationEnsuresExp)) {
                ...
                if (!VarExp.isLiteralTrue(decInitEnsures)) {
                    initializationEnsuresExp = MathExp.formConjunct(myFacilityInitStmt.getLocation().clone(),
                            initializationEnsuresExp, decInitEnsures);
                }
            }
        }
        ...
        for (TypeFamilyDec typeFamilyDec : instantiatedFacilityDecl.getConceptDeclaredTypes()) {
            // Extract the definition variables (if any)
            Exp defVarExp = generateDefVarExps(facilityName, typeFamilyDec, instantiatedFacilityDecl);
            if (VarExp.isLiteralTrue(initializationEnsuresExp)) {
            ...
            // is the empty set.
            EqualsExp typeRecepEqualsExp = createTypeReceptaclesEmptySetExp(facilityName, typeFamilyDec);
            if (VarExp.isLiteralTrue(initializationEnsuresExp)) {
        ...
        if (!VarExp.isLiteralTrue(initializationEnsuresExp)) {
            AssumeStmt initAssumeStmt = new AssumeStmt(myFacilityInitStmt.getLocation().clone(),
    ...
    /**
     * <p>
     * An helper method for creating an expression that indicate the type's receptacles is equal to the empty set.
     * </p>
     *
     * @param facilityName
     ...
     *
     * @return An expression that indicates that the instantiated type's {@code Receptacles} is equal to the empty set.
     */
    private EqualsExp createTypeReceptaclesEmptySetExp(PosSymbol facilityName, TypeFamilyDec typeFamilyDec) {
        // Create a new TypeReceptaclesExp
        TypeReceptaclesExp typeReceptaclesExp = new TypeReceptaclesExp(typeFamilyDec.getLocation().clone(),
                Utilities.createVarExp(myFacilityInitStmt.getLocation().clone(), facilityName,
                        typeFamilyDec.getName().clone(), typeFamilyDec.getModel().getMathType(),
        ...
        SetCollectionExp emptySetExp = new SetCollectionExp(myFacilityInitStmt.getLocation().clone(),
                new HashSet<MathExp>());
        ...
        EqualsExp equalsExp = new EqualsExp(myFacilityInitStmt.getLocation().clone(), typeReceptaclesExp, null,
                EqualsExp.Operator.EQUAL, emptySetExp);
        equalsExp.setMathType(myTypeGraph.BOOLEAN);
        equalsExp.setLocationDetailModel(new LocationDetailModel(myFacilityInitStmt.getLocation().clone(),
                myFacilityInitStmt.getLocation().clone(), "Receptacles of type: " + typeFamilyDec.getName().getName()));
    ...
    /**
     * <p>
     * An helper method for extracting the {@code initialization ensures} clause from a {@link SharedStateDec}.
     * </p>
     *
     * @param facilityName
     *            Name of the facility we are processing.
     * @param stateDec
     *            A shared variables block.
    ...
    private Exp getSharedVariableInitEnsures(PosSymbol facilityName, SharedStateDec stateDec,
        ...
        AssertionClause initEnsuresClause = stateDec.getInitialization().getEnsures();

        // Create the initialization ensures expression.
        Location initEnsuresLoc = initEnsuresClause.getAssertionExp().getLocation();
        initializationEnsuresExp = Utilities.formConjunct(initEnsuresLoc, null, initEnsuresClause,
                new LocationDetailModel(initEnsuresLoc.clone(), myFacilityInitStmt.getLocation().clone(),
                        "Facility Initialization Ensures Clause of " + facilityName.getName()));
        ...
        for (MathVarDec mathVarDec : stateDec.getAbstractStateVars()) {
            VarExp stateVarExp = Utilities.createVarExp(facilityName.getLocation().clone(), null, mathVarDec.getName(),
