...
import edu.clemson.rsrg.vcgeneration.sequents.SequentReduction;
import edu.clemson.rsrg.vcgeneration.sequents.reductiontree.ReductionTreeDotExporter;
import edu.clemson.rsrg.vcgeneration.sequents.reductiontree.ReductionTreeExporter;
...
import edu.clemson.rsrg.vcgeneration.utilities.treewalkers.UniqueSymbolNameExtractor;
import java.util.*;
...
/**
 * <p>
 * This class contains the logic for applying the {@code assume} rule to an {@link AssumeStmt}.
 * </p>
 *
 * @author Yu-Shan Sun
 * 
...
public class AssumeStmtRule extends AbstractProofRuleApplication implements ProofRuleApplication {
    ...
    /**
     * <p>
     * A map that indicates if a particular {@link Sequent} had an impacting reduction.
     * </p>
     */
     ...
     */
    public AssumeStmtRule(AssumeStmt assumeStmt, AssertiveCodeBlock block, VerificationContext context, STGroup stGroup,
            ST blockModel) {
    ...
    public final void applyRule() {
        ...
        if (VarExp.isLiteralTrue(myAssumeStmt.getAssertion())) {
            ...
            // common symbols.
            List<Exp> assumeExps = splitConjunctExp(assumeExp, new ArrayList<Exp>());
            ...
            if (assumeExps.size() != 1) {
                Graph<Sequent, DefaultEdge> reductionTree = new DefaultDirectedGraph<>(DefaultEdge.class);
                ...
                Sequent rootSequent = new Sequent(myAssumeStmt.getLocation(), Collections.singletonList(assumeExp),
                        new ArrayList<Exp>());
                Sequent updatedSequent = new Sequent(myAssumeStmt.getLocation(), assumeExps, new ArrayList<Exp>());
            ...
            for (VerificationCondition vc : myCurrentAssertiveCodeBlock.getVCs()) {
                ...
                for (Sequent reducedSequent : reducedSequents) {
                    ...
                    newVCs.add(new VerificationCondition(vc.getLocation().clone(), reducedSequent,
                            hasImpactingReduction, vc.getLocationDetailModel().clone()));
                }
            }
        ...
        stepModel.add("proofRuleName", ruleName).add("currentStateOfBlock", myCurrentAssertiveCodeBlock);
    ...
    /**
     * <p>
     * This is a helper method that checks to see if the given assume expression can be used to prove anything in
     ...
     *
     * @param seq
     *            The current {@link Sequent} we are processing.
     * @param remAssumeExpList
     *            The list of remaining assume expressions.
     *
            ...
            while (checkForMoreAntecedents) {
                ...
                for (Exp assumeExp : remAssumeExpList) {
                    ...
                    for (Exp antecedentExp : seqAntecedents) {
                        ...
                        // unique symbol names.
                        UniqueSymbolNameExtractor symbolNameExtractor = new UniqueSymbolNameExtractor();
    ...
    /**
     * <p>
     * This method checks to see if this the expression we passed is either a variable expression or a dotted expression
     * that contains a variable expression in the last position.
     * </p>
     *
     * @param exp
    ...
    /**
     * <p>
     * Rather than using the strict {@code equals} method that is defined for {@link Exp Exps}, this method checks to
     * see if all {@link Exp Exps} are {@code equivalent}.
     * </p>
     *
     * @param originalExpList
     *            The original expression list.
     * @param newExpList
     *            The new expression list.
     *
     * @return {@code true} if the lists contain {@code equivalent} {@link Exp Exps}, {@code false} otherwise.
     */
    private boolean isEquivalentExpList(List<Exp> originalExpList, List<Exp> newExpList) {
    ...
    /**
     * <p>
     * This method uses {@code sequent} to produce a list of reduced {@link Sequent Sequents}.
     * </p>
     *
     * @param sequent
     *            Original {@link Sequent}.
     * @param stepModel
     *            The model associated with this step.
     *
    ...
    private List<Sequent> reducedSequentForm(Sequent sequent, ST stepModel) {
        ...
        myImpactingReducedSequentMap.putAll(reduction.getImpactingReducedSequentMap());
     ...
     * @param expList
     *            List of expressions that have been split already.
     *
     * @return A list containing {@code expList} and the split result of {@code exp}.
     */
    ...
    /**
     * <p>
     * This method performs the substitutions on each of the expressions in the given list.
     * </p>
     *
     * @param expressions
     *            List of {@link Exp Exps}.
     * @param replacements
    ...
    /**
     * <p>
     * This is a helper method that checks to see if the any of the assume expressions can be substituted. If it can, it
     * will generate a new sequent. If it cannot, the original sequent will be kept. Once this step is over, it will
     * call {@link #applicationStep(Sequent, List)} to generate the final {@link Sequent}.
     * </p>
     *
     * @param seq
     *            The current {@link Sequent} we are processing.
     * @param assumeExpList
     *            A list of {@link Exp Exps} being assumed.
     *
     * @return The modified {@link Sequent}.
     */
    private Sequent substitutionStep(Sequent seq, List<Exp> assumeExpList) {
        ...
        return applicationStep(new Sequent(seq.getLocation(), newAntecedents, newConsequents), remAssumeExpList);
    }
}
