...
import edu.clemson.rsrg.vcgeneration.utilities.treewalkers.GenerateRememberRuleSubstitutionMap;
import java.util.*;
 ...
 * @version 1.0
 */
public class RememberStmtRule extends AbstractProofRuleApplication implements ProofRuleApplication {
     ...
     */
    public RememberStmtRule(AssertiveCodeBlock block, VerificationContext context, STGroup stGroup, ST blockModel) {
    ...
    public final void applyRule() {
        ...
        for (VerificationCondition vc : vcs) {
            newVCs.add(
                    new VerificationCondition(vc.getLocation(), vc.getName(), createReplacementSequent(vc.getSequent()),
    ...
    /**
     * <p>
     * An helper method that uses the {@link GenerateRememberRuleSubstitutionMap} walker to generate {@link Exp Exps}
     * that result from applying the {@code Remember} rule for each of the {@link Exp} in the {@link Sequent}.
     * </p>
     *
     * @param s
    ...
    private Sequent createReplacementSequent(Sequent s) {
        ...
        for (Exp antecedent : s.getAntecedents()) {
            ...
            GenerateRememberRuleSubstitutionMap expMapGenerator = new GenerateRememberRuleSubstitutionMap(antecedent);
        ...
        for (Exp consequent : s.getConcequents()) {
            ...
            GenerateRememberRuleSubstitutionMap expMapGenerator = new GenerateRememberRuleSubstitutionMap(consequent);
