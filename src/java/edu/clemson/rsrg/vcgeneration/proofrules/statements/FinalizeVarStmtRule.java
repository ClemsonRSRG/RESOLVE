...
/**
 * <p>
 * This class contains the logic for applying variable finalization to the variable declaration with a known program
 * type stored inside a {@link FinalizeVarStmt}.
 * </p>
 *
 * @author Yu-Shan Sun
 *
...
public class FinalizeVarStmtRule extends AbstractProofRuleApplication implements ProofRuleApplication {
     ...
     */
    public FinalizeVarStmtRule(FinalizeVarStmt finalVarStmt, MathSymbolTableBuilder symbolTableBuilder,
    ...
    public final void applyRule() {
        ...
        if (typeEntry instanceof ProgramTypeEntry) {
            ...
            finalizationEnsuresExp = getConceptTypeFinalEnsures(dec, programTypeEntry);
        }
        ...
        AssumeStmt finalAssumeStmt = new AssumeStmt(dec.getLocation(), finalizationEnsuresExp, false);
    ...
    /**
     * <p>
     * An helper method for extracting the {@code finalization ensures} clause for a {@link ProgramTypeEntry}.
     * </p>
     *
     * @param varDec
     *            The variable declaration we are finalizing.
     * @param programTypeEntry
     *            A program type entry.
     *
     * @return A finalization ensures clause.
     */
    private Exp getConceptTypeFinalEnsures(VarDec varDec, ProgramTypeEntry programTypeEntry) {
        ...
        AssertionClause finalEnsuresClause = type.getFinalization().getEnsures();

        // Create the modified finalization ensures clause with
        // the exemplar replaced with the variable declaration name.
        AssertionClause modifiedFinalEnsures = Utilities.getTypeFinalEnsuresClause(finalEnsuresClause,
        ...
        Location finalEnsuresLoc = modifiedFinalEnsures.getAssertionExp().getLocation();
        finalizationEnsuresExp = Utilities.formConjunct(finalEnsuresLoc, null, modifiedFinalEnsures,
                new LocationDetailModel(finalEnsuresLoc.clone(), varDec.getLocation().clone(),
        ...
        finalizationEnsuresExp = createEnsuresExpWithModifiedSharedVars(varDec.getLocation(), finalizationEnsuresExp,
                facQualifier, type.getFinalization().getAffectedVars());
    ...
    /**
     * <p>
     * An helper method for extracting the {@code finalization ensures} clause for a {@link TypeRepresentationDec}.
     * </p>
     *
     * @param varDec
     *            The variable declaration we are finalizing.
     ...
     * @return A finalization ensures clause.
     */
    private Exp getTypeRepresentationFinalEnsures(VarDec varDec, TypeRepresentationDec typeRepresentationDec) {
        Exp finalizationEnsuresExp;
        ...
        if (implementingType instanceof NameTy) {
            ...
            finalizationEnsuresExp = getConceptTypeFinalEnsures(varDec, programTypeEntry);
        }
        // Case #2: Type representation implemented using a record type
        else {
            RecordTy implementingTypeAsRecordTy = (RecordTy) implementingType;
            finalizationEnsuresExp = VarExp.getTrueVarExp(implementingType.getLocation(), myTypeGraph);
            ...
            for (VarDec innerDec : implementingTypeAsRecordTy.getFields()) {
                ...
                Exp innerFinalEnsuresExp = getConceptTypeFinalEnsures(innerDec, programTypeEntry);
                ...
                innerFinalEnsuresExp = innerFinalEnsuresExp.substitute(substitutionMap);

                if (VarExp.isLiteralTrue(finalizationEnsuresExp)) {
                    finalizationEnsuresExp = innerFinalEnsuresExp;
                } else {
                    finalizationEnsuresExp = MathExp.formConjunct(typeRepresentationDec.getLocation(),
                            finalizationEnsuresExp, innerFinalEnsuresExp);
                }
            }
        }
