 ...
 */
package edu.clemson.rsrg.vcgeneration.proofrules.declarations.operationdecl;

import edu.clemson.rsrg.absyn.clauses.AffectsClause;
import edu.clemson.rsrg.absyn.clauses.AssertionClause;
import edu.clemson.rsrg.absyn.clauses.AssertionClause.ClauseType;
...
import edu.clemson.rsrg.typeandpopulate.mathtypes.MTCartesian;
 ...
 * @version 1.0
 */
public class ProcedureDeclRule extends AbstractBlockDeclRule implements ProofRuleApplication {
    ...
    /**
     * <p>
     * This creates a new application for the {@code procedure} declaration rule.
     * </p>
     *
     * @param procedureDec
     *            The {@link ProcedureDec} we are applying the rule to.
     * @param procVarTypeEntries
     ...
     */
    public ProcedureDeclRule(ProcedureDec procedureDec, Map<VarDec, SymbolTableEntry> procVarTypeEntries,
        ...
        super(block, block.getCorrespondingOperation().getName(), symbolTableBuilder, moduleScope, context, stGroup,
                blockModel);
    ...
    public final void applyRule() {
        ...
        boolean isLocal = Utilities.isLocationOperation(myProcedureDec.getName().getName(), myCurrentModuleScope);
        ...
        if (myCurrentProcedureDecreasingExp != null) {
            ...
            VarExp pValExp = Utilities.createPValExp(myCurrentProcedureDecreasingExp.getLocation().clone(),
                    myCurrentModuleScope);
            myCurrentAssertiveCodeBlock.addFreeVar(pValExp);

            // Generate progress metric recursive call: P_Val = P_Exp
            EqualsExp equalsExp = new EqualsExp(myCurrentProcedureDecreasingExp.getLocation().clone(), pValExp.clone(),
                    null, Operator.EQUAL, myCurrentProcedureDecreasingExp.clone());
            ...
            equalsExp.setLocationDetailModel(new LocationDetailModel(
                    ...
                    myCurrentProcedureDecreasingExp.getLocation().clone(), "Progress Metric for Recursive Procedure"));

            // Add this expression as something we can assume to be true.
            AssumeStmt progressMetricAssume = new AssumeStmt(myCurrentProcedureDecreasingExp.getLocation().clone(),
                    equalsExp, false);
        ...
        if (inConceptRealiz && !isLocal) {
            ...
            correctOpHyp = Utilities.replaceFacilityFormalWithActual(correctOpHyp, myProcedureDec.getParameters(),
            ...
            ConfirmStmt correctOpHypStmt = new ConfirmStmt(myProcedureDec.getLocation().clone(), correctOpHyp, true);
            ...
            // Replace any facility declaration instantiation arguments
            // in the correct operation hypothesis expression.
            correspondenceExp = Utilities.replaceFacilityFormalWithActual(correspondenceExp,
                    myProcedureDec.getParameters(), myCurrentModuleScope.getDefiningElement().getName(),
                    myCurrentVerificationContext);

            // Use the expression to create an assume statement if it is not "true"
            AssumeStmt correspondenceStmt = new AssumeStmt(myProcedureDec.getLocation().clone(), correspondenceExp,
                    false);
        ...
        finalConfirmExp = Utilities.replaceFacilityFormalWithActual(finalConfirmExp, myProcedureDec.getParameters(),
                myCurrentModuleScope.getDefiningElement().getName(), myCurrentVerificationContext);

        // Use the ensures clause to create a final confirm statement
        ConfirmStmt finalConfirmStmt = new ConfirmStmt(myProcedureDec.getLocation().clone(), finalConfirmExp,
    ...
    /**
     * <p>
     * An helper method that uses the {@code Shared Variable} and any parameter type {@code conventions} to build the
     * appropriate expression that must be established for a {@code Concept Realization} operation to be correct.
     * </p>
     *
    ...
    private Exp createCorrectOpHypExp() {
        ...
        Exp retExp = myCurrentVerificationContext.createSharedStateRealizConventionExp(loc);
        ...
        if (retExp.getLocationDetailModel() != null) {
            ...
            retExp.setLocationDetailModel(new LocationDetailModel(model.getSourceLoc(), model.getDestinationLoc(),
                    model.getDetailMessage() + " Generated by " + myProcedureDec.getName().getName()));
        }
        ...
        for (ParameterVarDec parameterVarDec : myProcedureDec.getParameters()) {
            ...
            if (nameTy.getProgramType() instanceof PTRepresentation) {
                ...
                PTRepresentation representationType = (PTRepresentation) nameTy.getProgramType();
                ...
                if (ste.getDefiningElement() instanceof TypeRepresentationDec) {
                    AssertionClause conventionClause = Utilities.getTypeConventionClause(
                            ...
                            new PosSymbol(loc.clone(), representationType.getFamily().getExemplar().getName()),
                    ...
                    if (VarExp.isLiteralTrue(retExp)) {
                        retExp = Utilities.formConjunct(loc, null, conventionClause, conventionDetailModel);
                    } else {
                        retExp = Utilities.formConjunct(loc, retExp, conventionClause, conventionDetailModel);
                    }
                } else {
    ...
    /**
     * <p>
     * An helper method that uses the {@code Shared Variable} and any parameter type {@code correspondence} to build the
    ...
    private Exp createCorrespondenceExp() {
        ...
        Location loc = myProcedureDec.getLocation().clone();
        Exp retExp = myCurrentVerificationContext.createSharedStateRealizCorrespondenceExp(loc);
            ...
            if (nameTy.getProgramType() instanceof PTRepresentation) {
                ...
                if (ste.getDefiningElement() instanceof TypeRepresentationDec) {
                    AssertionClause correspondenceClause = Utilities.getTypeCorrespondenceClause(
                            ...
                            new PosSymbol(loc.clone(), representationType.getFamily().getExemplar().getName()), nameTy,
                            nameTy.getMathType(), null, myTypeGraph.BOOLEAN);
                    ...
                    if (VarExp.isLiteralTrue(retExp)) {
                        retExp = Utilities.formConjunct(loc, null, correspondenceClause, correspondenceDetailModel);
                    } else {
                        retExp = Utilities.formConjunct(loc, retExp, correspondenceClause, correspondenceDetailModel);
                    }
                } else {
                    // Not sure why it wouldn't be a TypeRepresentationDec
                    Utilities.notAType(ste, parameterVarDec.getLocation());
    ...
    /**
     * <p>
     * An helper method that uses the {@code ensures} clause from the operation entry and adds in additional
     * {@code ensures} clauses for different parameter modes and builds the appropriate {@code ensures} clause that will
     * be an {@link AssertiveCodeBlock AssertiveCodeBlock's} final {@code confirm} statement.
     * </p>
     *
    ...
    private Exp createFinalConfirmExp(boolean inConceptRealiz, boolean isLocal) {
        ...
        Exp paramEnsuresExp = VarExp.getTrueVarExp(procedureLoc.clone(), myTypeGraph);
        while (specParamVarDecIt.hasNext()) {
            ...
            // Query for the type entry in the symbol table
            SymbolTableEntry ste = Utilities.searchProgramType(procedureLoc, nameTy.getQualifier(), nameTy.getName(),
                    myCurrentModuleScope);
            ...
            if (parameterMode == ParameterMode.RESTORES) {
                ...
                if (typeEntry.getModelType() instanceof MTCartesian) {
                    MTCartesian cartesian = (MTCartesian) typeEntry.getModelType();
                    ...
                    for (int i = 0; i < cartesian.size(); i++) {
                        ...
                        VarExp elementExp = Utilities.createVarExp(restoresLoc.clone(), null,
                                new PosSymbol(restoresLoc.clone(), cartesian.getTag(i)), elementTypes.get(i), null);
                        ...
                        if (restoresConditionExp == null) {
                            ...
                            restoresConditionExp = InfixExp.formConjunct(restoresLoc.clone(), restoresConditionExp,
                                    equalsExp);
                        }
                    }
                } else {
                ...
                if (restoresConditionExp != null) {
                    restoresConditionExp.setLocationDetailModel(new LocationDetailModel(
                            ...
                            "Ensures Clause of " + myCurrentProcedureOperationEntry.getName() + " (Condition from \""
                    ...
                    if (VarExp.isLiteralTrue(paramEnsuresExp)) {
                        ...
                        paramEnsuresExp = InfixExp.formConjunct(paramEnsuresExp.getLocation(), paramEnsuresExp,
            ...
            // that the outgoing value is the initial value.
                ...
                if (typeEntry.getDefiningElement() instanceof TypeFamilyDec) {
                    ...
                    AssertionClause initEnsures = type.getInitialization().getEnsures();
                    modifiedInitEnsures = Utilities.getTypeInitEnsuresClause(initEnsures, procedureLoc.clone(), null,
                            parameterVarDec.getName(), type.getExemplar(), typeEntry.getModelType(), null);
                    ...
                    VarDec parameterAsVarDec = new VarDec(parameterVarDec.getName(), parameterVarDec.getTy());
                    modifiedInitEnsures = new AssertionClause(procedureLoc.clone(), ClauseType.ENSURES,
                ...
                clearsConditionExp.setLocationDetailModel(new LocationDetailModel(parameterVarDec.getLocation().clone(),
            ...
            if (ste.getDefiningElement() instanceof TypeRepresentationDec && inConceptRealiz && !isLocal) {
                ...
                substitutionParamToConc = addConceptualVariables(parameterExp, oldParameterExp, concVarExp,
                        substitutionParamToConc);
            }
        }

        // Add the operation's ensures clause
        AssertionClause ensuresClause = myCurrentProcedureOperationEntry.getEnsuresClause();
        ...
        ensuresExp.setLocationDetailModel(new LocationDetailModel(ensuresExp.getLocation().clone(),
                procedureLoc.clone(), "Ensures Clause of " + myCurrentProcedureOperationEntry.getName()));
        ...
        if (VarExp.isLiteralTrue(ensuresExp)) {
            ...
            if (VarExp.isLiteralTrue(paramEnsuresExp)) {
                ...
                retExp = InfixExp.formConjunct(ensuresExp.getLocation(), ensuresExp, paramEnsuresExp);
            }
        }
        ...
        for (SharedStateDec stateDec : sharedStateDecs) {
            for (MathVarDec mathVarDec : stateDec.getAbstractStateVars()) {
                ...
                if (!Utilities.containsEquivalentExp(myAffectedExps, stateVarExp)) {
                    retExp = createRestoresExpForSharedVars(procedureLoc, stateVarExp, oldStateVarExp, retExp);
                }
        ...
        for (TypeFamilyDec typeFamilyDec : myCurrentVerificationContext.getConceptDeclaredTypes()) {
            for (MathDefVariableDec mathDefVariableDec : typeFamilyDec.getDefinitionVarList()) {
                ...
                VarExp defVarExp = Utilities.createVarExp(procedureLoc.clone(), null, mathVarDec.getName(),
                        mathVarDec.getMathType(), null);
                ...
                if (!Utilities.containsEquivalentExp(myAffectedExps, defVarExp)) {
                    retExp = createRestoresExpForDefVars(procedureLoc, defVarExp, oldDefVarExp, retExp);
                }
