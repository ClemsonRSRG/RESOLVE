...
/**
 * <p>
 * This class contains the logic for establishing the {@code Shared Variable}'s {@code correspondence} is well defined.
 * </p>
 *
 * @author Yu-Shan Sun
 * 
 * @version 1.0
 */
public class SharedStateCorrRule extends AbstractProofRuleApplication implements ProofRuleApplication {
    ...
    /**
     * <p>
     * The {@code shared state} realization we are applying the rule to.
     * </p>
     */
     ...
     */
    public SharedStateCorrRule(SharedStateRealizationDec dec, SharedStateDec correspondingSharedStateDec,
            MathSymbolTableBuilder symbolTableBuilder, AssertiveCodeBlock block, VerificationContext context,
    ...
    public final void applyRule() {
        ...
        Exp topLevelAssumeExp = myCurrentVerificationContext
                .createTopLevelAssumeExpFromContext(mySharedStateRealizationDec.getLocation(), true, false);

        // ( Assume CPC and RPC and DC and RDC and SS_RC; )
        AssumeStmt topLevelAssumeStmt = new AssumeStmt(mySharedStateRealizationDec.getLocation().clone(),
        ...
        // ( Confirm VC; )
        Exp sharedVariableConstraint = myCorrespondingSharedStateDec.getConstraint().getAssertionExp().clone();
        ...
        sharedVariableConstraint.setLocationDetailModel(new LocationDetailModel(
                myCorrespondingSharedStateDec.getLocation().clone(), mySharedStateRealizationDec.getLocation().clone(),
                "Well Defined Correspondence for Shared Variables"));
        ConfirmStmt finalConfirmStmt = new ConfirmStmt(mySharedStateRealizationDec.getLocation().clone(),
                sharedVariableConstraint, VarExp.isLiteralTrue(sharedVariableConstraint));
