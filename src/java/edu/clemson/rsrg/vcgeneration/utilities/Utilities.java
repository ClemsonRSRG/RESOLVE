...
import edu.clemson.rsrg.absyn.expressions.mathexpr.EqualsExp.Operator;
...
import edu.clemson.rsrg.vcgeneration.utilities.formaltoactual.InstantiatedEnhSpecRealizItem;
...
/**
 * <p>
 * This class contains a bunch of utilities methods used by the {@link VCGenerator} and all of its associated
...
public class Utilities {
     ...
     *            Expression to check.
     *
     * @return {@code true} if an equivalent {@code exp} is in the collection, {@code false} otherwise.
     */
    public static boolean containsEquivalentExp(Collection<Exp> collection, Exp exp) {
        boolean found = false;
    ...
    /**
     * <p>
     * Converts the different types of {@link Exp} to the ones used by the VC Generator.
     * </p>
     *
     * @param oldExp
    ...
    public static Exp convertExp(Exp oldExp, ModuleScope scope) {
        ...
        if (oldExp instanceof ProgramIntegerExp) {
            IntegerExp exp = new IntegerExp(oldExp.getLocation(), null, ((ProgramIntegerExp) oldExp).getValue());
            ...
            MathSymbolEntry mse = searchMathSymbol(exp.getLocation(), "N", scope);
            try {
        ...
        // Case #2: ProgramCharacterExp
            ...
            CharExp exp = new CharExp(oldExp.getLocation(), ((ProgramCharExp) oldExp).getValue());
            exp.setMathType(oldExp.getMathType());
            retExp = exp;
        }
        // Case #3: ProgramStringExp
        else if (oldExp instanceof ProgramStringExp) {
            StringExp exp = new StringExp(oldExp.getLocation(), ((ProgramStringExp) oldExp).getValue());
            exp.setMathType(oldExp.getMathType());
            retExp = exp;
        }
        // Case #4: VariableDotExp
        else if (oldExp instanceof ProgramVariableDotExp) {
            List<ProgramVariableExp> segments = ((ProgramVariableDotExp) oldExp).getSegments();
            ...
            for (ProgramVariableExp v : segments) {
                ...
                if (v instanceof ProgramVariableNameExp) {
                    VarExp varExp = new VarExp(v.getLocation(), v.getQualifier(),
        ...
        // Case #5: VariableNameExp
            ...
            VarExp exp = new VarExp(oldExp.getLocation(), ((ProgramVariableNameExp) oldExp).getQualifier(),
    ...
    /**
     * <p>
     * This method returns the conceptual version of the {@link VarDec}.
     * </p>
     *
     * @param varDec
     *            The parameter variable.
     * @param varDecType
     *            Mathematical type for the parameter variable.
     * @param booleanType
    ...
    public static DotExp createConcVarExp(VarDec varDec, MTType varDecType, MTType booleanType) {
        ...
        VarExp varExp = Utilities.createVarExp(varDec.getLocation(), null, varDec.getName(), varDecType, null);

        // Create a VarExp with the name "Conc"
        VarExp concVarExp = Utilities.createVarExp(varDec.getLocation(), null,
                new PosSymbol(varDec.getLocation(), "Conc"), booleanType, null);
    ...
    /**
     * <p>
     * This method returns a {@link FunctionExp} with the specified name and arguments.
     * </p>
     *
     * @param loc
     *            Location that wants to create this function expression.
     * @param qualifier
     *            Qualifier for the function expression.
     * @param name
     *            Name of the function expression.
     * @param argExpList
     *            List of arguments to the function expression.
     * @param functionNameType
     *            Mathematical type for the function name.
     * @param funcType
     *            Mathematical type for the function expression.
     *
     * @return The new {@link FunctionExp}.
     */
    public static FunctionExp createFunctionExp(Location loc, PosSymbol qualifier, PosSymbol name, List<Exp> argExpList,
            MTType functionNameType, MTType funcType) {
        // Create a VarExp for the function name
        VarExp functionName = Utilities.createVarExp(loc, qualifier, name, functionNameType, null);
    ...
    /**
     * <p>
     * This method returns a {@link DotExp} with the {@link VarDec} and its initialization ensures clause.
     * </p>
     *
     * @param varDec
    ...
    public static DotExp createInitExp(VarDec varDec, MTType booleanType) {
        // Convert the declared variable into a VarExp
        VarExp varExp = Utilities.createVarExp(varDec.getLocation(), null, varDec.getName(),
        ...
        if (varDec.getTy() instanceof NameTy) {
            ...
            typeNameExp = Utilities.createVarExp(ty.getLocation(), ty.getQualifier(), ty.getName(), ty.getMathType(),
                    ty.getMathTypeValue());
        } else {
        ...
        FunctionExp isInitialExp = createFunctionExp(varDec.getLocation(), null,
                new PosSymbol(varDec.getLocation(), "Is_Initial"), isInitialArgs, varDec.getTy().getMathType(),
                booleanType);
    ...
    /**
     * <p>
     * This method creates a list of {@link VarExp VarExps} representing each of the {@code Operation's}
     * {@link ParameterVarDec ParameterVarDecs}.
     * </p>
     *
     * @param parameterVarDecs
     *            List of operation parameters.
     *
     * @return A list containing the {@link VarExp VarExps} representing each operation parameter.
     */
    public static List<VarExp> createOperationParamExpList(List<ParameterVarDec> parameterVarDecs) {
        ...
        for (ParameterVarDec dec : parameterVarDecs) {
            retExpList.add(Utilities.createVarExp(dec.getLocation(), null, dec.getName(), dec.getMathType(), null));
        }
    ...
    /**
     * <p>
     * This method returns a newly created {@link VarExp} with {@code P_Val} as the name and {@code N} as its math type.
     * </p>
     *
     * @param loc
     *            New {@link VarExp VarExp's} {@link Location}.
     * @param scope
     *            The module scope to start our search.
     *
    ...
    public static VarExp createPValExp(Location loc, ModuleScope scope) {
        ...
        try {
            ...
            retExp = createVarExp(loc.clone(), null, new PosSymbol(loc.clone(), "P_Val"), mse.getTypeValue(), null);
        } catch (SymbolNotOfKindTypeException e) {
            notAType(mse, loc);
        }
    ...
    /**
     * <p>
     * This method returns a newly created {@link VarExp} with the {@link PosSymbol} and math type provided.
     * </p>
     *
     * @param loc
     *            New {@link VarExp VarExp's} {@link Location}.
     * @param qualifier
     *            New {@link VarExp VarExp's} qualifier.
     * @param name
     *            New {@link VarExp VarExp's} name.
     * @param type
     *            New {@link VarExp VarExp's} math type.
     * @param typeValue
     *            New {@link VarExp VarExp's} math type value.
     *
     * @return The new {@link VarExp}.
     */
    public static VarExp createVarExp(Location loc, PosSymbol qualifier, PosSymbol name, MTType type,
            MTType typeValue) {
        // Create the VarExp
    ...
    /**
     * <p>
     * An helper method that creates a new "question mark" variable.
     * </p>
     *
     * @param currentBlock
     ...
     */
    public static VCVarExp createVCVarExp(AssertiveCodeBlock currentBlock, Exp varExp) {
        int newStateNum = 1;
    ...
    /**
     * <p>
     * An helper method to print a list of expressions.
     * </p>
     *
     * @param exps
    ...
    /**
     * <p>
     * An helper method that throws the appropriate message that the expression type that we found isn't handled.
     * </p>
     *
     * @param exp
     *            An expression.
     * @param loc
     *            Location where this expression was found.
     */
    public static void expNotHandled(Exp exp, Location loc) {
        String message = "[VCGenerator] Exp type not handled: " + exp.getClass().getCanonicalName();
        throw new SourceErrorException(message, loc);
    }

    /**
     * <p>
     * An helper method that uses the assertion expression and any {@code which_entails} expressions to {@code exp} to
     * form a new conjuncted expression.
     * </p>
     *
     * @param loc
     ...
     *            The original expression.
     * @param clause
     *            An {@link AssertionClause}.
     * @param clauseDetailModel
     *            The {@link LocationDetailModel} associated with {@code clause}.
     *
     * @return A new {@link Exp}.
     */
    public static Exp formConjunct(Location loc, Exp exp, AssertionClause clause,
            LocationDetailModel clauseDetailModel) {
        Exp retExp;
        ...
        if (clause.getWhichEntailsExp() != null) {
            ...
            whichEntailsExp.setLocationDetailModel(new LocationDetailModel(entailsLoc.clone(), entailsLoc.clone(),
                    "Which_Entails Expression Located at " + clause.getLocation()));
    ...
    /**
     * <p>
     * An helper method for locating the associated {@code Type Family} from a {@link TypeRepresentationDec}.
     * </p>
     *
     * @param dec
     *            An {@link TypeRepresentationDec}.
     * @param context
     *            The current {@link VerificationContext}.
     *
     * @return A {@link TypeFamilyDec}.
     */
    public static TypeFamilyDec getAssociatedTypeFamilyDec(TypeRepresentationDec dec, VerificationContext context) {
        // Obtain the type family we are implementing
        TypeFamilyDec typeFamilyDec = null;
        Iterator<TypeFamilyDec> conceptTypeIt = context.getConceptDeclaredTypes().iterator();
        ...
        if (typeFamilyDec == null) {
            ...
            // by accident.
            Utilities.noSuchSymbol(null, dec.getName().getName(), dec.getLocation());
        }
    ...
    /**
     * <p>
     * An helper method for locating a facility qualifier (if any) from a raw program type.
     * </p>
     *
     * @param ty
     *            A raw program type.
     ...
     *            The current {@link VerificationContext}.
     *
     * @return A facility qualifier if the program type came from a facility instantiation, {@code null} otherwise.
     */
    public static PosSymbol getFacilityQualifier(NameTy ty, VerificationContext context) {
        PosSymbol facQualifier = ty.getQualifier();

        // Check to see if there is a facility that instantiated a type
        // that matches "ty".
        if (facQualifier == null) {
            Iterator<InstantiatedFacilityDecl> it = context.getProcessedInstFacilityDecls().iterator();
    ...
    /**
     * <p>
     * An helper method for locating the instantiating facility (if any) from an operation entry.
     * </p>
     *
     * @param operationEntry
     *            An {@link OperationEntry}.
     * @param context
     *            The current {@link VerificationContext}.
     *
     * @return A {@link InstantiatedFacilityDecl} if {@code operationEntry} is from a module that has been instantiated,
     *         {@code null} otherwise.
     */
    public static InstantiatedFacilityDecl getInstantiatingFacility(OperationEntry operationEntry,
            VerificationContext context) {
        InstantiatedFacilityDecl instantiatedFacilityDecl = null;
        ModuleIdentifier sourceModuleIdentifier = operationEntry.getSourceModuleIdentifier();
        ...
        while (it.hasNext() && instantiatedFacilityDecl == null) {
            ...
            if (sourceModuleIdentifier.equals(new ModuleIdentifier(facilityDec.getConceptName().getName()))) {
                instantiatedFacilityDecl = decl;
            } else {
                Iterator<EnhancementSpecRealizItem> specRealizItemIt = facilityDec.getEnhancementRealizPairs()
                        .iterator();
                while (specRealizItemIt.hasNext() && instantiatedFacilityDecl == null) {
                    EnhancementSpecRealizItem item = specRealizItemIt.next();
                    if (sourceModuleIdentifier.equals(new ModuleIdentifier(item.getEnhancementName().getName()))) {
                        instantiatedFacilityDecl = decl;
                    }
                }
            }
    ...
    /**
     * <p>
     * An helper method that returns {@link ProgramFunctionExp ProgramFunctionExp's} corresponding
     * {@link OperationEntry}.
     * </p>
     *
     * @param functionExp
     *            A program function expression.
     * @param scope
     *            The module scope to start our search.
     *
    ...
    public static OperationEntry getOperationEntry(ProgramFunctionExp functionExp, ModuleScope scope) {
        ...
        return Utilities.searchOperation(functionExp.getLocation(), functionExp.getQualifier(), functionExp.getName(),
                argTypes, ImportStrategy.IMPORT_NAMED, FacilityStrategy.FACILITY_INSTANTIATE, scope);
    }

    /**
     * <p>
     * Given the original {@code constraint} clause, use the provided information on the actual parameter variable to
     * substitute the {@code exemplar} in the {@code constraint} clause and create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalConstraintClause
     *            The {@link AssertionClause} containing the original {@code constraint} clause.
     * @param loc
     *            The location in the AST that we are currently visiting.
     * @param qualifier
     *            The parameter variable's qualifier.
     * @param name
     *            The parameter variable's name.
     * @param exemplarName
     *            The {@code exemplar} name for the corresponding type.
     * @param type
     *            The mathematical type associated with this type.
     * @param typeValue
     *            The mathematical type value associated with this type.
     *
     * @return A modified {@link AssertionClause} containing the new {@code constraint} clause.
     */
    public static AssertionClause getTypeConstraintClause(AssertionClause originalConstraintClause, Location loc,
        ...
        Exp constraintWithReplacements = originalConstraintClause.getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalConstraintClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalConstraintClause.getWhichEntailsExp().substitute(substitutions);
        }

        return new AssertionClause(newLoc, AssertionClause.ClauseType.CONSTRAINT, constraintWithReplacements,
                whichEntailsWithReplacements);
    }

    /**
     * <p>
     * Given the original {@code convention} clause, use the provided information on the actual parameter variable to
     * substitute the {@code exemplar} in the {@code convention} clause and create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalConventionClause
     *            The {@link AssertionClause} containing the original {@code convention} clause.
     * @param loc
     *            The location in the AST that we are currently visiting.
     * @param name
     *            The parameter variable's name.
     ...
     *            The mathematical type value associated with this type.
     *
     * @return A modified {@link AssertionClause} containing the new {@code convention} clause.
     */
    public static AssertionClause getTypeConventionClause(AssertionClause originalConventionClause, Location loc,
            PosSymbol name, PosSymbol exemplarName, MTType type, MTType typeValue) {
        // Create a variable expression from the declared variable
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp conventionWithReplacements = originalConventionClause.getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalConventionClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalConventionClause.getWhichEntailsExp().substitute(substitutions);
        }

        return new AssertionClause(newLoc, AssertionClause.ClauseType.CONVENTION, conventionWithReplacements,
                whichEntailsWithReplacements);
    }

    /**
     * <p>
     * Given the original {@code correspondence} clause, use the provided information on the actual parameter variable
     * to substitute the {@code exemplar} in the {@code correspondence} clause and create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalCorrespondenceClause
     *            The {@link AssertionClause} containing the original {@code correspondence} clause.
     * @param loc
     *            The location in the AST that we are currently visiting.
     * @param name
     *            The parameter variable's name.
     * @param ty
     *            The parameter's raw type.
     * @param exemplarName
     *            The {@code exemplar} name for the corresponding type.
     * @param exemplarTy
     *            The {@code exemplar}'s raw type.
     * @param type
     *            The mathematical type associated with this type.
     * @param typeValue
     *            The mathematical type value associated with this type.
     ...
     *            Mathematical boolean type.
     *
     * @return A modified {@link AssertionClause} containing the new {@code correspondence} clause.
     */
    public static AssertionClause getTypeCorrespondenceClause(AssertionClause originalCorrespondenceClause,
            Location loc, PosSymbol name, Ty ty, PosSymbol exemplarName, Ty exemplarTy, MTType type, MTType typeValue,
            MTType booleanType) {
        // Create a variable expression from the declared variable
        VarExp varDecExp = Utilities.createVarExp(loc, null, name, type, typeValue);
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp correspondenceWithReplacements = originalCorrespondenceClause.getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalCorrespondenceClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalCorrespondenceClause.getWhichEntailsExp().substitute(substitutions);
        }

        return new AssertionClause(newLoc, AssertionClause.ClauseType.CORRESPONDENCE, correspondenceWithReplacements,
                whichEntailsWithReplacements);
    }

    /**
     * <p>
     * Given the original {@code finalization ensures} clause, use the provided information on a program variable to
     * substitute the {@code #exemplar} in the {@code finalization ensures} clause and create a new
     * {@link AssertionClause}.
     * </p>
     *
     * @param originalFinalEnsuresClause
     *            The {@link AssertionClause} containing the original {@code finalization ensures} clause.
     * @param loc
     *            The location in the AST that we are currently visiting.
     * @param qualifier
     *            The program variable's qualifier.
     * @param name
     *            The program variable's name.
     * @param exemplarName
     *            The {@code exemplar} name for the corresponding type.
     * @param type
     *            The mathematical type associated with this type.
     ...
     *            The mathematical type value associated with this type.
     *
     * @return A modified {@link AssertionClause} containing the new {@code finalization ensures} clause.
     */
    public static AssertionClause getTypeFinalEnsuresClause(AssertionClause originalFinalEnsuresClause, Location loc,
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp finalEnsuresWithReplacements = originalFinalEnsuresClause.getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalFinalEnsuresClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalFinalEnsuresClause.getWhichEntailsExp().substitute(substitutions);
        }

        return new AssertionClause(newLoc, AssertionClause.ClauseType.ENSURES, finalEnsuresWithReplacements,
                whichEntailsWithReplacements);
    }

    /**
     * <p>
     * Given the original {@code initialization ensures} clause, use the provided information on a program variable to
     * substitute the {@code exemplar} in the {@code initialization ensures} clause and create a new
     * {@link AssertionClause}.
     * </p>
     *
     * @param originalInitEnsuresClause
     *            The {@link AssertionClause} containing the original {@code initialization ensures} clause.
     * @param loc
     *            The location in the AST that we are currently visiting.
     * @param qualifier
     *            The program variable's qualifier.
     ...
     *            The mathematical type value associated with this type.
     *
     * @return A modified {@link AssertionClause} containing the new {@code initialization ensures} clause.
     */
    public static AssertionClause getTypeInitEnsuresClause(AssertionClause originalInitEnsuresClause, Location loc,
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp initEnsuresWithReplacements = originalInitEnsuresClause.getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalInitEnsuresClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalInitEnsuresClause.getWhichEntailsExp().substitute(substitutions);
        }

        return new AssertionClause(newLoc, AssertionClause.ClauseType.ENSURES, initEnsuresWithReplacements,
                whichEntailsWithReplacements);
    }

    /**
     * <p>
     * Given a {@link ProgramVariableExp}, return the associated {@link PosSymbol} name.
     * </p>
     *
     * @param exp
    ...
    public static PosSymbol getVarName(ProgramVariableExp exp) {
        ...
        // Program dotted expression.
            ...
            List<ProgramVariableExp> segments = ((ProgramVariableDotExp) exp).getSegments();
     ...
     * @param scope
     *            The module scope we are searching.
     *
    ...
    public static boolean isLocationOperation(String name, ModuleScope scope) {
        ...
        List<SymbolTableEntry> entries = scope
                .query(new NameQuery(null, name, ImportStrategy.IMPORT_NONE, FacilityStrategy.FACILITY_IGNORE, true));
    ...
    /**
     * <p>
     * An helper method that negates the incoming expression.
     * </p>
     *
     * @param exp
    ...
    public static Exp negateExp(Exp exp, MTType booleanType) {
        ...
        if (exp instanceof EqualsExp) {
            ...
            if (expAsEqualsExp.getOperator() == Operator.EQUAL) {
                ...
                retExp = new PrefixExp(expAsEqualsExp.getLocation().clone(), null,
                        new PosSymbol(expAsEqualsExp.getLocation().clone(), "not"), expAsEqualsExp.clone());
            }
            // Case 1.2: a /= b
            else {
                ...
                retExp = new EqualsExp(expAsEqualsExp.getLocation().clone(), expAsEqualsExp.getLeft().clone(),
                        newQualifier, Operator.EQUAL, expAsEqualsExp.getRight().clone());
            }
        }
        // Case 2: Some kind of prefix expression
            ...
            if (expAsPrefixExp.getOperatorAsString().equals("not")) {
                ...
                retExp = new PrefixExp(expAsPrefixExp.getLocation().clone(), null,
                        new PosSymbol(expAsPrefixExp.getLocation().clone(), "not"), expAsPrefixExp.clone());
            }
        }
        // Case 3: All other kinds of expressions.
        else {
            retExp = new PrefixExp(exp.getLocation().clone(), null, new PosSymbol(exp.getLocation().clone(), "not"),
                    exp.clone());
        }
    ...
    /**
     * <p>
     * An helper method that throws the appropriate message that the symbol table entry that we found isn't a type.
     * </p>
     *
     * @param entry
     *            A symbol table entry.
     * @param loc
     *            Location where this entry was found.
     */
    public static void notAType(SymbolTableEntry entry, Location loc) {
        throw new SourceErrorException(
                "[VCGenerator] " + entry.getSourceModuleIdentifier().fullyQualifiedRepresentation(entry.getName())
                        + " is not known to be a type.",
                loc);
    }
    ...
    public static void noSuchModule(Location loc) {
        throw new SourceErrorException("[VCGenerator] Module does not exist or is not in scope.", loc);
    }
     ...
     */
    public static void noSuchSymbol(PosSymbol qualifier, String symbolName, Location loc) {
        String message;

        if (qualifier == null) {
            message = "[VCGenerator] No such symbol: " + symbolName;
        } else {
            message = "[VCGenerator] No such symbol in module: " + qualifier.getName() + "::" + symbolName;
        }
    ...
    /**
     * <p>
     * This method is used to check for there are paths from {@code originalSequent} to each sequent in
     * {@code resultSequents} in the reduction tree.
     * </p>
     *
     * @param g
     *            The reduction tree.
     ...
     * @param resultSequents
     *            The {@link Sequent} that resulted from the sequent reduction applications.
     *
     * @return {@code true} if all the {@link Sequent} in {@code resultSequents} have a path from
     *         {@code originalSequent} in the reduction tree, {@code false} otherwise.
     */
    public static boolean pathsExist(Graph<Sequent, DefaultEdge> g, Sequent originalSequent,
            List<Sequent> resultSequents) {
        boolean retVal = true;
    ...
    /**
     * <p>
     * This method is used to replace any parameter declarations whose program types come from instantiated facility
     * declarations. This will replace any the instantiated type's formal with its actual instantiation expression in
     * {@code clauseExp}.
     * </p>
     *
     * @param clauseExp
     *            Some clause expression we are trying to replace.
     * @param paramList
     *            List of parameter declarations from the operation we are trying to call.
     ...
     */
    public static Exp replaceFacilityFormalWithActual(Exp clauseExp, List<ParameterVarDec> paramList,
            PosSymbol currentModuleName, VerificationContext context) {
        ...
        for (ParameterVarDec dec : paramList) {
            ...
            if ((dec.getTy() instanceof NameTy) && !(dec.getTy().getProgramType() instanceof PTGeneric)) {
                ...
                if (decTyAsNameTy.getQualifier() == null) {
                    ...
                    Iterator<TypeFamilyDec> it = context.getConceptDeclaredTypes().iterator();
                    while (it.hasNext() && isInstantiatedType) {
                        // If the name matches, then it must be a concept abstract type
                        if (decTyAsNameTy.getName().getName().equals(it.next().getName().getName())) {
                            isInstantiatedType = false;
                        }
                    }

                    // Check all representation types.
                    Iterator<AbstractTypeRepresentationDec> it2 = context.getLocalTypeRepresentationDecs().iterator();
                    while (it2.hasNext() && isInstantiatedType) {
                        // If the name matches, then it must be the representation type
                        if (decTyAsNameTy.getName().getName().equals(it2.next().getName().getName())) {
                            isInstantiatedType = false;
                        }
                    }
                } else {
                    ...
                    if (decTyAsNameTy.getQualifier().getName().equals(currentModuleName.getName())) {
                        isInstantiatedType = false;
                    }
                }
                ...
                if (isInstantiatedType) {
                    ...
                    if (decTyAsNameTy.getQualifier() == null) {
                        while (it.hasNext() && instantiatedFacilityDecl == null) {
                            ...
                            Iterator<TypeFamilyDec> it2 = nextDec.getConceptDeclaredTypes().iterator();
                            while (it2.hasNext() && instantiatedFacilityDecl == null) {
                                if (decTyAsNameTy.getName().getName().equals(it2.next().getName().getName())) {
                        ...
                        while (it.hasNext() && instantiatedFacilityDecl == null) {
                            ...
                            InstantiatedFacilityDecl nextDec = it.next();
                            if (decTyAsNameTy.getQualifier().getName()
                                    .equals(nextDec.getInstantiatedFacilityName().getName())) {
                                instantiatedFacilityDecl = nextDec;
                            }
                        }
                    }
                    ...
                    if (instantiatedFacilityDecl == null) {
                        ...
                        FormalActualLists conceptFormalActuals = instantiatedFacilityDecl.getConceptParamArgLists();
                        modifiedClauseExp = replaceFormalWithActual(modifiedClauseExp,
                                conceptFormalActuals.getFormalParamList(), conceptFormalActuals.getActualArgList());

                        // Replace all concept's shared variables by adding
                        // the facility qualifier.
                        PosSymbol facName = instantiatedFacilityDecl.getInstantiatedFacilityName();
                        List<SharedStateDec> sharedStateDecs = instantiatedFacilityDecl.getConceptSharedStates();
                        for (SharedStateDec sharedStateDec : sharedStateDecs) {
                            ...
                            for (MathVarDec mathVarDec : sharedStateDec.getAbstractStateVars()) {
                                // Convert to VarExp
                                sharedVarsAsVarExps.add(Utilities.createVarExp(mathVarDec.getLocation().clone(), null,
                                ...
                                qualifiedSharedVars.add(Utilities.createVarExp(mathVarDec.getLocation().clone(),
                                        facName.clone(), mathVarDec.getName().clone(), mathVarDec.getMathType(), null));
                            }

                            // Replace in clause
                            modifiedClauseExp = replaceFormalWithActual(modifiedClauseExp, sharedVarsAsVarExps,
                                    qualifiedSharedVars);
                        }

                        // Replace all concept's definition variables by adding
                        // the facility qualifier.
                        List<TypeFamilyDec> typeFamilyDecs = instantiatedFacilityDecl.getConceptDeclaredTypes();
                        for (TypeFamilyDec typeFamilyDec : typeFamilyDecs) {
                            ...
                            for (MathDefVariableDec mathDefVariableDec : typeFamilyDec.getDefinitionVarList()) {
                                ...
                                // Convert to VarExp
                                defVarsExp.add(Utilities.createVarExp(mathVarDec.getLocation().clone(), null,
                                        mathVarDec.getName().clone(), mathVarDec.getMathType(), null));
                            ...
                            // Replace in clause
                            modifiedClauseExp = replaceFormalWithActual(modifiedClauseExp, defVarsExp,
                                    qualifiedDefVars);
                        }

                        // Replace concept realization formal parameters
                        // with actual instantiation arguments
                        FormalActualLists conceptRealizFormalActuals = instantiatedFacilityDecl
                                .getConceptRealizParamArgLists();
                        modifiedClauseExp = replaceFormalWithActual(modifiedClauseExp,
                                conceptRealizFormalActuals.getFormalParamList(),
                                conceptRealizFormalActuals.getActualArgList());

                        // Replace enhancement/enhancement realization's
                        // formal parameters with actual instantiation arguments.
                        List<InstantiatedEnhSpecRealizItem> items = instantiatedFacilityDecl
                                .getInstantiatedEnhSpecRealizItems();
                        for (InstantiatedEnhSpecRealizItem item : items) {
                            // Enhancement
                            FormalActualLists enhancementFormalActuals = item.getEnhancementParamArgLists();
                            modifiedClauseExp = replaceFormalWithActual(modifiedClauseExp,
                                    enhancementFormalActuals.getFormalParamList(),
                                    enhancementFormalActuals.getActualArgList());

                            // Enhancement Realization
                            FormalActualLists enhancementRealizFormalActuals = item.getEnhancementRealizParamArgLists();
                            modifiedClauseExp = replaceFormalWithActual(modifiedClauseExp,
                                    enhancementRealizFormalActuals.getFormalParamList(),
                                    enhancementRealizFormalActuals.getActualArgList());
                        }
                    }
                }
            }
    ...
    /**
     * <p>
     * This method is used to replace the module parameters with the actual instantiated arguments. Note that both of
     * these have been converted to mathematical expressions.
     * </p>
     *
     * @param exp
     ...
     *             This exception is thrown when we pass two lists that aren't equal in size.
     */
    public static Exp replaceFormalWithActual(Exp exp, List<VarExp> formalParams, List<Exp> actualArgs) {
        ...
        if (formalParams.size() == actualArgs.size()) {
            ...
            throw new MiscErrorException("[VCGenerator] Formal parameter size is different than actual argument size.",
                    new RuntimeException());
        }
    ...
    /**
     * <p>
     * Given a math symbol name, locate and return the {@link MathSymbolEntry} stored in the symbol table.
     * </p>
     *
     * @param loc
     *            The location in the AST that we are currently visiting.
     * @param name
     *            The string name of the math symbol.
     * @param scope
     *            The module scope to start our search.
     *
     * @return A {@link MathSymbolEntry} from the symbol table.
     */
    public static MathSymbolEntry searchMathSymbol(Location loc, String name, ModuleScope scope) {
        // Query for the corresponding math symbol
        MathSymbolEntry ms = null;
        try {
            ms = scope.queryForOne(new UnqualifiedNameQuery(name, ImportStrategy.IMPORT_RECURSIVE,
                    FacilityStrategy.FACILITY_IGNORE, true, true)).toMathSymbolEntry(loc);
    ...
    /**
     * <p>
     * Given the qualifier, name and the list of argument types, locate and return the {@link OperationEntry} stored in
     * the symbol table.
     * </p>
     *
     * @param loc
     ...
     */
    public static OperationEntry searchOperation(Location loc, PosSymbol qualifier, PosSymbol name,
            List<PTType> argTypes, ImportStrategy importStrategy, FacilityStrategy facilityStrategy,
            ModuleScope scope) {
        // Query for the corresponding operation
        OperationEntry op = null;
        try {
            op = scope.queryForOne(new OperationQuery(qualifier, name, argTypes, importStrategy, facilityStrategy));
        } catch (NoSuchSymbolException nsse) {
    ...
    /**
     * <p>
     * Given the name of the type locate and return the {@link SymbolTableEntry} stored in the symbol table.
     * </p>
     *
     * @param loc
     *            The location in the AST that we are currently visiting.
     ...
     *            The module scope to start our search.
     *
     * @return A {@link SymbolTableEntry} from the symbol table.
     */
    public static SymbolTableEntry searchProgramType(Location loc, PosSymbol qualifier, PosSymbol name,
            ModuleScope scope) {
        SymbolTableEntry retEntry = null;

        List<SymbolTableEntry> entries = scope.query(new NameQuery(qualifier, name, ImportStrategy.IMPORT_NAMED,
    ...
    /**
     * <p>
     * An helper method that throws the appropriate raw type not handled message.
     * </p>
     *
     * @param ty
     *            A raw type.
     * @param loc
     *            Location where this raw type was found.
     */
    public static void tyNotHandled(Ty ty, Location loc) {
        throw new SourceErrorException("[VCGenerator] Ty not handled: " + ty.toString(), loc);
    }
