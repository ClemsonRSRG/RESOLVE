...
public class ProgramFunctionExpWalker extends TreeWalkerVisitor {
    ...
    /**
     * <p>
     * The current procedure declaration we are processing.
     * </p>
     */
    ...
    /**
     * <p>
     * If this is a {@code Recursive Procedure}, then this will contain the {@code decreasing} clause.
     * </p>
     */
    ...
    /**
     * <p>
     * A map that contains the modified ensures clause with the formal replaced with the actuals for each of the nested
     * function calls.
     * </p>
     */
    ...
    /**
     * <p>
     * A list that contains the modified requires clauses with the formal replaced with the actuals for each of the
    ...
    /**
     * <p>
     * A list that contains any generated termination {@code Confirm} statements.
     * </p>
     */
     ...
     */
    public ProgramFunctionExpWalker(VerificationContext context, ModuleScope moduleScope, TypeGraph g) {
     ...
     *
     * @param entry
     *            The current visiting {@code Procedure} declaration's {@link OperationEntry}.
     * @param decreasingExp
     *            The {@code decreasing} clause for the visiting
     * @param context
     *            The verification context that contains all the information we have collected so far.
     * @param moduleScope
     *            The current module scope we are visiting.
     ...
     *            The current type graph.
     */
    public ProgramFunctionExpWalker(OperationEntry entry, Exp decreasingExp, VerificationContext context,
    ...
    public final void postProgramFunctionExp(ProgramFunctionExp exp) {
        ...
        OperationEntry operationEntry = Utilities.getOperationEntry(exp, myCurrentModuleScope);
                ...
                if (decl.getInstantiatedFacilityName().getName().equals(qualifier.getName())) {
                    ...
                    for (SharedStateDec stateDec : decl.getConceptSharedStates()) {
                        for (MathVarDec varDec : stateDec.getAbstractStateVars()) {
                            ...
                            VarExp varDecAsVarExp = Utilities.createVarExp(exp.getLocation().clone(), null,
                    ...
                    for (TypeFamilyDec typeFamilyDec : decl.getConceptDeclaredTypes()) {
                        for (MathDefVariableDec mathDefVariableDec : typeFamilyDec.getDefinitionVarList()) {
                            ...
                            VarExp defVarAsVarExp = Utilities.createVarExp(exp.getLocation().clone(), null,
                                    mathDefVariableDec.getName(), mathDefVariableDec.getMathType(), null);
        ...
        if (!VarExp.isLiteralTrue(operationDec.getRequires().getAssertionExp())) {
            ...
            Exp requiresExp = replaceFormalWithActualReq(operationDec.getRequires().getAssertionExp(),
                    operationDec.getParameters(), exp.getArguments());

            // Store the location detail for the function call's requires clause
            requiresExp.setLocationDetailModel(new LocationDetailModel(operationDec.getRequires().getLocation().clone(),
                    exp.getLocation().clone(), "Requires Clause of " + fullOperationName));

            // Replace any facility declaration instantiation arguments
            // in the requires clause.
            requiresExp = Utilities.replaceFacilityFormalWithActual(requiresExp, operationDec.getParameters(),
        ...
        Exp ensuresExp = replaceFormalWithActualEns(operationDec.getEnsures().getAssertionExp(),
                operationDec.getParameters(), exp.getArguments());

        // Store the location detail for the function call's ensures clause
        ensuresExp.setLocationDetailModel(new LocationDetailModel(operationDec.getEnsures().getLocation().clone(),
                exp.getLocation().clone(), "Ensures Clause of " + fullOperationName));

        // Replace any facility declaration instantiation arguments
        // in the ensures clause.
        ensuresExp = Utilities.replaceFacilityFormalWithActual(ensuresExp, operationDec.getParameters(),
                myCurrentModuleScope.getDefiningElement().getName(), myCurrentVerificationContext);
        ...
        if (myCurrentOperationEntry != null && myCurrentOperationEntry.equals(operationEntry)) {
     ...
     *
     * @param exp
     *            The outermost {@link ProgramFunctionExp} that we wish to extract an {@code ensures} clause for.
     *
     * @return The complete ensures clause as an {@link Exp}.
     *
     * @throws SourceErrorException
     *             This is thrown when we can't locate the ensures clause for {@code exp}.
     */
    public final Exp getEnsuresClause(ProgramFunctionExp exp) {
        ...
        if (myEnsuresClauseMap.containsKey(exp)) {
            ...
            throw new SourceErrorException("[VCGenerator] Cannot locate the ensures clause for: " + exp.toString()
                    + ". Our ensures clause map contains: " + myEnsuresClauseMap.toString(), exp.getLocation());
        }
    ...
    /**
     * <p>
     * This method returns the final modified requires clause after all the necessary replacement/substitutions have
     * been made.
     * </p>
     *
     * @param loc
     *            The location to be stored inside the requires clause generated by this walker.
     *
    ...
    public final Exp getRequiresClause(Location loc) {
        ...
        for (Exp exp : myRequiresClauseList) {
            ...
            // Else form a conjunct.
                ...
                allRequiresExp = InfixExp.formConjunct(loc.clone(), allRequiresExp, exp.clone());
            }
        }
    ...
    /**
     * <p>
     * This method returns the list of termination {@code Confirm} clauses (if any).
     * </p>
     *
     * @return {@code Confirm} clauses generated from recursive calls to our current {@code Procedure} declaration.
     */
    ...
    /**
     * <p>
     * An helper method for sanity checking the generated expression from the {@link ProgramFunctionExpWalker}.
     * </p>
     *
     * @param generatedExp
     *            The generated {@code ensures} clause expression.
     *
     * @return The appropriate mathematical form of the condition {@link Exp}.
     */
        ...
        if (generatedExp instanceof EqualsExp) {
            ...
            if (generatedExpAsEqualsExp.getLeft() instanceof VarExp) {
                ...
                throw new SourceErrorException(
                        "[VCGenerator] Condition expression: " + generatedExp.toString()
                                + " is not of the form: <OperationName> = <expression> " + generatedExp.getLocation(),
            ...
            throw new SourceErrorException(
                    "[VCGenerator] Condition expression: " + generatedExp.toString()
                            + " is not an equivalence expression " + generatedExp.getLocation(),
                    generatedExp.getLocation());
        }
    ...
    /**
     * <p>
     * An helper method that generates termination {@code Confirm} statements.
     * </p>
     *
     * @param functionExp
     *            A program function expression.
    ...
    private void generateTerminationConfirmStmt(ProgramFunctionExp functionExp) {
        ...
        if (myDecreasingExp == null) {
            throw new SourceErrorException(
                    "[VCGenerator] Cannot locate the decreasing clause for: " + functionExp.toString(),
                    functionExp.getLocation());
        } else {
            // Generate the termination of recursive call: 1 + P_Exp <= P_Val
            VarExp pValExp = Utilities.createPValExp(myDecreasingExp.getLocation().clone(), myCurrentModuleScope);
            IntegerExp oneExp = new IntegerExp(myDecreasingExp.getLocation().clone(), null, 1);
            oneExp.setMathType(myDecreasingExp.getMathType());

            InfixExp sumExp = new InfixExp(myDecreasingExp.getLocation().clone(), oneExp, null,
                    new PosSymbol(myDecreasingExp.getLocation().clone(), "+"), myDecreasingExp.clone());
            sumExp.setMathType(myDecreasingExp.getMathType());

            InfixExp terminationExp = new InfixExp(myDecreasingExp.getLocation().clone(), sumExp, null,
                    new PosSymbol(myDecreasingExp.getLocation().clone(), "<="), pValExp.clone());
            terminationExp.setMathType(myTypeGraph.BOOLEAN);

            // Store the location detail for the recursive function call's
            // termination expression.
            terminationExp.setLocationDetailModel(new LocationDetailModel(myDecreasingExp.getLocation().clone(),
                    functionExp.getLocation().clone(), "Termination of Recursive Call"));

            // Generate a new ConfirmStmt using terminationExp
            ConfirmStmt confirmStmt = new ConfirmStmt(terminationExp.getLocation().clone(), terminationExp, false);
    ...
    /**
     * <p>
     * An helper method that replaces the formal with the actual variables inside the {@code ensures} clause.
     * </p>
     *
     * @param ensures
     *            The {@code ensures} clause as an {@link Exp}.
     * @param paramList
     *            The list of parameter variables.
     * @param argList
     *            The list of arguments from the operation call.
     *
     * @return A potentially modified {@code ensures} clause represented using an {@link Exp}.
     */
    private Exp replaceFormalWithActualEns(Exp ensures, List<ParameterVarDec> paramList, List<ProgramExp> argList) {
        ...
        for (int i = 0; i < argList.size(); i++) {
            ...
            if (exp instanceof ProgramFunctionExp) {
                ...
                if (myEnsuresClauseMap.containsKey(exp)) {
                    ...
                    throw new SourceErrorException(
                            "[VCGenerator] Could not find the modified ensures clause of: " + exp.toString(),
                            exp.getLocation());
                }
            }
            ...
            VarExp paramExpAsVarExp = Utilities.createVarExp(varDec.getLocation(), null, varDec.getName(),
                    exp.getMathType(), exp.getMathTypeValue());

            // A temporary VarExp that avoids any formal with the same name as the actual.
            VarExp tempExp = Utilities.createVarExp(varDec.getLocation(), null,
                    new PosSymbol(varDec.getLocation(), "_" + varDec.getName().getName()), replExp.getMathType(),
                    replExp.getMathTypeValue());
    ...
    /**
     * <p>
     * An helper method that replaces the formal with the actual variables inside the {@code requires} clause.
     * </p>
     *
     * @param requires
     *            The {@code requires} clause as an {@link Exp}.
     * @param paramList
     *            The list of parameter variables.
     * @param argList
     *            The list of arguments from the operation call.
     *
     * @return A potentially modified {@code requires} clause represented using an {@link Exp}.
     */
    private Exp replaceFormalWithActualReq(Exp requires, List<ParameterVarDec> paramList, List<ProgramExp> argList) {
        // YS: We need two replacement maps in case we happen to have the
        // same names in formal parameter arguments and in the argument list.
        Map<Exp, Exp> paramToTemp = new HashMap<>();
        Map<Exp, Exp> tempToActual = new HashMap<>();
