...
import edu.clemson.rsrg.absyn.declarations.moduledecl.FacilityModuleDec;
...
/**
 * <p>
 * This is a sanity checker for making sure our uses list does not contain a {@code sharing concept} directly or is
 * importing an instantiated {@code facility} of a {@code sharing concept}.
 * </p>
 *
 * @author Yu-Shan Sun
 * 
...
public class NoSharingConceptImportChecker {
    ...
    /**
     * <p>
     * Location object from the uses item.
     * </p>
     */
    ...
    /**
     * <p>
     * The module being imported by the uses item.
     * </p>
     */
    ...
    /**
     * <p>
     * Creates a sanity checker for checking whether or not the we import a {@code sharing concept}.
     * </p>
     *
     * @param location
     *            The location for the uses item.
     * @param moduleDec
     *            The module being imported.
     * @param currentScope
     *            The current scope.
     */
    public NoSharingConceptImportChecker(Location location, ModuleDec moduleDec, ScopeBuilder currentScope) {
        myLocation = location;
    ...
    /**
     * <p>
     * This method indicates whether or not we are importing a {@code sharing concept}.
     * </p>
     *
     * @return {@code true} if this uses item imports a sharing concept, {@code false} otherwise.
     */
    public final boolean importingSharingConcept() {
        ...
        if (myModuleDec instanceof ConceptModuleDec) {
            ...
            FacilityEntry facilityEntry = getFacilityEntry(((ShortFacilityModuleDec) myModuleDec).getDec().getName());
            ...
            while (facilityDecIt.hasNext() && !retval) {
                ...
                if (dec instanceof FacilityDec) {
                    FacilityEntry facilityEntry = getFacilityEntry(dec.getName());
                    retval = facilityEntry.isSharingConceptInstantion();
                }
            }
        }
    ...
    /**
     * <p>
     * An helper method for querying for a facility entry.
     * </p>
     *
     * @param name
     *            A name for the facility declaration.
     *
     * @return A {@link FacilityEntry} if found. Otherwise a {@link SourceErrorException} will be thrown.
     */
    private FacilityEntry getFacilityEntry(PosSymbol name) {
        FacilityEntry entry;

        try {
            entry = myCurrentScope.queryForOne(
                    new NameQuery(null, name, ImportStrategy.IMPORT_RECURSIVE, FacilityStrategy.FACILITY_GENERIC, true))
                    .toFacilityEntry(myLocation);
        } catch (NoSuchSymbolException nsse) {
            throw new SourceErrorException("No module found with the given name: " + name, myLocation);
        } catch (DuplicateSymbolException dse) {
            throw new SourceErrorException("Duplicate symbol: " + name + ". Consider qualifying.", myLocation);
        }
