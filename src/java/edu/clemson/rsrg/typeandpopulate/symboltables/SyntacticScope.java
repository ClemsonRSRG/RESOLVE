...
/**
 * <p>
 * A <code>SyntacticScope</code> corresponds to a RESOLVE scope that arises because of a concrete piece of source code
 * (rather than, for example, the built-in global scope, or the top-level dummy scope), represented by a
 * {@link ResolveConceptualElement} called its <em>defining element</em>. Such a scope therefore exists in a <em>lexical
 * hierarchy</em> of those scopes introduced by its defining element's parent elements, necessarily passing through a
 * {@link ModuleScope} (belonging to this scope's <em>source module</em>) before reaching the top-level dummy scope. If
 * this scope corresponds to a module, it is its own source module.
 * </p>
 *
 * <p>
 * The symbols in this scope's lexical parents are therefore implicitly available from within this scope. Optionally,
 * symbols from the transitive closure of all modules imported by this scope's source module (its <em>recursive
 * imports</em>) are available, as are any symbols found in facilities defined in the source module or any modules
 * directly imported by the source module (its <em>named imports</em>, which do not recursively include any modules
 * imported from within named imports).
 * </p>
 *
 * <p>
 * Note that this class has no public constructor. Instances of this class can be retrieved from a
 * {@link ScopeRepository} or constructed via some of the methods of {@link MathSymbolTableBuilder}.
 * </p>
 *
 * @version 2.0
 */
public abstract class SyntacticScope extends AbstractScope {
     ...
     *            The symbol table bindings.
     */
    SyntacticScope(ScopeRepository source, ResolveConceptualElement definingElement, Scope parent,
    ...
    public final <E extends SymbolTableEntry> boolean addMatches(TableSearcher<E> searcher, List<E> matches,
        ...
        if (!searchedScopes.contains(this)) {
            ...
            if (instantiatingFacility != null) {
                symbolTableView = new InstantiatedSymbolTable(myBindings, genericInstantiations, instantiatingFacility);
            }

            finished = searcher.addMatches(symbolTableView, matches, l);

            if (!finished) {
                finished = myParent.addMatches(searcher, matches, searchedScopes, genericInstantiations,
    ...
    public final List<ProgramParameterEntry> getFormalParameterEntries() {
        ...
        Iterator<ProgramParameterEntry> formalBindings = myBindings.iterateByType(ProgramParameterEntry.class);
    ...
    /**
     * <p>
     * Returns the module identifier for the module that instantiated this scope.
     * </p>
     *
    ...
    public final <E extends SymbolTableEntry> E queryForOne(SymbolQuery<E> query)
        ...
        if (results.isEmpty()) {
            throw new NoSuchSymbolException("No entries found!", new IllegalStateException());
        } else if (results.size() > 1) {
            throw new DuplicateSymbolException("Found duplicate entries!", results.get(0));
        }
