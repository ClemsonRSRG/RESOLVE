...
/**
 * <p>
 * An implementation of {@link SymbolQuery} that decorates an existing <code>SymbolQuery</code>, post processing its
 * results and returning the processed set of results.
 * </p>
 *
 * @param <T>
 *            The return type of the base <code>SymbolQuery</code>.
 * @param <R>
 *            The return type of the resultant, processed entries.
 *
 * @version 2.0
 */
abstract class ResultProcessingQuery<T extends SymbolTableEntry, R extends SymbolTableEntry> implements SymbolQuery<R> {
    ...
    /**
     * <p>
     * This query uses the provided query to search for all entries of type <code>T</code> and further processes the
     * entries with type <code>R</code>.
     * </p>
     *
     * @param baseQuery
     *            The query that is being decorated.
     * @param processing
     *            A mapping between two types of entries.
     */
    protected ResultProcessingQuery(SymbolQuery<T> baseQuery, Mapping<T, R> processing) {
     ...
     */
    @Override
    public final List<R> searchFromContext(Scope source, ScopeRepository repo) throws DuplicateSymbolException {
    ...
    static class SimpleSymbolQuery extends BaseSymbolQuery<SymbolTableEntry> {
        ...
        /**
         * <p>
         * This query searches for all {@link SymbolTableEntry} that using a {@link TableSearcher}.
         * </p>
         */
        SimpleSymbolQuery(ScopeSearchPath path, TableSearcher<SymbolTableEntry> searcher) {
            super(path, searcher);
        }
