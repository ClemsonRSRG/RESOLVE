 ...
 */
package edu.clemson.rsrg.typeandpopulate.query.searchpath;
...
import edu.clemson.rsrg.typeandpopulate.symboltables.*;
...
public class UnqualifiedPath implements ScopeSearchPath {
     ...
     *            Boolean flag that indicates whether or not local items have priority.
     */
    public UnqualifiedPath(ImportStrategy importStrategy, FacilityStrategy facilityStrategy, boolean localPriority) {
     ...
     *
     * @param searcher
     *            A <code>TableSearcher</code> to apply to each scope along the search path.
     * @param source
     *            The current scope from which the search was spawned.
     * @param repo
     ...
     */
    @Override
    public final <E extends SymbolTableEntry> List<E> searchFromContext(TableSearcher<E> searcher, Scope source,
        ...
        searchModule(searcher, source, repo, result, searchedScopes, genericInstantiations, null, myImportStrategy, 0);
     ...
     *            Set of scopes searched.
     * @param <E>
     *            Entry type.
     *
     * @return {@code true} if we are done searching, {@code false} otherwise.
     */
    private <E extends SymbolTableEntry> boolean searchFacilities(TableSearcher<E> searcher, List<E> result,
            Scope source, Map<String, PTType> genericInstantiations, Set<Scope> searchedScopes)
            throws DuplicateSymbolException {

        List<FacilityEntry> facilities = source.getMatches(EntryTypeSearcher.FACILITY_SEARCHER,
        ...
        while (!finished && facilitiesIter.hasNext()) {
            ...
            facilityScope = facilityConcept.getScope(myFacilityStrategy.equals(FacilityStrategy.FACILITY_INSTANTIATE));

            finished = facilityScope.addMatches(searcher, result, searchedScopes, genericInstantiations, null,
                    SearchContext.FACILITY);

            // YS Edits
            // Search any enhancements in this facility declaration
            if (!finished) {
                List<ModuleParameterization> enhancementList = facility.getEnhancements();
                for (ModuleParameterization facEnh : enhancementList) {
                    // Obtain the scope for the enhancement
                    facilityScope = facEnh.getScope(myFacilityStrategy.equals(FacilityStrategy.FACILITY_INSTANTIATE));
     ...
     *            Facility that instantiated this class.
     * @param importStrategy
     *            An import strategy.
     * @param depth
     *            Integer flag that indicates how deep we search.
     * @param <E>
     *            Entry type.
     *
     * @return {@code true} if we are done searching, {@code false} otherwise.
    ...
    private <E extends SymbolTableEntry> boolean searchModule(TableSearcher<E> searcher, Scope source,
            ScopeRepository repo, List<E> results, Set<Scope> searchedScopes, Map<String, PTType> genericInstantiations,
            FacilityEntry instantiatingFacility, ImportStrategy importStrategy, int depth)
            throws DuplicateSymbolException {

        // First we search locally
        boolean finished = source.addMatches(searcher, results, searchedScopes, genericInstantiations,
        ...
        if (!finished && myFacilityStrategy != FacilityStrategy.FACILITY_IGNORE) {
            ...
            finished = searchFacilities(searcher, results, source, genericInstantiations, searchedScopes);
        }
        ...
        if ((results.isEmpty() || !myLocalPriorityFlag) && source instanceof SyntacticScope
            ...
            try {
                ...
                while (!finished && importsIter.hasNext()) {
                    ...
                    finished = searchModule(searcher, importScope, repo, results, searchedScopes, genericInstantiations,
                            instantiatingFacility, importStrategy.cascadingStrategy(), depth + 1);
                }
