...
/**
 * <p>
 * A <code>MathFunctionNamedQuery</code> takes a (possibly-null) qualifier and a name and searches for all
 * {@link MathSymbolEntry} that match. If the qualifier is non-null, the appropriate facility or module is searched. If
 * it <em>is</em> null, a search is performed using a recursive <code>ImportStrategy</code> and ignores all
...
public class MathFunctionNamedQuery implements MultimatchSymbolQuery<MathSymbolEntry> {
    ...
    /**
     * <p>
     * This query searches for all {@link MathSymbolEntry} that match the given name represented as a {@link PosSymbol}
     * using a recursive import strategy, no facility strategy and no local priority.
     * </p>
     *
     * @param qualifier
    ...
    public MathFunctionNamedQuery(PosSymbol qualifier, PosSymbol name) {
        myNameQuery = new NameQuery(qualifier, name, ImportStrategy.IMPORT_RECURSIVE,
