 ...
 * {@link #visitFile(Path, BasicFileAttributes)} method and each directory via optional {@link #preVisitDirectory} or
 * {@link #postVisitDirectory} methods. Override others as needed.
 * </p>
 *
 * @author Yu-Shan Sun
 * @author Daniel Welch
...
public class FileLocator extends SimpleFileVisitor<Path> {
    ...
    /**
     * Constructs a new {@code FileLocator} that will match based on the {@code (pattern, extensions)} pair provided.
     *
     * @param pattern
     *            An extensionless pattern.
     * @param extensions
     *            An list of valid extensions to choose from after a myPattern is matched (e.g.
     *            {@code ["java", "cpp", "groovy"]}).
     */
    public FileLocator(String pattern, List<String> extensions) {
        myPattern = pattern;
        myPathMatcher = FileSystems.getDefault().getPathMatcher("glob:" + pattern + "." + parseExtensions(extensions));
    }

    /**
     * Constructs a new {@code FileLocator} that will match based on the {@code extensions} provided.
     *
     * @param extension
     *            An list of valid extensions to choose from after a myPattern is matched (e.g.
     *            {@code ["java", "cpp", "groovy"]}).
    ...
    public FileLocator(String extension) {
        myPathMatcher = FileSystems.getDefault().getPathMatcher("glob:*.{" + extension + "}");
    }
     ...
     *             If a file matching pattern could not be found or if we found more than one file.
     *
     * @return The matching file.
     */
    public final File getFile() throws FileSystemException {
        if (myMatches.size() == 0) {
            throw new NoSuchFileException("File matching name '" + myPattern + "' could not be found");
        } else if (myMatches.size() > 1) {
            throw new FileSystemException("Found more than one file matching the name '" + myPattern + "'.");
        }
    ...
    /**
     * <p>
     * Using a path and the basic file attributes, attempt to find the file that matches.
     * </p>
     *
     * @param file
     ...
     *
     * @param extensions
     *            The list of searching extensions.
     *
