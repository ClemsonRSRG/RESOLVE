...
import edu.clemson.rsrg.init.output.FileOutputListener;
import edu.clemson.rsrg.init.output.OutputListener;
import edu.clemson.rsrg.statushandling.StatusHandler;
import edu.clemson.rsrg.statushandling.WriterStatusHandler;
...
import edu.clemson.rsrg.init.flag.FlagManager;
...
/**
 * <p>
 * This class stores all necessary objects and flags needed during the compilation environment.
 * </p>
 *
 * @author Yu-Shan Sun
 * @author Daniel Welch
...
public class CompileEnvironment {
    ...
    /**
     * <p>
     * This contains the absolute path to the RESOLVE workspace directory.
     * </p>
     */
    ...
    /**
     * <p>
     * This contains all modules we have currently seen. This includes both complete and incomplete modules. A module is
     * complete when we are done processing it. An incomplete module usually means that we are still processing it's
     * import.
     * </p>
     */
    ...
    /**
     * <p>
     * This map stores all externally realizations for a particular concept. The {@code Archiver} should be the only one
     * that cares about these files.
     * </p>
     */
    ...
    /**
     * <p>
     * This is the default status handler for the RESOLVE compiler.
     * </p>
     */
    ...
    /**
     * <p>
     * This list stores all the incomplete modules.
     * </p>
     */
    ...
    /**
     * <p>
     * This list stores listener objects that provides instant feedback to the interested party when we are done with a
    ...
    /**
     * <p>
     * This stores all user created files from the WebIDE/WebAPI.
     * </p>
     */
    ...
    /**
     * <p>
     * This object contains all the flag objects that have been created by the different modules.
     * </p>
     */
    ...
    /**
     * <p>
     * Instantiates a compilation environment to store all necessary modules, files and flags.
     * </p>
     *
     * @param args
     ...
     *             There was some kind of dependency error with the user specified flags.
     * @throws IOException
     *             There was an error creating the specified error log file.
     */
    public CompileEnvironment(String[] args, String compilerVersion, StatusHandler statusHandler)
            throws FlagDependencyException, IOException {
        flags = new FlagManager(args);
        myCompilingModules = new LinkedHashMap<>();
        ...
        if (flags.isFlagSet(ResolveCompiler.FLAG_WORKSPACE_DIR)) {
            path = flags.getFlagArgument(ResolveCompiler.FLAG_WORKSPACE_DIR, "Path");
        }
        ...
        if (flags.isFlagSet(ResolveCompiler.FLAG_DEBUG_FILE_OUT)) {
            ...
            Path infoFilePath = Paths.get(myCompileDir.getAbsolutePath(), "Log-" + dateFormat.format(date) + ".log");
            Path errorFilePath = Paths.get(myCompileDir.getAbsolutePath(),
                    "Debug-Log-" + dateFormat.format(date) + ".log");
            Charset charset = Charset.forName("UTF-8");

            statusHandler = new WriterStatusHandler(Files.newBufferedWriter(infoFilePath, charset, CREATE, APPEND),
        ...
        if (flags.isFlagSet(ResolveCompiler.FLAG_DEBUG)) {
            synchronized (System.out) {
                ...
                myStatusHandler.info(null, "RESOLVE Compiler/Verifier - " + compilerVersion + " Version.");
    ...
    /**
     * <p>
     * Adds this file as an externally realized file.
     * </p>
     *
     * @param id
     *            The ID for the {@link ResolveFile} that we want to set as externally realized.
     * @param file
     *            The externally realized file.
     */
     ...
     *
     * @param listener
     *            A new {@link OutputListener} object.
     */
    ...
    /**
     * <p>
     * Remove the module associated with the {@link ModuleIdentifier} from our incomplete module stack. This indicates
     * the completion of this module.
     * </p>
     *
     * @param mid
     *            Completed module's identifier.
     */
    ...
    /**
     * <p>
     * Constructs a record containing the module id, the file, and the module dec, and places it in the module
     * environment. Also places the module into a stack that indicates compilation has begun on this module but has not
     * completed.
     * </p>
     *
     * @param file
     ...
     * @param moduleDec
     *            The module representation declaration.
     */
    public final void constructRecord(ResolveFile file, ModuleDec moduleDec) {
        ...
        myCompilingModules.put(mid, new AbstractMap.SimpleEntry<>(moduleDec, file));
        myIncompleteModules.add(mid);
    }

    /**
     * <p>
     * Returns true if the specified module is present in the compilation environment, has an associated file and a
     * valid module dec.
     * </p>
     *
     * @param id
     *            The ID for the {@link ResolveFile} we want to search for.
     *
     * @return {@code true} if we have compiled this {@link ModuleIdentifier}, {@code false} otherwise.
     */
    ...
    /**
     * <p>
     * Returns the {@link ModuleDec} associated with the specified id.
     * </p>
     *
     * @param id
     *            The ID for the {@link ResolveFile} we want to search for.
    ...
    /**
     * <p>
     * Returns the remaining arguments not handled by the compile environment.
     * </p>
     *
    ...
    /**
     * <p>
     * Returns all the output listeners that are interested in the compilation results.
     * </p>
     *
    ...
    /**
     * <p>
     * Returns the compiler's status handler object.
     * </p>
     *
    ...
    /**
     * <p>
     * The symbol table containing all symbol information.
     * </p>
     *
    ...
    /**
     * <p>
     * Returns {@link ResolveFile} for the specified string object. Notice that the pre-condition for this method is
     * that the key exist in the map.
     * </p>
     *
     * @param fileBasicInfo
     ...
     */
    public final ResolveFile getUserFileFromMap(ResolveFileBasicInfo fileBasicInfo) {
        return myUserFileMap.get(fileBasicInfo);
    }

    /**
     * <p>
     * Returns a pointer to the current RESOLVE workspace directory.
     * </p>
     *
    ...
    /**
     * <p>
     * This checks to see if the module associated with this id is an externally realized file.
     * </p>
     *
     * @param id
     *            The ID for the {@link ResolveFile} we want to search for.
    ...
    /**
     * <p>
     * This checks to see if the module associated with this id has been compiled or not.
     * </p>
     *
     * @param id
     *            The ID for the {@link ResolveFile} we want to search for.
    ...
    /**
     * <p>
     * This checks to see if the file is a user created file from the WebIDE/WebAPI.
     * </p>
     *
     * @param fileBasicInfo
     *            The name of the file including any known parent directory.
     *
     * @return {@code true} if it is a user created file from the WebIDE/WebAPI, {@code false} otherwise.
     */
    ...
    /**
     * <p>
     * Used to set a map of user files when invoking the compiler from the WebIDE/WebAPI.
     * </p>
     *
     * @param fMap
     *            The map of user created files.
     */
    ...
    /**
     * <p>
     * Sets this table as our new symbol table.
     * </p>
     *
     * @param table
     *            The newly created and blank symbol table.
     */
    public final void setSymbolTable(ScopeRepository table) {
        if (table == null) {
            throw new MiscErrorException("Symbol table may not be set to null!", new IllegalArgumentException());
        }

        if (mySymbolTable != null) {
            throw new MiscErrorException("Symbol table may only be set once!", new IllegalStateException());
        }
    ...
    /**
     * <p>
     * Sets a new type graph to indicate relationship between types.
     * </p>
     *
     * @param t
     *            The newly created type graph.
     */
