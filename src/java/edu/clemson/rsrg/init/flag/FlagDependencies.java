...
/**
 * <p>
 * <code>FlagDependencies</code> represents a static container for information about the dependencies between
 * <code>Flag</code>s. It can be <em>sealed</em> to prevent further changes from being made.
 * </p>
 *
...
public class FlagDependencies {
    ...
    private static final Map<String, Flag> myFlagNames = new HashMap<>();

    private static final Map<String, List<Flag>> myFlagSections = new HashMap<>();

    private static final Map<Flag, FlagInfo> myFlags = new HashMap<>();

    private static final Set<Flag> myFlagsWithDefaultArguments = new HashSet<>();

    /**
     * <p>
     * Set to true once this class has been sealed. Once sealed, no new flags or dependencies may be created.
     * </p>
     */
    ...
    /**
     * <p>
     * Declaring the default constructor private stops anyone from instantiating this class, as it is intended to be
     * used only statically.
     * </p>
     */
    ...
    /**
     * <p>
     * Seals the dependencies. Further attempts to create <code>Flag</code>s or dependencies between them will throw an
     ...
     * </p>
     *
     * @return <code>true</code> <strong>iff</strong> <code>seal()</code> has already been called.
     */
    ...
    static void introduceFlag(Flag f) {
        ...
        if (mySealedFlag) {
            throw new IllegalStateException("Cannot create a new flag after "
                    ...
                    + "flag static and included a call to your setUpFlags() " + "method in setUpFlagDependencies()?");
        }

        if (myFlags.containsKey(f)) {
            throw new DuplicateEntryException(f.getName());
        }
    ...
    /**
     * <p>
     * Returns the <code>Flag</code> corresponding to a particular invocation, that is: a hyphen plus the name of the
     * flag, as it would appear on the command line.
     * </p>
     *
     * @param name
     *            The invocation, which must take the form of <code>"-" + someFlagName</code>.
     *
     ...
     */
    public static Flag getFlagByInvocation(String name) throws NoSuchElementException {
    ...
    static Set<Flag> getFlagImplications(Flag f) {
        ...
        return new HashSet<>(info.implies);
    }
        ...
        if (!found) {
            ...
            if (s.size() == 1) {
                throw new FlagDependencyException(
                        "The -" + m.getFlagReason(f) + " flag requires that the " + flagList + " flag be " + "set.");
            } else {
    ...
    /**
     * <p>
     * Returns a <code>Set</code> containing all <code>Flag</code>s that have default arguments provided (and should
     * therefore be set automatically).
     * </p>
     *
     * @return A <code>Set</code> of all <code>Flag</code>s with default arguments.
     */
    ...
    /**
     * <p>
     * Creates a <em>requires</em> relationship between two <code>Flag</code>s. If the user specifies the
     * <code>ifOn</code> flag, but not the <code>require</code> flag, an error will be generated.
     * </p>
     *
     * @param ifOn
     *            The flag that requires the other.
     * @param require
     *            The flag that must be on if <code>ifOn</code> is on.
     */
    public static void addRequires(Flag ifOn, Flag require) {
        checkSealedState();

        Set<Flag> s = new HashSet<>();
        s.add(require);
    ...
    /**
     * <p>
     * Creates a <em>requires at least one of</em> relationship between a flag and a set of others.
     * </p>
     *
     * @param ifOn
     *            The flag that requires at least one of the others.
     * @param requireOne
     *            The flags at least one of which must be on it <code>ifOn</code> is on.
     */
    ...
    /**
     * <p>
     * Creates an <em>implies</em> relationship between two flags. If the user turns on the <code>ifOn</code> flag, the
     * <code>impliesOn</code> flag will be automatically turned on too.
     * </p>
     *
     * @param ifOn
     *            The flag that implies the other.
     * @param impliesOn
     *            The flag that should be turned on if <code>ifOn</code> is on.
     *
     * @throws IllegalArgumentException
     *             If <code>impliesOn</code> takes parameters. Flags with parameters may not be implied.
     */
    public static void addImplies(Flag ifOn, Flag impliesOn) {
        ...
        if (impliesOn.getParameterCount() != 0) {
            throw new IllegalArgumentException("A flag that takes parameters " + "cannot be implied.  " + impliesOn
                    + " takes " + impliesOn.getParameterCount() + " parameters.");
        }
    ...
    /**
     * <p>
     * Creates an <em>excludes</em> relationship between two flags. If the user specifies both <code>ifOn</code>
     * <em>and</em> <code>exclude</code>, an error will be generated.
     * </p>
     *
     * @param ifOn
     *            The flag that excludes the other.
     * @param exclude
     *            The flag that must not be on if <code>ifOn</code> is on.
     */
    ...
    private static void checkFlagHasBeenIntroduced(Flag f) {
        if (!myFlags.containsKey(f)) {
            throw new MiscErrorException(f.getName(), new NoSuchElementException());
        }
    }
    ...
    private static void checkSealedState() {
        if (mySealedFlag) {
            throw new IllegalStateException(
                    "Cannot create dependencies " + "after dependency set-up time.  Are you sure you "
     ...
     *
     * @param extended
     *            Whether or not the listing should represent an extended listing.
     *
     * @return The listing.
     */
        ...
        for (Map.Entry<String, List<Flag>> e : myFlagSections.entrySet()) {
            ...
            for (Flag f : flags) {
                if (f.shouldDisplay(extended)) {
                    retval += wordWrap("    -" + formatInto(f.getName(), 20) + "  " + f.getDescription() + "\n", 80,
                            27);
                }
            }
        }
    ...
    private static class FlagInfo {
         ...
         * </p>
         */
        public final Set<Set<Flag>> requires = new HashSet<>();

        public final Set<Flag> implies = new HashSet<>();

        public final Set<Flag> excludes = new HashSet<>();
    }
}
