 ...
 */
package edu.clemson.rsrg.parsing.utilities;
...
import edu.clemson.rsrg.absyn.declarations.variabledecl.AbstractVarDec;
...
/**
 * <p>
 * This class performs the various different syntactic sugar conversions using part of the RESOLVE abstract syntax tree.
...
public class SyntacticSugarConverter extends TreeWalkerVisitor {
    ...
    /**
     * <p>
     * Once we are done walking the tree, the top most node will create the new element.
     * </p>
     */
    ...
    /**
     * <p>
     * This stores the new statements created by current statement we are visiting.
     * </p>
     */
    ...
    /**
     * <p>
     * This stores the {@link ParameterVarDec}, {@link FacilityDec} and {@link VarDec} obtained from the parent node.
     * </p>
     */
    ...
    /**
     * <p>
     * This is a map from the original {@link ResolveConceptualElement} to the replacing
    ...
    /**
     * <p>
     * This class attempts to convert all syntactic sugar before we generate the final RESOLVE AST.
     * </p>
     *
     * @param arrayNameTyToInnerTyMap
     *            A map that provides mapping from newly declared array name types to the types of elements in the
     *            array.
     * @param typeRepresentationDecList
     *            The list of type representation declarations.
     * @param sharedStateRealizationDecs
     *            The list of shared state representation declarations.
     * @param newElementCounter
     *            A counter that indicates the number of elements created by the tree building process.
     */
    public SyntacticSugarConverter(Map<NameTy, NameTy> arrayNameTyToInnerTyMap,
            ...
            List<AbstractSharedStateRealizationDec> sharedStateRealizationDecs, int newElementCounter) {
    ...
    /**
     * <p>
     * This should be the top-most node that we start with when processing syntactic sugar conversions inside a
     * {@link FacilityInitFinalItem}.
     * </p>
     *
     * @param e
     *            Current {@link FacilityInitFinalItem} we are visiting.
     */
    @Override
    public void preFacilityInitFinalItem(FacilityInitFinalItem e) {
        // Store the params, facility and variable declarations
        myParentNodeElementsContainer = new ParentNodeElementsContainer(new ArrayList<ParameterVarDec>(),
                e.getFacilities(), e.getVariables());

        // Create a new collector
        myResolveElementCollectorStack.push(new ResolveConceptualElementCollector(e));
    }

    /**
     * <p>
     * This should be the last element we walk. All syntactic sugar should have been performed and we can safely create
     * the new {@link FacilityInitFinalItem} to be returned.
     * </p>
     *
     * @param e
     *            Current {@link FacilityInitFinalItem} we are visiting.
    ...
    public void postFacilityInitFinalItem(FacilityInitFinalItem e) {
        ...
        ResolveConceptualElementCollector collector = myResolveElementCollectorStack.pop();
        myFinalProcessedElement = new FacilityInitFinalItem(e.getLocation().clone(), e.getItemType(), affectsClause,
                e.getRequires().clone(), e.getEnsures().clone(), myParentNodeElementsContainer.facilityDecs,
     ...
     * <p>
     * This should be the top-most node that we start with when processing syntactic sugar conversions inside a
     * {@link OperationProcedureDec}.
     * </p>
     *
     * @param e
     *            Current {@link OperationProcedureDec} we are visiting.
     */
    @Override
    public void preOperationProcedureDec(OperationProcedureDec e) {
        // Store the params, facility and variable declarations
        myParentNodeElementsContainer = new ParentNodeElementsContainer(e.getWrappedOpDec().getParameters(),
                e.getFacilities(), e.getVariables());
     ...
     * <p>
     * This should be the last element we walk. All syntactic sugar should have been performed and we can safely create
     * the new {@link OperationProcedureDec} to be returned.
     * </p>
     *
     * @param e
     *            Current {@link OperationProcedureDec} we are visiting.
    ...
    public void postOperationProcedureDec(OperationProcedureDec e) {
        ...
        OperationDec newOperationDec = new OperationDec(opDec.getName().clone(),
                myParentNodeElementsContainer.parameterVarDecs, returnTy, affectsClause, opDec.getRequires().clone(),
        ...
        ResolveConceptualElementCollector collector = myResolveElementCollectorStack.pop();
        myFinalProcessedElement = new OperationProcedureDec(newOperationDec, decreasingClause,
                myParentNodeElementsContainer.facilityDecs, myParentNodeElementsContainer.varDecs, collector.stmts,
                recursiveFlag);
     ...
     *
     * @param e
     *            Current {@link ProcedureDec} we are visiting.
     */
    @Override
    public void preProcedureDec(ProcedureDec e) {
        // Store the params, facility and variable declarations
        myParentNodeElementsContainer = new ParentNodeElementsContainer(e.getParameters(), e.getFacilities(),
                e.getVariables());
     ...
     * <p>
     * This should be the last element we walk. All syntactic sugar should have been performed and we can safely create
     * the new {@link ProcedureDec} to be returned.
     * </p>
     *
     * @param e
     *            Current {@link ProcedureDec} we are visiting.
    ...
    public void postProcedureDec(ProcedureDec e) {
        ...
        // Build the new ProcedureDec
        ResolveConceptualElementCollector collector = myResolveElementCollectorStack.pop();
        myFinalProcessedElement = new ProcedureDec(e.getName().clone(), myParentNodeElementsContainer.parameterVarDecs,
                returnTy, affectsClause, decreasingClause, myParentNodeElementsContainer.facilityDecs,
                myParentNodeElementsContainer.varDecs, collector.stmts, recursiveFlag);
     ...
     * <p>
     * This should be the top-most node that we start with when processing syntactic sugar conversions inside a
     * {@link RealizInitFinalItem}.
     * </p>
     *
     * @param e
     *            Current {@link RealizInitFinalItem} we are visiting.
     */
    @Override
     ...
     * <p>
     * This should be the last element we walk. All syntactic sugar should have been performed and we can safely create
     * the new {@link RealizInitFinalItem} to be returned.
     * </p>
     *
     * @param e
     *            Current {@link RealizInitFinalItem} we are visiting.
    ...
    public void postRealizInitFinalItem(RealizInitFinalItem e) {
        ...
        // Build the new ProcedureDec
        ResolveConceptualElementCollector collector = myResolveElementCollectorStack.pop();
        myFinalProcessedElement = new RealizInitFinalItem(e.getLocation().clone(), e.getItemType(), affectsClause,
                myParentNodeElementsContainer.facilityDecs, myParentNodeElementsContainer.varDecs, collector.stmts);
    ...
    /**
     * <p>
     * This statement could have syntactic sugar conversions, so we will need to have a way to store the new
     * {@link Statement}s that get generated.
     * </p>
     *
     * @param e
     *            Current {@link CallStmt} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * This statement could have syntactic sugar conversions, so it checks to see if we have generated new
     * {@link Statement}s and place those in the appropriate location.
     * </p>
     *
     * @param e
     *            Current {@link CallStmt} we are visiting.
    ...
    public void postCallStmt(CallStmt e) {
        ...
        if (myReplacingElementsMap.containsKey(e.getFunctionExp())) {
            exp = (ProgramFunctionExp) myReplacingElementsMap.remove(e.getFunctionExp());
        } else {
        ...
        while (!myNewStatementsContainer.newPostStmts.isEmpty()) {
            addToInnerMostCollector(myNewStatementsContainer.newPostStmts.remove());
        }
    ...
    /**
     * <p>
     * This statement doesn't need to do any syntactic sugar conversions, therefore we create a new {@link ConfirmStmt}
     * for future use.
     * </p>
     *
     * @param e
     *            Current {@link ConfirmStmt} we are visiting.
     */
    @Override
     ...
     *
     * @param e
     *            Current {@link FuncAssignStmt} we are visiting.
     */
    @Override
    ...
    public void postFuncAssignStmt(FuncAssignStmt e) {
        ...
        boolean isLeftArrayExp = ArrayConversionUtilities.isProgArrayExp(leftExp);
        boolean isRightArrayExp = ArrayConversionUtilities.isProgArrayExp(rightExp);
        ...
        if (!isLeftArrayExp && isRightArrayExp) {
            ...
            ProgramVariableExp arrayNameExp = ArrayConversionUtilities.getArrayNameExp(rightExp);
            ProgramExp arrayIndexExp = ArrayConversionUtilities.getArrayIndexExp(rightExp);
            NameTy arrayTy = findArrayType(arrayNameExp);

            newStatement = new FuncAssignStmt(l.clone(), (ProgramVariableExp) leftExp.clone(), ArrayConversionUtilities
                    .buildEntryReplicaCall(l, arrayTy.getQualifier(), arrayNameExp, arrayIndexExp));
        }
        ...
        // index and "x" is a variable.)
            ...
            // Obtain the array type, name and index
            ProgramVariableExp arrayNameExp = ArrayConversionUtilities.getArrayNameExp(leftExp);
            ProgramExp arrayIndexExp = ArrayConversionUtilities.getArrayIndexExp(leftExp);
            NameTy arrayTy = findArrayType(arrayNameExp);

            newStatement = ArrayConversionUtilities.buildAssignEntryCall(l.clone(), rightExp.clone(),
                    arrayTy.getQualifier(), arrayNameExp, arrayIndexExp);
        }
        ...
        // "i" and "j" are indexes)
            ...
            // Obtain the array type, name and index
            ProgramVariableExp leftArrayNameExp = ArrayConversionUtilities.getArrayNameExp(leftExp);
            ProgramVariableExp rightArrayNameExp = ArrayConversionUtilities.getArrayNameExp(rightExp);
            ProgramExp leftArrayIndexExp = ArrayConversionUtilities.getArrayIndexExp(leftExp);
            ProgramExp rightArrayIndexExp = ArrayConversionUtilities.getArrayIndexExp(rightExp);
            ...
            newStatement = ArrayConversionUtilities.buildAssignEntryCall(l.clone(), newEntryReplicaCall,
                    leftArrayTy.getQualifier(), leftArrayNameExp, leftArrayIndexExp);
        }
        ...
        // FuncAssignStmt.
            ...
            if (rightExp instanceof ProgramFunctionExp) {
                ...
                newRightExp = new ProgramFunctionExp(rightExp.getLocation().clone(), null,
                        new PosSymbol(rightExp.getLocation().clone(), "Replica"), args);
            }

            newStatement = new FuncAssignStmt(l.clone(), (ProgramVariableExp) leftExp.clone(), newRightExp);
        }
     ...
     *
     * @param e
     *            Current {@link IfStmt} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * We are done visiting this node, therefore we create a new {@link IfStmt} for future use.
     * </p>
     *
     * @param e
     *            Current {@link IfStmt} we are visiting.
    ...
    public void postIfStmt(IfStmt e) {
        ...
        if (!collector.instantiatingElement.equals(e)) {
            throw new MiscErrorException("Something went wrong during the syntactic sugar conversion",
        ...
        addToInnerMostCollector(
                new IfStmt(e.getLocation().clone(), newIfConditionItem, newElseIfItems, collector.stmts));
    }

    /**
     * <p>
     * This statement doesn't need to do any syntactic sugar conversions, therefore we create a new {@link MemoryStmt}
     * for future use.
     * </p>
     *
     * @param e
     *            Current {@link MemoryStmt} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * This statement doesn't need to do any syntactic sugar conversions, therefore we create a new {@link PresumeStmt}
     * for future use.
     * </p>
     *
     * @param e
     *            Current {@link PresumeStmt} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * This statement could have syntactic sugar conversions, so it generates the appropriate "swap" call.
     * </p>
     *
     * @param e
     *            Current {@link SwapStmt} we are visiting.
     */
    @Override
    public void postSwapStmt(SwapStmt e) {
        ...
        if (!isLeftArrayExp && isRightArrayExp) {
            ...
            // New "Swap_Entry" call
            newStatement = ArrayConversionUtilities.buildSwapEntryCall(l, leftExp, arrayTy.getQualifier(), arrayNameExp,
                    arrayIndexExp);
        }
        // Case #2: Right expression is not an expression that is a ProgramVariableArrayExp,
        // but the left is a ProgramVariableArrayExp.
        ...
        // index and "x" is a variable.)
            ...
            // New "Swap_Entry" call
            newStatement = ArrayConversionUtilities.buildSwapEntryCall(l, rightExp, arrayTy.getQualifier(),
        ...
        // "i" and "j" are indexes)
            ...
            if (!leftArrayNameExp.equivalent(rightArrayNameExp)) {
                ...
                if (rightArrayNameExp instanceof ProgramVariableNameExp) {
                    rightArrayNameAsPosSymbol = ((ProgramVariableNameExp) rightArrayNameExp).getName();
                } else {
                    StringBuilder sb = new StringBuilder();
                    Iterator<ProgramVariableExp> segsIt = ((ProgramVariableDotExp) rightArrayNameExp).getSegments()
                            .iterator();
                    while (segsIt.hasNext()) {
                        ProgramVariableNameExp current = (ProgramVariableNameExp) segsIt.next();
                    ...
                    rightArrayNameAsPosSymbol = new PosSymbol(rightArrayNameExp.getLocation().clone(), sb.toString());
                }

                throw new SourceErrorException("Cannot swap elements between two different arrays!",
                        rightArrayNameAsPosSymbol);
            }

            // New "Swap_Two_Entries" call
            NameTy arrayTy = findArrayType(leftArrayNameExp);
            newStatement = ArrayConversionUtilities.buildSwapTwoEntriesCall(l, arrayTy.getQualifier(), leftArrayNameExp,
                    leftArrayIndexExp, rightArrayIndexExp);
        }
        // Case #4: If it is not cases 1-4, then we build a regular
        // SwapStmt.
        else {
            newStatement = new SwapStmt(l.clone(), (ProgramVariableExp) leftExp.clone(),
    ...
    /**
     * <p>
     * This loop condition could have syntactic sugar conversions, so we will need to have a way to store the new
     * {@link Statement}s that get generated.
     * </p>
     *
     * @param e
     *            Current {@link WhileStmt} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * After visiting the loop condition, we could have generated new {@link Statement}s, so we will need to place those
     ...
     *            Current {@link WhileStmt} we are visiting.
     * @param previous
     *            The previous {@link ResolveConceptualElement} visited.
     * @param next
     *            The next {@link ResolveConceptualElement} to visit.
     */
    @Override
    public void midWhileStmt(WhileStmt e, ResolveConceptualElement previous, ResolveConceptualElement next) {
        ...
        if (previous instanceof ProgramExp && next instanceof LoopVerificationItem) {
            // Obtain the right container to insert the statements that appear before
            // this WhileStmt.
            ResolveConceptualElementCollector whileStmtCollector = myResolveElementCollectorStack.pop();
    ...
    /**
     * <p>
     * We are done visiting this node, therefore we create a new {@link WhileStmt} for future use.
     * </p>
     *
     * @param e
     *            Current {@link WhileStmt} we are visiting.
    ...
    public void postWhileStmt(WhileStmt e) {
        ...
        if (myReplacingElementsMap.containsKey(conditionExp)) {
            newConditionExp = (ProgramExp) myReplacingElementsMap.remove(conditionExp);
        } else {
        ...
        addToInnerMostCollector(new WhileStmt(e.getLocation().clone(), newConditionExp, newItem, collector.stmts));
    }
    ...
    /**
     * <p>
     * This if-condition could have syntactic sugar conversions, so we will need to have a way to store the new
     * {@link Statement}s that get generated.
     * </p>
     *
     * @param e
     *            Current {@link IfConditionItem} we are visiting.
     */
    @Override
    ...
    /**
     * <p>
     * After visiting the if-condition, we could have generated new {@link Statement}s, so we will need to place those
     * in the appropriate locations.
     * </p>
     *
     * @param e
    ...
    public void midIfConditionItem(IfConditionItem e, ResolveConceptualElement previous,
        ...
        if (previous instanceof ProgramExp && next instanceof VirtualListNode) {
            ...
            ResolveConceptualElementCollector ifConditionItemCollector = myResolveElementCollectorStack.pop();
            ResolveConceptualElementCollector ifStmtCollector = myResolveElementCollectorStack.pop();
            ...
            while (!myNewStatementsContainer.newPostStmts.isEmpty()) {
                Statement statement = myNewStatementsContainer.newPostStmts.remove();
    ...
    /**
     * <p>
     * We are done visiting this node, therefore we create a new {@link IfConditionItem} for future use.
     * </p>
     *
     * @param e
     *            Current {@link IfConditionItem} we are visiting.
    ...
    public void postIfConditionItem(IfConditionItem e) {
        ...
        myReplacingElementsMap.put(e, new IfConditionItem(e.getLocation().clone(), newConditionExp, collector.stmts));
    }

    /**
     * <p>
     * The user might not have supplied a {@code changing} clause, therefore we will need to generate one.
     * </p>
     *
     * @param e
     *            Current {@link LoopVerificationItem} we are visiting.
     */
    @Override
    public void postLoopVerificationItem(LoopVerificationItem e) {
        if (e.getChangingVars().isEmpty()) {
            ...
            AssertionClause newMaintainingClause = e.getMaintainingClause().clone();
            AssertionClause newDecreasingClause = e.getDecreasingClause().clone();

            myReplacingElementsMap.put(e, new LoopVerificationItem(e.getLocation().clone(), formChangingClause(),
                    newMaintainingClause, newDecreasingClause, newElapsedTimeClause));
        }
    }
    ...
    /**
     * <p>
     * This replaces the {@link ProgramVariableArrayExp} inside the calling arguments with appropriate swapping
     * operation from {@code Static_Array_Template}.
     * </p>
     *
     * @param e
     *            Current {@link ProgramFunctionExp} we are visiting.
     */
    @Override
    public void postProgramFunctionExp(ProgramFunctionExp e) {
        ...
        for (ProgramExp arg : args) {
            ...
            if (ArrayConversionUtilities.isProgArrayExp(arg)) {
                ...
                VarDec newArrayVarDec = ArrayConversionUtilities.buildTempArrayNameVarDec(arrayNameExp, arrayContentsTy,
                        ++myNewElementCounter);
                ...
                ProgramVariableNameExp newArrayVarDecAsProgramExp = new ProgramVariableNameExp(
                        newArrayVarDec.getLocation().clone(), null, newArrayVarDec.getName());
        ...
        myReplacingElementsMap.put(e,
                new ProgramFunctionExp(e.getLocation().clone(), qualifier, e.getName().clone(), newArgs));
    }
    ...
    /**
     * <p>
     * This method returns the current integer value for the new element counter.
     * </p>
     *
    ...
    /**
     * <p>
     * This method returns the new (and potentially modified) element that includes the syntactic sugar conversions.
     * </p>
     *
    ...
    public final ResolveConceptualElement getProcessedElement() {
        if (myFinalProcessedElement == null) {
            throw new MiscErrorException("The new element didn't get constructed appropriately!",
    ...
    /**
     * <p>
     * An helper method that retrieves the static array type for the array expression.
     * </p>
     *
     * @param exp
     ...
     *                Could not find the array's instantiating type.
     */
    private NameTy findArrayType(ProgramVariableExp exp) {
        ...
        if (expCopy instanceof ProgramVariableDotExp) {
            ...
            List<ProgramVariableExp> segments = ((ProgramVariableDotExp) expCopy).getSegments();
            ProgramVariableNameExp firstExp = (ProgramVariableNameExp) segments.remove(0);
            ...
            if (varDec == null) {
                throw new MiscErrorException("Cannot locate the content type for the array: " + expCopy.toString(),
                        new IllegalStateException());
            } else {
                ...
                for (ProgramVariableExp variableExp : segments) {
                    ProgramVariableNameExp variableNameExp = (ProgramVariableNameExp) variableExp;
                    recordTy = searchRecords(recordTy, variableNameExp.getName().getName());
                }
            ...
            String arrayVarName = ((ProgramVariableNameExp) expCopy).getName().getName();
    ...
    /**
     * <p>
     * An helper method that adds all the variables in scope as {@code changing}.
     * </p>
     *
    ...
    /**
     * <p>
     * An helper method to try to locate a {@link ParameterVarDec} based on the string name passed in.
     * </p>
     *
     * @param name
     *            Name of the variable we are trying to find.
     *
     * @return A copy of the {@link ParameterVarDec} if found, else {@code null}.
     *
     * @exception MiscErrorException
     *                Encountered two parameter variables with the same name.
     */
        ...
        for (ParameterVarDec v : myParentNodeElementsContainer.parameterVarDecs) {
            if (v.getName().getName().equals(name)) {
                if (parameterVarDec == null) {
                    ...
                    throw new MiscErrorException("Found two parameter variables for: " + name,
                            new IllegalStateException());
                }
            }
        }
    ...
    /**
     * <p>
     * An helper method to try to locate a {@link NameTy} in all of our {@link AbstractTypeRepresentationDec} based on
     * the {@link NameTy} and the name.
     * </p>
     *
     * @param rawTy
     *            The {@link NameTy} for a type representation.
     * @param name
     *            Name of the variable we are trying to find.
     *
     ...
     *
     * @exception MiscErrorException
     *                Encountered some kind of error while searching for the proper record type.
     */
        ...
        for (AbstractTypeRepresentationDec typeRepresentationDec : myCopyTRList) {
            if (typeRepresentationDec.getName().equals(rawTy.getName())) {
                ...
                if (representationTy instanceof RecordTy) {
                    List<VarDec> varDecs = ((RecordTy) representationTy).getFields();
                    for (VarDec v : varDecs) {
                        if (v.getName().getName().equals(name)) {
                            if (returnTy == null) {
                                returnTy = (NameTy) v.getTy().clone();
                            } else {
                                throw new MiscErrorException("Found two variables for: " + name,
                                        new IllegalStateException());
                            }
                        }
                    }
                    ...
                    throw new MiscErrorException(
                            "Variable [" + name + "] does not point to a type that contains a record",
                            new NullPointerException());
                }
            }
        }
    ...
    /**
     * <p>
     * An helper method to try to locate a {@link VarDec} in all of our {@link AbstractSharedStateRealizationDec} based
     * on the string name passed in.
     * </p>
     *
     * @param name
     *            Name of the variable we are trying to find.
     ...
     *
     * @exception MiscErrorException
     *                Found two variables with the same name.
     */
            ...
            for (VarDec v : stateVars) {
                if (v.getName().getName().equals(name)) {
                    if (varDec == null) {
                        ...
                        throw new MiscErrorException("Found two variables for: " + name, new IllegalStateException());
                    }
                }
            }
        }
    ...
    /**
     * <p>
     * An helper method to try to locate a {@link VarDec} based on the string name passed in.
     * </p>
     *
     * @param name
     *            Name of the variable we are trying to find.
    ...
    /**
     * <p>
     * This holds a copy of the {@link ParameterVarDec}, {@link VarDec} and {@link FacilityDec} for the incoming parent
     * node.
     * </p>
     */
    private class ParentNodeElementsContainer {
        ...
        /**
         * <p>
         * This constructs a temporary structure to store the elements from the incoming parent node we are walking.
         * </p>
         *
         * @param params
         ...
         * @param vars
         *            List of regular variables.
         */
        ParentNodeElementsContainer(List<ParameterVarDec> params, List<FacilityDec> facs, List<VarDec> vars) {
        ...
        /**
         * <p>
         * An helper method to create a new list of {@link FacilityDec}s that is a deep copy of the one passed in.
         * </p>
         *
         * @param facilityDecs
         *            The original list of {@link FacilityDec}s.
         *
        ...
        /**
         * <p>
         * An helper method to create a new list of {@link ParameterVarDec}s that is a deep copy of the one passed in.
         * </p>
         *
         * @param parameterVarDecs
         *            The original list of {@link ParameterVarDec}s.
         *
        ...
        /**
         * <p>
         * An helper method to create a new list of {@link VarDec}s that is a deep copy of the one passed in.
         * </p>
         *
         * @param varDecs
         *            The original list of {@link VarDec}s.
         *
    ...
    /**
     * <p>
     * As we walk though the various different nodes that can contain a list of {@link Statement}s, once we are done
     * with a particular {@link Statement}, it will be added to this class. Once we are done walking all the statements
     * and we are back to the {@link ResolveConceptualElement} that contains the list of {@link Statement}s, we will use
     * the elements stored in this class to create the new object.
     * </p>
     */
    private class ResolveConceptualElementCollector {
        ...
        /**
         * <p>
         * This constructs a temporary structure to store the list of {@link Statement}s.
         * </p>
         *
         * @param e
         *            The element that created this object.
         */
    ...
    /**
     * <p>
     * This holds new {@link Statement}s related to syntactic sugar conversions for {@link ProgramVariableArrayExp}.
     * </p>
     */
    private class NewStatementsContainer {
        ...
        /**
         * <p>
         * This constructs a temporary structure to store all the new statements that resulted from syntactic sugar
         * conversions for {@link ProgramVariableArrayExp}.
         * </p>
         */
