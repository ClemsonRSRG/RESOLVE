...
/**
 * <p>
 * This is a sanity checker for making sure the {@link OperationDec} is valid function operation declaration.
 * </p>
 *
 * @author Yu-Shan Sun
 *
...
public class ValidFunctionOpDeclChecker {
    ...
    /**
     * <p>
     * Creates a sanity checker for checking the various different criteria for being a valid function operation
     * declaration.
     * </p>
     *
     * @param functionOperation
     *            The associated function operation declaration.
     */
    ...
    /**
     * <p>
     * Checks to see if the provided {@link OperationDec} is a valid function operation declaration. A valid function
     * operation declaration must have an {@code ensures} clause of the form:
     * {@code <FuncOpDeclName> = <Expression/Value>}. It also shouldn't affect any global state variables.
     * </p>
     *
     * @throws SourceErrorException
     *             This is thrown if it is not a valid function operation declaration. The message will indicate why it
     *             is not valid.
     */
    public final void checkFunctionOpDecl() {
        ...
        if (myFunctionOperationDec.getAffectedVars() != null) {
            throw new SourceErrorException("Function operation: " + funcOpName + " cannot contain an affects clause.",
                    funcOpLoc);
        }
        ...
        for (ParameterVarDec varDec : myFunctionOperationDec.getParameters()) {
            ...
            if (varDecMode.equals(ParameterMode.ALTERS) || varDecMode.equals(ParameterMode.CLEARS)
                    || varDecMode.equals(ParameterMode.REPLACES) || varDecMode.equals(ParameterMode.UPDATES)) {
                throw new SourceErrorException("Function parameter: " + varDec.getName().getName()
                        + ", must have either PRESERVES, RESTORES or EVALUATES mode.", varDec.getLocation());
            }
        }
        ...
        if (!VarExp.isLiteralTrue(ensuresExp)) {
            ...
            if (ensuresExp instanceof EqualsExp && ((EqualsExp) ensuresExp).getOperator().equals(Operator.EQUAL)) {
                ...
                if (ensuresExpAsEqualsExp.containsVar(funcOpName, false)) {
                    ...
                    if (leftExp instanceof VarExp && ((VarExp) leftExp).getName().getName().equals(funcOpName)) {
                        // 7. Make sure that the function name isn't on the right hand side.
                        if (rightExp.containsVar(funcOpName, false)) {
                            throw new SourceErrorException("Function operation name: " + funcOpName
                                    + " can only appear on the left hand side of the ensures clause. "
                                    + "The ensures clause must be of the form: '" + funcOpName
                                    + " = <Expression/Value>'", funcOpLoc);
                        }
                    } else {
                        throw new SourceErrorException("Function operation name: " + funcOpName
                                + " must be the only thing on the left hand side of the equals expression. "
                                + "The ensures clause must be of the form: '" + funcOpName + " = <Expression/Value>'",
                                funcOpLoc);
                    }
                } else {
                    throw new SourceErrorException("Function operation name: " + funcOpName
                ...
                throw new SourceErrorException("Function operation: " + funcOpName
                        + " must have an ensures clause of the form: '" + funcOpName + " = <Expression/Value>'",
                        funcOpLoc);
            }
        } else {
            throw new SourceErrorException(
                    "Function operation: " + funcOpName + " cannot have 'ensures true;' as its specification.",
                    funcOpLoc);
        }
    }
