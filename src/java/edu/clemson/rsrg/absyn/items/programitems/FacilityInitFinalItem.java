...
/**
 * <p>
 * This is the class for all the facility type initialization/finalization block objects that the compiler builds using
...
public class FacilityInitFinalItem extends AbstractInitFinalItem {
    ...
    /**
     * <p>
     * This constructs a facility type initialization/finalization block that happens when a variable of this type is
     ...
     * @param requires
     *            A {@link AssertionClause} representing the operation's requires clause.
     * @param ensures
     *            A {@link AssertionClause} representing the operation's ensures clause.
     * @param facilities
     ...
     */
    public FacilityInitFinalItem(Location l, ItemType type, AffectsClause affects, AssertionClause requires,
            AssertionClause ensures, List<FacilityDec> facilities, List<VarDec> variables, List<Statement> statements) {
    ...
    public final String asString(int indentSize, int innerIndentInc) {
        ...
        // ensures clause
        sb.append(myEnsures.asString(indentSize + innerIndentInc, innerIndentInc));

        for (FacilityDec myFacilityDec : myFacilityDecs) {
            sb.append(myFacilityDec.asString(indentSize + innerIndentInc, innerIndentInc));
        }

        for (VarDec myVariableDec : myVariableDecs) {
            sb.append(myVariableDec.asString(indentSize + innerIndentInc, innerIndentInc));
        }

        for (Statement myStatement : myStatements) {
            sb.append(myStatement.asString(indentSize + innerIndentInc, innerIndentInc));
        }
    ...
    public final FacilityInitFinalItem clone() {
        ...
        return new FacilityInitFinalItem(cloneLocation(), myItemType, newAffects, myRequires.clone(), myEnsures.clone(),
