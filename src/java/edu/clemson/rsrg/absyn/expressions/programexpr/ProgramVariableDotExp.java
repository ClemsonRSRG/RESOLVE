...
/**
 * <p>
 * This is the class for all the programming dotted expression objects that the compiler builds using the ANTLR4 AST
 * nodes.
 * </p>
 *
 * @version 2.0
...
public class ProgramVariableDotExp extends ProgramVariableExp {
    ...
    /**
     * <p>
     * This constructs a programming variable dotted expression to keep track of all the inner expressions.
     * </p>
     *
     * @param l
     *            A {@link Location} representation object.
     * @param segments
     *            A list of {@link Exp} object.
     */
    ...
    public final boolean equivalent(Exp e) {
        ...
        if (retval) {
            ProgramVariableDotExp eAsProgramVariableDotExp = (ProgramVariableDotExp) e;

            retval = posSymbolEquivalent(getQualifier(), eAsProgramVariableDotExp.getQualifier());

            if (mySegmentExps != null && eAsProgramVariableDotExp.mySegmentExps != null) {
                Iterator<ProgramVariableExp> thisSegmentExps = mySegmentExps.iterator();
                Iterator<ProgramVariableExp> eSegmentExps = eAsProgramVariableDotExp.mySegmentExps.iterator();

                while (retval && thisSegmentExps.hasNext() && eSegmentExps.hasNext()) {
                    retval &= thisSegmentExps.next().equivalent(eSegmentExps.next());
                }

                // Both had better have run out at the same time
                retval &= (!thisSegmentExps.hasNext()) && (!eSegmentExps.hasNext());
            }
        }
    ...
    /**
     * <p>
     * This is a helper method that makes a copy of the list containing all the segment expressions.
     * </p>
     *
