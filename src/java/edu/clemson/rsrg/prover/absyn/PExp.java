 ...
 */
package edu.clemson.rsrg.prover.absyn;
...
import edu.clemson.rsrg.prover.absyn.expressions.PAlternatives;
import edu.clemson.rsrg.prover.absyn.expressions.PLambda;
...
import edu.clemson.rsrg.prover.absyn.visitors.NestedPExpVisitors;
import edu.clemson.rsrg.prover.absyn.visitors.PExpTextRenderingVisitor;
 ...
 * <em>only</em> mathematical expressions.
 * </p>
 *
...
public abstract class PExp {
    ...
    /**
     * <p>
     * An helper constructor that allow us to store the calculated hash values and mathematical type information for
     * objects created from a class that inherits from {@code PExp}.
     * </p>
     *
     * @param hashes
     *            An helper object that contains the structure hash and value hash.
     * @param type
     *            The expression's mathematical type.
     * @param typeValue
     *            The expression's mathematical type value.
     ...
     *            The expression's mathematical type value.
     */
    protected PExp(int structureHash, int valueHash, MTType type, MTType typeValue) {
        myMathType = type;
    ...
    /**
     * <p>
     * This method is the {@code accept()} method in a visitor pattern for invoking an instance of {@link PExpVisitor}.
     * </p>
     *
     * @param v
     *            A visitor for {@link PExp PExps}.
     */
    public abstract void accept(PExpVisitor v);

    /**
     * <p>
     * This method returns a mapping of expressions that that can be bound from the target.
     * </p>
     *
     * @param target
    ...
    /**
     * <p>
     * This method binds the expressions found in our mapping to the specified target.
     * </p>
     *
     * @param target
     *            The target expression.
     * @param accumulator
     *            A mapping of expressions to be bound.
     *
     * @throws BindingException
     *             Some error occurred during binding.
     */
    ...
    /**
     * <p>
     * This method converts an expression generated from the AST to the prover's immutable expression hierarchy.
     * </p>
     *
     * @param g
     *            The current type graph.
     * @param e
     *            An expression from the compiler AST.
     *
    ...
    public static PExp buildPExp(TypeGraph g, Exp e) {
        ...
        if (e instanceof FunctionExp) {
            ...
            if (g != null) {
                MTFunction fullType = new MTFunction(g, e.getMathType(), paramTypes);
                retval = new PSymbol(fullType, e.getMathTypeValue(),
                        fullName(eAsFunctionExp.getQualifier(), eAsFunctionExp.getName().getName().getName()),
                        arguments, convertExpQuantification(eAsFunctionExp.getQuantification()));
            } else {
            ...
            retval = new PSymbol(e.getMathType(), e.getMathTypeValue(), eAsPrefixExp.getOperatorAsPosSymbol().getName(),
                    arguments);
            ...
            retval = new PSymbol(e.getMathType(), e.getMathTypeValue(), eAsInfixExp.getOperatorAsPosSymbol().getName(),
            ...
            retval = new PSymbol(e.getMathType(), e.getMathTypeValue(),
                    ...
                    eAsOutfixExp.getOperator().getRightDelimiterString(), arguments, PSymbol.DisplayType.OUTFIX);
            ...
            retval = new PSymbol(e.getMathType(), e.getMathTypeValue(), eAsStringExp.getValue());
            ...
            retval = new PSymbol(e.getMathType(), e.getMathTypeValue(), symbol.toString(), arguments);
        } else if (e instanceof VarExp) {
            VarExp eAsVarExp = (VarExp) e;

            retval = new PSymbol(eAsVarExp.getMathType(), eAsVarExp.getMathTypeValue(),
                    fullName(eAsVarExp.getQualifier(), eAsVarExp.getName().getName()),
            ...
            retval = new PLambda(new ArrayBackedImmutableList<>(parameters), PExp.buildPExp(g, eAsLambdaExp.getBody()));
            ...
            throw new RuntimeException(
                    "Expressions of type " + e.getClass() + " are not accepted by the prover." + e.getLocation());
        }
    ...
    /**
     * <p>
     * This method checks to see if the current expression contains an existential quantifier.
     * </p>
     *
    ...
    /**
     * <p>
     * This method attempts to find an expression with the given name in our sub-expressions.
     * </p>
     *
     * @param name
     *            Expression name to be searched.
     *
     * @return {@code true} if this expression contains a sub-expression that matches the specified name, {@code false}
     *         otherwise.
     */
    ...
    /**
     * <p>
     * This method attempts to flip all quantifiers to generate a new expression.
     * </p>
     *
    ...
    /**
     * <p>
     * This method returns a list of sub-expressions that are function applications in the current expression.
     * </p>
     *
    ...
    /**
     * <p>
     * This method gets the mathematical type value associated with this expression.
     * </p>
     *
     * @return A {@link MTType} type object.
     */
    ...
    /**
     * <p>
     * This method returns a set of quantified variables for the current expression.
     * </p>
     *
    ...
    /**
     * <p>
     * This method returns an iterator for iterating over the list of sub-expressions.
     * </p>
     *
     * @return An iterator.
     */
    ...
    /**
     * <p>
     * This method checks to see if this expression is obviously equivalent to {@code true}.
     * </p>
     *
     * @return {@code true} if it is obviously equivalent to mathematical {@code true} expression, {@code false}
     *         otherwise.
     */
     ...
     * </p>
     *
     * @return {@code true} if it is a variable expression, {@code false} otherwise.
     */
    public abstract boolean isVariable();

    /**
     * <p>
     * This method processes the current expression using its string representation.
     * </p>
     *
     * @param visitor
     *            A visitor for {@link PExp PExps}.
     ...
     */
    public final void processStringRepresentation(PExpVisitor visitor, Appendable a) {
    ...
    /**
     * <p>
     * This method returns a DEEP COPY of this expression, with all instances of {@link PExp PExps} that occur as keys
     * in {@code substitutions} replaced with their corresponding values.
     * </p>
     *
     * @param substitutions
     *            A mapping from {@link PExp PExps} that should be substituted out to the {@link PExp PExps} that should
     *            replace them.
     *
     * @return A new {@link PExp} that is a deep copy of the original with the provided substitutions made.
     */
    ...
    /**
     * <p>
     * This method checks to see if the specified type matches the current expression's mathematical type.
     * </p>
     *
     * @param other
     *            Some other mathematical type.
     *
    ...
    /**
     * <p>
     * This method checks to see if the specified expression has a type that matches the current expression's
     * mathematical type.
     * </p>
     *
     * @param other
     *            Some other {@link PExp}.
     *
     * @return {@code true} if it matches, {@code false} otherwise.
     */
    ...
    /**
     * <p>
     * This method returns a new expression by substituting the specified value in all the sub-expressions.
     * </p>
     *
     * @param path
     *            A path for iterating over the sub-expressions.
     * @param newValue
     *            New expression value to be substituted.
     *
     ...
     *
     * @param index
     *            Index to an expression argument.
     * @param e
     *            The {@link PExp} to replace the one in our argument list.
     *
     * @return A new {@link PExp} with the expression at the specified index replaced with {@code e}.
     */
    public abstract PExp withSubExpressionReplaced(int index, PExp e);

    /**
     * <p>
     * This method provides a mapping of expressions that needs to be replaced at each index key.
     * </p>
     *
     * @param e
     *            A mapping from index to some expression argument.
     *
     * @return A new {@link PExp} with the expression replaced by all the key-value pairs in {@code e}.
     */
    ...
    /**
     * <p>
     * This method returns a new expression with the mathematical type value replaced.
     * </p>
     *
     * @param t
     *            A new mathematical type value.
     *
    ...
    /**
     * <p>
     * This method returns a list of sub-expressions that are function applications in the current expression without
     * using the cache.
     * </p>
     *
     * @return A list of sub-expressions that are function applications.
    ...
    /**
     * <p>
     * This method returns a set of quantified variables without using the cache.
     * </p>
     *
    ...
    /**
     * <p>
     * Takes an expression and distributes its quantifiers down to the variables, removing the quantifiers in the
     * process. (That is, in the expression <code>For all x, x = y</code>, the variable <code>x</code> in the equals
     ...
     *
     * @param e
     *            The expression for whom quantifiers should be distributed downward. This will largely be modified in
     *            place.
     * @param quantifiedVariables
     *            A map of variable names that should be considered quantified mapped to the quantifiers that apply to
     *            them.
     *
     * @return The root of the new expression.
     */
    private static Exp applyQuantification(Exp e, Map<String, SymbolTableEntry.Quantification> quantifiedVariables) {
        Exp retval;
        ...
        if (e instanceof QuantExp) {
            ...
            for (MathVarDec v : variableNames) {
                quantifiedVariables.put(v.getName().getName(), eAsQuantifier.getQuantification());
            }
            ...
            if (e instanceof VarExp) {
                ...
                for (Exp subExpression : subExpressions) {
                    ...
                    substitutions.put(curSubExpression, applyQuantification(curSubExpression, quantifiedVariables));
                }
    ...
    /**
     * <p>
     * An helper method for converting the quantification to the one provided by {@link PSymbol}.
     * </p>
     *
     * @param q
     *            A symbol table quantifier.
     *
     * @return A prover expression version of the quantifier.
     */
    private static PSymbol.Quantification convertExpQuantification(SymbolTableEntry.Quantification q) {
     ...
     *            A name.
     *
     * @return A string with the qualifier name (if not {@code null}) prepended before the specified name.
     */
    ...
    /**
     * <p>
     * An helper construct for storing the structure hash and value hash for a {@code PExp}.
     * </p>
     */
