...
import edu.clemson.rsrg.prover.absyn.iterators.PSymbolArgumentIterator;
...
/**
 * <p>
 * A {@code PSymbol} represents a reference to a named element such as a variable, constant, or function. More
 * specifically, all three are represented as function calls, with the former two represented as functions with no
 * arguments.
 * </p>
 *
...
public class PSymbol extends PExp {
    ...
    public enum DisplayType {
        ...
        /**
         * <p>
         * This method is called to before visiting a {@link PSymbol} using the specified {@link PExpVisitor}.
         * </p>
         *
         * @param v
         *            A prover expression visitor.
         * @param s
         *            A prover symbol expression.
         */
        ...
        /**
         * <p>
         * This method is called right before we end the visit for a {@link PSymbol} using the specified
         * {@link PExpVisitor}.
         * </p>
         * </p>
         *
        ...
        /**
         * <p>
         * This method is called to after visiting a {@link PSymbol} using the specified {@link PExpVisitor}.
         * </p>
         *
         * @param v
         *            A prover expression visitor.
    ...
    /**
     * <p>
     * This creates a prover representation of a reference to a named element such as a variable, constant, or function.
     * </p>
     *
     * @param type
     ...
     * @param leftPrint
     *            The expression's left hand side as a string.
     * @param rightPrint
     *            The expression's right hand side as a string.
     * @param arguments
     *            The expression's arguments.
     ...
     *            The expression's display type.
     */
    public PSymbol(MTType type, MTType typeValue, String leftPrint, String rightPrint, Collection<PExp> arguments,
            Quantification quantification, DisplayType display) {
        this(type, typeValue, leftPrint, rightPrint, new ArrayBackedImmutableList<>(arguments), quantification,
                display);
    }
     ...
     *            The expression's display type.
     */
    public PSymbol(MTType type, MTType typeValue, String leftPrint, String rightPrint, ImmutableList<PExp> arguments,
            Quantification quantification, DisplayType display) {
        ...
        if (quantification == Quantification.FOR_ALL) {
            ...
            if (myArgumentsSize == 0) {
                ...
                accumulator.put(new PSymbol(myMathType, myMathTypeValue, name),
                        new PSymbol(sTarget.getPreApplicationType(), null, sTarget.name));
                ...
                while (thisArgumentsIter.hasNext()) {
                    thisArgumentsIter.next().substitute(accumulator).bindTo(targetArgumentsIter.next(), accumulator);
                }
            }
        } else {
            ...
            if (!(myMathType.isSubtypeOf(target.getMathType()) || target.getMathType().isSubtypeOf(myMathType))) {
    ...
    public final PExp flipQuantifiers() {
        ...
        if (argumentChanged) {
            retval = new PSymbol(myMathType, myMathTypeValue, leftPrint, rightPrint, Arrays.asList(myScratchSpace),
            ...
            if (flipped == quantification) {
                ...
                retval = new PSymbol(myMathType, myMathTypeValue, leftPrint, rightPrint, arguments, flipped,
                        displayType);
            }
        }
    ...
    public final Set<String> getNonQuantifiedSymbols() {
        ...
        while (argumentIter.hasNext()) {
            Set<String> r = ((PSymbol) argumentIter.next()).getNonQuantifiedSymbols();
            result.addAll(r);
        }
    ...
    /**
     * <p>
     * This method checks to see if this expression represents some kind of function.
     * </p>
     *
     * @return {@code true} if it is some kind of function expression, {@code false} otherwise.
     */
    ...
    /**
     * <p>
     * This creates a new {@link PSymbol} by replacing the specified index with the new argument.
     * </p>
     *
     * @param index
    ...
    public final PSymbol setArgument(int index, PExp newArgument) {
        ...
        return new PSymbol(myMathType, myMathTypeValue, leftPrint, rightPrint, newArguments, quantification,
                displayType);
    }
    ...
    public final PSymbol setName(String newName) {
        return new PSymbol(myMathType, myMathTypeValue, newName, rightPrint, arguments, quantification, displayType);
    }
    ...
    public final PExp substitute(Map<PExp, PExp> substitutions) {
        ...
        if (retval == null) {
            ...
            if (arguments.size() > 0 && displayType.equals(DisplayType.PREFIX)) {
                PExp asVar = new PSymbol(getMathType(), getMathTypeValue(), leftPrint, quantification);
            ...
            if (argumentChanged) {
                retval = new PSymbol(myMathType, myMathTypeValue, newLeft, newRight,
                        new ArrayBackedImmutableList<>(myScratchSpace), newQuantification, displayType);
            } else {
                // changed this to handle case where func name changes but args don't -- mike
                retval = new PSymbol(myMathType, myMathTypeValue, newLeft, newRight, arguments, newQuantification,
                        displayType);
            }
        }
    ...
    public final PSymbol withTypeReplaced(MTType t) {
        return new PSymbol(t, myMathTypeValue, leftPrint, rightPrint, arguments, quantification, displayType);
    }
    ...
    public final PSymbol withTypeValueReplaced(MTType t) {
        return new PSymbol(myMathType, t, leftPrint, rightPrint, arguments, quantification, displayType);
    }
    ...
    /**
     * <p>
     * An helper method that computes a structure and value hash given the specified arguments.
     * </p>
     *
     * @param left
     *            The expression's left hand side as a string.
    ...
    /**
     * <p>
     * An helper method that gets the pre-application mathematical type associated with this expression.
     * </p>
     *
    ...
    private MTType getPreApplicationType() {
        if (myPreApplicationType == null) {
            ...
            myPreApplicationType = new MTFunction(getMathType().getTypeGraph(), getMathType(), argTypes);
        }
