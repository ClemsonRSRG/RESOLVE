 ...
 */
package edu.clemson.rsrg.prover.utilities.expressions;
...
import edu.clemson.rsrg.prover.exception.NotPSymbolException;
...
/**
 * <p>
 * This class represents a conjunction of normalized atomic expression.
 * </p>
 *
...
public class ConjunctionOfNormalizedAtomicExpressions {
    ...
    /**
     * <p>
     * A mapping of normalized atomic expressions.
     * </p>
     */
    ...
    /**
     * <p>
     * A flag that indicates if the current expression evaluates to {@code false}.
     * </p>
     */
    ...
    /**
     * <p>
     * A timer that indicates how long to search for a proof.
     * </p>
     */
    ...
    /**
     * <p>
     * A mapping of integer values used to represent a set of {@link NormalizedAtomicExpression}.
     * </p>
     */
    ...
    /**
     * <p>
     * A reference to the immutable {@code VC} we are currently processing.
     * </p>
     */
    ...
    /**
     * <p>
     * This constructs a conjunction of normalized atomic expressions.
     * </p>
     *
     * @param vc
     *            An immutable copy of a {@code VC}.
     * @param registry
     *            A registry of symbols encountered so far.
     */
    ...
    /**
     * <p>
     * This method adds an expression to this conjunction as a new {@link NormalizedAtomicExpression}.
     * </p>
     *
     * @param expression
     *            A prover expression.
    ...
    public final String addExpression(PExp expression) {
        if (myEvaluatesToFalseFlag || (myTimeToEnd > 0 && System.currentTimeMillis() > myTimeToEnd)) {
            return "";
        }
    ...
    /**
     * <p>
     * This method adds an expression and begins to track changes.
     * </p>
     *
     * @param expression
     *            A prover expression.
     * @param timeToEnd
     *            A value that indicates when we will stop proving this expression.
     *
     * @return A string representation of the expression added.
     */
    public final String addExpressionAndTrackChanges(PExp expression, long timeToEnd) {
    ...
    /**
     * <p>
     * This method adds a new formula expression.
     * </p>
     *
     * @param formula
     *            A prover expression representing a formula.
     *
     * @return A string representation of the formula added.
     */
    public final int addFormula(PExp formula) {
        /*
         * experimentally handling = i.e.: (|?S| = 0) = (?S = Empty_String)) is broken down by addExpression so (|?S| =
         * 0) is an argument should return int for true if known to be equal, otherwise return root representative.
         */
        if (formula.getTopLevelOperation().equals("=B")) {
            ...
            /*
             * if (lhs == rhs) { return m_registry.getIndexForSymbol("true"); } else {
             */
            // insert =(lhs,rhs) = someNewRoot

            int questEq = myRegistry.getIndexForSymbol("=B");
            NormalizedAtomicExpression pred = new NormalizedAtomicExpression(myRegistry,
                    new int[] { questEq, lhs, rhs });
        ...
        NormalizedAtomicExpression newExpr = new NormalizedAtomicExpression(myRegistry, ne);
        if (myEvaluatesToFalseFlag) {
            return -1;
        }
    ...
    /**
     * <p>
     * This method clears all stored expressions.
     * </p>
     */
    ...
    /**
     * <p>
     * This method returns whether or not this {@code VC} evaluates to {@code false}.
     * </p>
     *
     * @return {@code true} if the prover has determined that this {@code VC} evaluates to {@code false}, {@code false}
     *         otherwise.
     */
    ...
    /**
     * <p>
     * This method attempts to locate the specified expression in this conjunction.
     * </p>
     *
     * @param exp
     *            A prover expression.
    ...
    public final PExp find(PExp exp) {
        if (exp.getSubExpressions().size() == 0) {
            String s = myRegistry.getRootSymbolForSymbol(exp.getTopLevelOperation());
            if (s.equals("")) {
                return exp;
            } else {
        ...
        while (it.hasNext()) {
            ...
            if (fcur.getSubExpressions().size() > 0
                    || !myRegistry.mySymbolToIndex.containsKey(fcur.getTopLevelOperation())) {
                irreducible = true;
            }
        ...
        String op = myRegistry.getRootSymbolForSymbol(exp.getTopLevelOperation());
        if (!irreducible && !op.equals("")) {
            int[] ia;
            ia = new int[args.size() + 1];
            ia[0] = myRegistry.getIndexForSymbol(op);
            for (int i = 1; i < ia.length; ++i) {
                ia[i] = myRegistry.getIndexForSymbol(args.get(i - 1).getTopLevelOperation());
            }

            NormalizedAtomicExpression na = new NormalizedAtomicExpression(myRegistry, ia);
            if (myExpressionSet.containsKey(na) && myExpressionSet.get(na).readRoot() >= 0) {
                ...
                return new PSymbol(exp.getMathType(), exp.getMathTypeValue(), exp.getTopLevelOperation(), args);
            }
        } else {
            return new PSymbol(exp.getMathType(), exp.getMathTypeValue(), exp.getTopLevelOperation(), args);
        }
    ...
    /**
     * <p>
     * This method returns matches from an expression using the override set.
     * </p>
     *
     * @param expr
     *            A {@link NormalizedAtomicExpression}.
     * @param foreignSymbolOverrideSet
     *            A set containing symbols to override.
     *
    ...
    /**
     * <p>
     * This method returns the set of normalized atomic expressions used as keys in this conjunction.
     * </p>
     *
    ...
    /**
     * <p>
     * This method returns the set of expressions based on the symbol number.
     * </p>
     *
     * @param symk
     *            A symbol number.
    ...
    public final Set<NormalizedAtomicExpression> getUses(int symk) {
        ...
        Map<Integer, Set<NormalizedAtomicExpression>> usesByPos = myUseMap.get(symk);
    ...
    /**
     * <p>
     * This method returns the current number of expressions in this conjunction.
     * </p>
     *
    ...
    /**
     * <p>
     * An helper method for adding an atomic formula.
     * </p>
     *
     * @param atomicFormula
     *            One sided expression. (= new root) is appended and expression is inserted if no match of the side is
     *            found. Otherwise current root is returned.
     *
    ...
    private int addAtomicFormula(NormalizedAtomicExpression atomicFormula) {
        ...
        MTType typeOfFormula = myRegistry.getTypeByIndex(atomicFormula.readPosition(0));
        // this is the full type and is necessarily a function type

        MTType rangeType = ((MTFunction) typeOfFormula).getRange();
        String symName = myRegistry.getSymbolForIndex(atomicFormula.readPosition(0));
    ...
    /**
     * <p>
     * An helper method for adding an expression to the conjunction set.
     * </p>
     *
     * @param nae
     *            A {@link NormalizedAtomicExpression}.
    ...
    /**
     * <p>
     * An helper method for associating a symbol number with an expression.
     * </p>
     *
     * @param symk
     *            A symbol number.
    ...
    private void addMapUse(int symk, NormalizedAtomicExpression nae) {
        if (!myUseMap.containsKey(symk)) {
            myUseMap.put(symk, new HashMap<Integer, Set<NormalizedAtomicExpression>>());
        }

        Map<Integer, Set<NormalizedAtomicExpression>> posMapRef = myUseMap.get(symk);
    ...
    /**
     * <p>
     * An helper method for applying the prover's built-in logic.
     * </p>
     *
     * @param nm
     *            A {@link NormalizedAtomicExpression}.
     * @param tank
     *            A stack of built-in operators.
     */
    private void applyBuiltInLogic(NormalizedAtomicExpression nm, Stack<Integer> tank) {
        ...
        if ((arg1 == tr || arg1 == fl) && (arg2 == tr || arg2 == fl) && (rhs == tr || rhs == fl)) {
            return;
        }

        // guard: return if all var and op is not equals or or
        if (!(op.equals("=B") || op.equals("orB")) && arg1 != tr && arg1 != fl && (arg2 != tr && arg2 != fl)
                && (rhs != tr && rhs != fl)) {
            return;
        }
    ...
    /**
     * <p>
     * An helper method for adding a particular symbol to the registry.
     * </p>
     *
     * @param ps
     *            A prover symbol.
    ...
    private int addPSymbol(PSymbol ps) {
        ...
        } else if (ps.isFunction() || ps.getMathType().getClass().getSimpleName().equals("MTFunction")) {
     ...
     *
     * @param a
     *            An integer value representing some symbol.
     * @param b
     *            Another integer value representing some symbol.
     *
    ...
    /**
     * <p>
     * An helper method for computing a set of bindings.
     * </p>
     *
     * @param filteredSet
     *            A set of filtered {@link NormalizedAtomicExpression}.
     * @param baseMap
     *            A map containing base symbols.
     * @param unmappedWildcards
     *            An array containing unmapped wild card symbols.
     * @param searchReg
    ...
    private Set<Map<String, String>> computeBindings(Set<NormalizedAtomicExpression> filteredSet,
        ...
        next: for (NormalizedAtomicExpression e : filteredSet) {
            ...
            for (int i = 0; i < unmappedWildcards.length; ++i) {
                ...
                String ac = (i < unmappedWildcards.length - 1 ? e.readSymbol(i)
                ...
                MTType wildType = searchReg.getTypeByIndex(searchReg.getIndexForSymbol(wc));
                MTType localType = myRegistry.getTypeByIndex(myRegistry.getIndexForSymbol(ac));
    ...
    /**
     * <p>
     * An helper method for obtaining the bindings for an expression.
     * </p>
     *
     * @param expr
     *            A {@link NormalizedAtomicExpression}.
     * @param foreignSymbolOverride
     *            A mapping of symbols to override.
     *
     * @return A set containing expression binding maps.
     */
    private Set<Map<String, String>> getBindingsForSearchExpr(NormalizedAtomicExpression expr,
            Map<String, String> foreignSymbolOverride) {
        int[] searchKeys = expr.rootedLiterals(foreignSymbolOverride, myRegistry);
        if (searchKeys == null) {
            return null;
        }
        ...
        if (isCommutOp && searchKeys.length == 4 && (searchKeys[1] != searchKeys[2])) {
            if (!(searchKeys[1] == -1 || searchKeys[2] == -1)) {
                ...
                if (searchKeys[1] > searchKeys[2]) {
                    ...
                    return computeBindings(getExprsMatchingAtPosition(searchKeys), foreignSymbolOverride,
                            unMappedWildCards, expr.getRegistry());
                }
            } else {
                // only one blank. 2 searches
                rSet = computeBindings(getExprsMatchingAtPosition(searchKeys), foreignSymbolOverride, unMappedWildCards,
                        expr.getRegistry());
                int t = searchKeys[1];
                ...
                rSet.addAll(computeBindings(getExprsMatchingAtPosition(searchKeys), foreignSymbolOverride,
                        unMappedWildCards, expr.getRegistry()));
        ...
        return computeBindings(getExprsMatchingAtPosition(searchKeys), foreignSymbolOverride, unMappedWildCards,
                expr.getRegistry());
    }

    /**
     * <p>
     * An helper method for searching for expressions that match the specified positions.
     * </p>
     *
     * @param searchKey
     *            An array of search keys represented using integer values.
     *
     * @return A set of {@link NormalizedAtomicExpression} matching the search key.
     */
    private Set<NormalizedAtomicExpression> getExprsMatchingAtPosition(int[] searchKey) {
        ...
        for (int p = 0; p < searchKey.length; ++p) {
            ...
            if (!myUseMap.containsKey(k)) {
                return rSet; // k is not used in the conjunction, but is still in Registry. Can happen with
                             // =, not, etc.
            }
    ...
    /**
     * <p>
     * An helper method for getting the set of expressions located at the current position./p>
     *
     * @param symk
     *            A symbol number.
     * @param pos
     *            A position number.
     *
     * @return A set of {@link NormalizedAtomicExpression} referenced by {@code symk} and located at {@code pos}.
     */
        ...
        nextUse: for (NormalizedAtomicExpression nm : bUses) {
            ...
            if (nm == ne) {
                ...
                if (myExpressionSet.containsKey(ne)) {
                    ...
                    if (oldRoot != neroot && !((oldRoot == a || oldRoot == b) && (neroot == a || neroot == b))) {
    ...
    /**
     * <p>
     * An helper method for merging operators.
     * </p>
     *
     * @param a
     *            An integer value representing some operator.
     * @param b
     *            Another integer value representing some operator.
     *
    ...
    private String mergeOperators(int a, int b) {
        ...
        while (!holdingTank.empty()) {
            ...
            rString.append(myRegistry.getSymbolForIndex(opA)).append("/").append(myRegistry.getSymbolForIndex(opB))
                    .append(",");
    ...
    /**
     * <p>
     * An helper method that removes an expression from this conjunction.
     * </p>
     *
     * @param nae
     *            A {@link NormalizedAtomicExpression}.
    ...
    /**
     * <p>
     * An helper method for removing an expression from our use map.
     * </p>
     *
     * @param symK
     *            A symbol number.
