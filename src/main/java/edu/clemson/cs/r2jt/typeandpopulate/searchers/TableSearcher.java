...
/**
 * <p>
 * A <code>TableSearcher</code> is a strategy for searching a {@link SymbolTable
 * SymbolTable},
 * adding any {@link SymbolTableEntry SymbolTableEntry}s that match the search
 * to an accumulator.
 * </p>
 *
 * @param <E> Permits concrete implementations of this interface to refine the
 *        type of
 *        <code>SymbolTableEntry</code> they will match. This searcher
 *        guarantees that any entry it
 *        matches will descend from <code>E</code>. Put another way: no matched
 *        entry will not be a
 *        subtype of <code>E</code>.
 */
public interface TableSearcher<E extends SymbolTableEntry> {
     ...
     *         list of search
     *         results&mdash;i.e., no further symbol table entries should be
     *         considered.
