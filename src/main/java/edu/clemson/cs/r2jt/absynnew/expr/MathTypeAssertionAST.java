...
/**
 * A {@code TypeAssertionAST}, generally, allows a sub-expression to be
 ...
 *          consequent of a top-level implies expression.</li>
 * </ul>
 *
 * In the first case, it defines a slot to be bound implicitly by some
 * component of the type of the actual parameter.  In this case, the expression
 * must be a {@link MathSymbolAST}.  Some examples:
 *
 * {@code Definition Foo(S : Str(T : Powerset(Z)) : T; }
 * <p>
 * Here, the {@code T : Powerset(Z)} part is an implicit type parameter.
 * If one were to pass a {@code Str(N)} to {@code Foo}, then the
 * return type would be {@code T}.  If one were to pass {@code Str(B)}
 * to {@code Foo} that would be a typechecking error, since {@code B}
 * is not in {@code Powerset(Z)}.
 * </p>
 * {@code Definition Bar(L : Set(T : MType), E : Powerset(T)) : T;}
 * <p>
 * Here, {@code T : MType} is an implicit type parameter, and the type of both
 * the second parameter and the return value are dependent on the binding of
 * that parameter.  Implicit parameters are always bound left to right, so the
