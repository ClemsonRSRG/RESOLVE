...
public class Utilities {
	...
	public List<VarDec> getGlobalVarList() { return myGlobalVarList; }
	
	/** Returns the list containing the local variables. */
	public List<VarDec> getLocalVarList() { return myLocalVarList; }
	
	/** Returns the list containing the records variables in a Facility. */
	public List<FacilityTypeDec> getFacilityTypeList() { return myFacilityTypeList; }
	
	/** Returns the list containing the records variables in a Concept/Enhancement Realization. */
	public List<RepresentationDec> getRepresentationDecList() { return myRepresentationDecList; }
	
	/** Returns the list containing the parameter variables of the current operation. */
	public List<VarDec> getParameterVarList() { return myParameterVarList; }
	
	/** Returns the list containing uses items. */
	public List<UsesItem> getUsesList() { return myUsesList; }
	...
	public void setGlobalVarList(List<VarDec> list) { this.myGlobalVarList = list; }
	
	/** Sets the list containing the local variables. */
	public void setLocalVarList(List<VarDec> list) { this.myLocalVarList = list; }
	
	/** Sets the list containing the records variables in a Facility. */
	public void setFacilityTypeList(List<FacilityTypeDec> list) { this.myFacilityTypeList = list; }
	
	/** Sets the list containing the records variables in a Concept/Enhancement Realization. */
	public void setRepresentationDecList(List<RepresentationDec> list) { this.myRepresentationDecList = list; }
	
	/** Sets the list containing the parameter variables of the current operation. */
	public void setParameterVarList(List<VarDec> list) { this.myParameterVarList = list; }
	
	/** Sets the list containing uses items. */
	public void setUsesList(List<UsesItem> list) { this.myUsesList = list; }
     ...
     */    
    public FacilityDec createFacilityDec(
    		Location location, 
    		String name, 
    		String conceptName,
    		String conceptRealizationName,    		
    ...
    public void initLocalProcMap(List<Dec> decList) {    	   
    	...
    	while (it.hasNext()) {
    		...
    		if (current instanceof ProcedureDec) {
    			...
    			List<ParameterVarDec> parameterList = currentProcDec.getParameters();
	...
	public void addToUsesList(String filename, Location location) {
		...
		if (!isThere) {
			myUsesList.add(new UsesItem(new PosSymbol(location, Symbol.symbol(filename))));
		}
	}
     ...
     * 
     */
	public VariableNameExp createVariableNameExp(VariableNameExp oldExp, int counter) {
		/* Variable */
		VariableNameExp newExp = new VariableNameExp();
		
		/* Create the new name */
		PosSymbol oldName = newExp.getName();
		PosSymbol newName = new PosSymbol(newExp.getLocation(), 
				Symbol.symbol("_RepArg_" + oldName.getName() + "_" + counter));
		...
		if (myRepresentationDecList != null) {
			...
			while (it.hasNext()) {
				...
				if (tempRepDec.getName().getName().equals(ty.getName().getName())) {
					...
					/* Get list of fields */
					VarDec varInRecord = iterateFindVarDec(((VariableArrayExp) exp).getName(), tempTy.getFields());					
					if (varInRecord != null) {
						/* Set the return ty */
    ...
    public void loopAndAddSwapCalls(Iterator<ResolveConceptualElement> it, Location location) {
    	...
    	while (it.hasNext()) {
    		...
    		if (temp instanceof FacilityOperationDec) {
    			...
    			List<Statement> stmtList = addSwapCalls(location, ((FacilityOperationDec) temp).getStatements());
     ...
     * 
     */
    public void loopAndAddSwapStmts(Iterator<ResolveConceptualElement> it, Location location) {
    	/* Iterate through our ancestors */
    	while (it.hasNext()) {
    		/* Obtain a temp from it */
    		ResolveConceptualElement temp = it.next();
    		...
    		if (temp instanceof FacilityOperationDec) {
    			/* Update the list of statements inside the FacilityOperation */
    			List<Statement> stmtList = addSwapStmts(location, ((FacilityOperationDec) temp).getStatements());
    			((FacilityOperationDec) temp).setStatements(stmtList);
    			break;
    		} 
    		/* Look for ProcedureDec */
    			...
    			/* Update the list of statements inside the Procedure */
    			List<Statement> stmtList = addSwapStmts(location, ((ProcedureDec) temp).getStatements());
    			((ProcedureDec) temp).setStatements(stmtList);
    			break;
    		}
    	}
    ...
    public ProgramParamExp createReplicaCall(ProgramExp oldExp) {
		...
		newExp = new ProgramParamExp(oldExp.getLocation(), 
     ...
     * 
     */
	private List<Statement> addSwapStmts(Location location, List<Statement> stmtList) {
    	/* Loop through the list */
		for (int i = 0; i < stmtList.size(); i++) {
			/* CallStmts */
			if (stmtList.get(i) instanceof CallStmt && 
