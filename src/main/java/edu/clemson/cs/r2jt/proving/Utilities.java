...
public class Utilities {
	...
	public static List<Exp> applySubstitutionToAssumptions(
		...
		BindReplace replacement = new BindReplace(substitution.getLeft(),
		...
		replacement = new BindReplace(substitution.getRight(),
	...
	public static List<Exp> applyImplicationToAssumptions(List<Exp> assumptions,
			Exp antecedent, Exp consequent, MathExpTypeResolver typer,
			long timeoutAt) throws TimeoutException {
		List<Exp> retval;
		
		Conjuncts antecedents = new Conjuncts(antecedent);
		
		retval = applyImplicationToAssumptions(assumptions, antecedents, 
	...
	public static void satisfy(Iterable<Exp> assumptions, List<Exp> antecedents,
			...
			MathExpTypeResolver typer, List<Exp> consequent, 
			...
			while (assumptionsIter.hasNext()) {
				...
				if (subBinding != null) {
					...
					satisfy(assumptions, antecedents, 
							curAntecedentIndex + 1, workingBindings, typer, 
							consequent, accumulator, timeoutAt);
				}
			}
		}
	}
	...
	public static Map<PosSymbol, Exp> bind(Exp pattern, Exp e, 
		...
		if (pattern instanceof VarExp) {
			...
			if (patternVar.getQuantification() == QuantExp.FORALL) {
				try {
					if (pattern.getType() != null && e.getType() != null &&
							(typer.matchTypes(null, pattern.getType(), 
							e.getType(), true, false))) {
					...
					if (retval == null && myBindDebugFlag) {
						...
						else if (!typer.matchTypes(null, pattern.getType(),
								e.getType(), true, false)) {
	...
	private static Map<PosSymbol, Exp> bindSubExpressions(Exp pattern,
			Exp e, MathExpTypeResolver typer) {
	 ...
	 * @return
	 */
	private static Map<Exp, Exp> newBindSubExpressions(Exp pattern,
			Exp e, MathExpTypeResolver typer) {
