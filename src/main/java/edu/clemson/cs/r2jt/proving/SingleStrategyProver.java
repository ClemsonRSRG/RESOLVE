...
class SingleStrategyProver implements VCProver {
	...
	public SingleStrategyProver(RuleProvider ruleChooser, 
		...
		myProofCountOrder = BigInteger.valueOf(
				ruleChooser.getApproximateRuleSetSize());
	...
	private void propagateTheorems(final VerificationCondition vC, 
				Metrics m, long timeoutAt) 
			throws UnableToProveException, TimeoutException {
		...
		for (int j = 0; j < 5; j++) {
			for (Implication i : IMPLICATIONS) {
				...
				vC.setAntecedents(new Conjuncts(
						Utilities.applyImplicationToAssumptions(
						vC.getAntecedents(), i.getAntecedent(), 
						i.getConsequent(), TYPER, timeoutAt)));
			}
	...
	private static boolean isRepeatState(final VerificationCondition vC,
		...
		Iterator<VerificationCondition> pastStatesIter =
			pastStates.iterator();
	...
	private void applyReplaceStep(final MatchReplace matcher,
		...
		MatchApplicator replacement = new MatchApplicator(
				vC.getConsequents(), matcher);
		...
		while (newConsequents != null) {			
			...
			try {
				VerificationCondition newVC = new VerificationCondition(
						vC.getAntecedents(), newConsequents);
	 ...
	 *                 top level.
	 */
	private void incrementProgress(final int curLength, 
			final Metrics metrics) {
		if (curLength == 0) {
			metrics.rulesTried++;
			
			if (metrics.progressListener != null) {
				double progress = ((double) metrics.rulesTried) / 
						metrics.ruleCount;
