...
/**
 * <p>
 * The <code>TreeWalker</code> is used to apply the visitor pattern to the
 * RESOLVE abstract syntax
 * tree. The visitor logic is implemented as a <code>TreeWalkerVisitor</code>.
 */
public class TreeWalker {
     ...
     * 
     * @param twv An instance of TreeWalkerVisitor which implements visitor
    ...
    /**
     * Visits the node <code>e</code> by calling pre- visitor methods,
     * recursively visiting child
     * nodes, and calling post- visitor methods. If the
     * <code>TreeWalkerVisitor</code> contains a
     * method named <code>walk[className]</code>, that method is called instead
     * of visiting the
     * children.
     * 
        ...
        while (iter.hasNext()) {
            ...
            try {
                ...
                if (pre || post) { // pre and post methods
                    visitorMethod = this.myVisitor.getClass()
                            .getMethod(methodName, currentClass);
                }
                else { // mid methods
                    visitorMethod = this.myVisitor.getClass().getMethod(
                            methodName, currentClass, paramType, paramType);
                }
    ...
    private boolean walkOverride(ResolveConceptualElement e) {
        ...
        while (iter.hasNext() && !foundOverride) {
            ...
            if (!c.equals(VirtualListNode.class)) {
                ...
                try {
                    Method walkMethod = this.myVisitor.getClass()
                            .getMethod(walkMethodName, c);
                    foundOverride =
