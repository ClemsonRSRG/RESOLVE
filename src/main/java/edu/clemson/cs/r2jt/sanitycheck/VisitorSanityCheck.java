...
public class VisitorSanityCheck extends TreeWalkerStackVisitor {
	...
	List<SymbolTable> mySTList = null;
	...
	private SymbolTable myAssociatedConceptSymbolTable;
...
//				 if(dec.getRecursive() != vrc.isRecursive()) {
    ...
    public void postFacilityDec(FacilityDec dec){
    	...
    	while(it.hasNext()){
    		...
    		if(itemProfileName != null){
    			...
    			EnhancementBodyModuleDec bodyDec = (EnhancementBodyModuleDec)myCompileEnvironment.getModuleDec(realizID);
	...
	public void preConceptBodyModuleDec(ConceptBodyModuleDec dec) {
		...
		myAssociatedConceptSymbolTable = myCompileEnvironment
				.getSymbolTable(id);
	...
	public void preEnhancementBodyModuleDec(EnhancementBodyModuleDec dec) {
		ModuleID id = ModuleID.createEnhancementID(dec.getEnhancementName(),
				dec.getConceptName());
		myAssociatedConceptSymbolTable = myCompileEnvironment
				.getSymbolTable(id);
	...
	public void preProcedureDec(ProcedureDec dec) {
		...
		if(associatedOperationDec != null && associatedOperationDec.getEnsures() != null){
			...
			if(dec.getRecursive() && dec.getDecreasing() == null) {
				err.error(dec.getName().getLocation(), "Cannot leave out the Decreasing clause of a recursive procedure when specified with an Ensures clause.");
			}	
		}
		...
		try {
			...
			if (operation != null) {
				...
				List<Mode> operationParameterModes = getParameterVarEntryModes(operation
						.getParameters());

				// Get the mode for each procedure parameter
				List<Mode> procedureParameterModes = getParameterVarDecModes(dec
						.getParameters());
				...
				if (correctNumberOfParameters) {
					...
					String iName = myAssociatedConceptSymbolTable.getModuleID()
							.getName().toString();
					err
							.error(
...
//			stmt.setChanging(myCurProcedureDec.getAllVariables());
	...
	public void preCallStmt(CallStmt stmt) {
		...
		//sanityCheckPrimaryOperation(stmt.getName().getSymbol(), stmt.getName()
				//.getLocation());
		sanityCheckParamModeChanging(stmt.getName().getSymbol(),
										stmt.getArguments());
	...
	private void sanityCheckPrimaryOperation(Symbol sym, Location loc) {
		...
		if (myCurrentConceptBodyModuleDec != null) {
			List<Symbol> operationList = myAssociatedConceptSymbolTable
					.getModuleScope().getLocalOperationNames();
	...
	private void sanityCheckImplementedProcedures(String name,
		...
		List<Symbol> operationList = st.getModuleScope()
	...
	private void sanityCheckProcedureParameterModes(
		...
		for (int curArgumentIndex = 0; curArgumentIndex < operationParameterModes
			...
			try {
				...
				String iName = myAssociatedConceptSymbolTable.getModuleID()
						.getName().toString();
				Location l1 = procedure.getParameters().get(curArgumentIndex)
						.getName().getLocation();
	...
	private void sanityCheckChanging(VariableExp varExp) {
		List<VariableExp> changingExpressions = myCurWhileStatement
				.getChanging();
		...
		if(changingExpressions != null){
			Iterator<VariableExp> changingExpressionsIterator = changingExpressions
			.iterator();
			while (changingExpressionsIterator.hasNext() && !changeOk) {
	
				try {
					curChangingExpression = (VariableNameExp) changingExpressionsIterator
							.next();
	...
	private OperationEntry getConceptOperation(Symbol name)
		...
		ModuleScope conceptModuleScope = myAssociatedConceptSymbolTable
				.getModuleScope();
	...
	private String foundMissingProceduresMessage(String name, String iName,
		...
		while (it.hasNext()) {
			entry = it.next();
			msg += err.printErrorLine(entry.getLocation().getFile(), entry
					.getLocation().getPos());
		}
		return msg;
	}
