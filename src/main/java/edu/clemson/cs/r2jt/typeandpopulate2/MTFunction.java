...
public class MTFunction extends MTAbstract<MTFunction> {
     ...
     */
    public MTFunction deschematize(List<ExprAST> parameters)
    ...
    private void deschematizeParameter(String formalParameterName,
            MTType formalParameterType, ExprAST actualParameter,
     ...
     * @return
     */
    public boolean parametersMatch(List<ExprAST> parameters,
            TypeComparison<ExprAST, MTType> comparison) {
        ...
        if (myDomain == myTypeGraph.VOID) {
            ...
            if (!result && (parameters.size() == 1)) {
                ExprAST parameter = parameters.get(0);
                result =
    ...
    private static List<String> getParamNames(List<MathVariableAST> params) {
        List<String> names = new LinkedList<String>();

        for (MathVariableAST d : params) {
            names.add(d.getName().getText());
        }
        return names;
    }

    private static List<MTType> getParamTypes(List<MathVariableAST> params) {
    ...
    public static MTType buildParameterType(TypeGraph g,
        ...
        default:
            List<MTCartesian.Element> elements =
                    new LinkedList<edu.clemson.cs.r2jt.typeandpopulate2.MTCartesian.Element>();
