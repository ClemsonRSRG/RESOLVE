...
public class JavaTranslator extends AbstractTranslator {
    ...
    /**
     * <p>
     * The main translator flag. Tells the compiler convert RESOLVE source code
     * to Java source code.
     * </p>
     */
    ...
    /**
     * <p>
     * Tells the compiler to regenerate Java code for all supporting RESOLVE
     * source files.
     * </p>
     */
    ...
    /**
     * <p>
     * A mapping between the <code>ModuleArgumentItem</code>s representing the
     * actual arguments of a
     * <code>FacilityDec</code> and their formal <code>ModuleParameterDec</code>
     * bound counterparts.
     * </p>
     */
    ...
    /**
     * <p>
     * This set keeps track of the names of any <code>OperationDec</code>s that
     * parameterize the
     * current module.
     * </p>
     */
    ...
    /**
     * <p>
     * A <code>ModuleParameterization</code> corresponding to the
     * <code>EnhancementBodyItem</code>
     * being walked.
     * </p>
     */
    ...
    public void preFacilityModuleDec(FacilityModuleDec node) {
        ...
        ST facility = myGroup.getInstanceOf("facility_class").add("name",
    ...
    public void preConceptModuleDec(ConceptModuleDec node) {
        ...
        ST concept = myGroup.getInstanceOf("interface_class")
    ...
    public void preEnhancementModuleDec(EnhancementModuleDec node) {
        ...
        ST enhancement = myGroup.getInstanceOf("interface_class")
                .add("name", node.getName().getName())
                .add("extend", node.getConceptName().getName());
    ...
    public void preEnhancementBodyModuleDec(EnhancementBodyModuleDec node) {
        ...
        ST enhancementBody = myGroup.getInstanceOf("enhancement_body_class")
                .add("name", node.getName().getName())
                .add("conceptname", node.getConceptName().getName());
    ...
    public void preFacilityTypeDec(FacilityTypeDec e) {
        ST record = myGroup.getInstanceOf("record_class")
                .add("name", e.getName().getName()).add("facility", true);
    ...
    /**
     * <p>
     * This is where we give the enhancement body all the functionality defined
     * in the base concept.
    ...
    public void postEnhancementBodyModuleDec(EnhancementBodyModuleDec node) {
        ...
        try {
            ModuleScope conceptScope = myBuilder.getModuleScope(
                    new ModuleIdentifier(node.getConceptName().getName()));
            ...
            List<ProgramTypeDefinitionEntry> conceptTypes = conceptScope
            ...
            for (OperationEntry o : conceptOperations) {
                ...
                addEnhancementConceptualFunction(returnType, o.getName(),
                        o.getParameters());
            }
            ...
            for (ProgramParameterEntry p : getModuleFormalParameters(
                ...
                addEnhancementConceptualFunction(p.getDeclaredType(),
            ...
            for (ProgramTypeDefinitionEntry e : conceptTypes) {
                addEnhancementConceptualFunction(e.getProgramType(),
    ...
    public void preConceptBodyModuleDec(ConceptBodyModuleDec node) {
        ...
        ST conceptBody = myGroup.getInstanceOf("concept_body_class")
                .add("name", node.getName().getName())
                .add("implement", node.getConceptName().getName());
    ...
    public void preEnhancementBodyItem(EnhancementBodyItem node) {
        ...
        for (ModuleParameterization m : enhancements) {
            if (m.getModuleIdentifier().toString()
                    .equals(node.getName().getName())) {
                constructFacilityArgBindings(m,
    ...
    public void postFacilityDec(FacilityDec node) {
        ...
        ST facilityVariable = myGroup.getInstanceOf("var_decl")
                .add("type", facilityType).add("name", node.getName().getName())
                .add("init", myActiveTemplates.pop());

        myActiveTemplates.peek().add("variables", facilityVariable);

        myDynamicImports.add(myGroup.getInstanceOf("include")
    ...
    public void preModuleArgumentItem(ModuleArgumentItem node) {
        ...
        if (type instanceof PTVoid) {
            ...
            if (wrappedDec instanceof OperationDec) {
                ST argItem = getOperationArgItemTemplate(
                        (OperationDec) myFacilityBindings.get(node)
                                .getWrappedDec(),
                        node.getQualifier(), node.getName());
            ...
            ST argItem = myGroup.getInstanceOf("var_init")
    ...
    public void postConceptTypeParamDec(ConceptTypeParamDec node) {
        ...
        try {
            ProgramParameterEntry ppe = myScope
                    .queryForOne(new NameAndEntryTypeQuery(null, node.getName(),
    ...
    public void postConstantParamDec(ConstantParamDec node) {
        ...
        getter.add("stmts",
                myGroup.getInstanceOf("return_stmt").add("name", name));
    ...
    public void preModuleParameterDec(ModuleParameterDec node) {
        ...
        if (node.getWrappedDec() instanceof OperationDec) {
            ...
            ST parameter = myGroup.getInstanceOf("parameter")
                    .add("type", node.getName().getName())
                    .add("name", node.getName().getName() + "Param");

            ST operationInterface = myGroup.getInstanceOf("interface_class")
                    .add("name", node.getName().getName());
    ...
    public void preTypeDec(TypeDec node) {
        ...
        ST typeDec = myGroup.getInstanceOf("interface_class")
                .add("name", node.getName().getName()).add("extend", "RType")
                .add("public", false);

        myActiveTemplates.peek().add("classes", typeDec);

        try {
            ProgramTypeDefinitionEntry ptde = myScope
                    .queryForOne(
                            new UnqualifiedNameQuery(node.getName().getName()))
                    .toProgramTypeDefinitionEntry(node.getLocation());

            ST typeDefinition = getOperationLikeTemplate(ptde.getProgramType(),
                    "create" + node.getName().getName(), false);
    ...
    public void preCallStmt(CallStmt node) {
        ...
        ST callStmt;
        String qualifier = getCallQualifier(node.getQualifier(), node.getName(),
                node.getArguments());
        ...
        if (myParameterOperationNames.contains(node.getName().getName())) {
            callStmt = myGroup.getInstanceOf("qualified_call")
                    .add("name", node.getName().getName())
                    .add("qualifier", node.getName().getName() + "Param");
        }
            ...
            callStmt = myGroup.getInstanceOf("unqualified_call").add("name",
                    node.getName().getName());
        }
    ...
    public void preRepresentationDec(RepresentationDec node) {
        ...
        PTType repType = types.get(0).toProgramTypeEntry(node.getLocation())
                .getProgramType();
    ...
    public void postRepresentationDec(RepresentationDec node) {
        ST instance = myGroup.getInstanceOf("facility_init").add("realization",
                node.getName().getName());
        ...
        try {
            ...
            ST createMethod = getOperationLikeTemplate(pte.getProgramType(),
                    "create" + node.getName().getName(), true).add("stmts",
                            returnStmt);
    ...
    public void preSwapStmt(SwapStmt node) {
        ...
        ST swapStmt;
        boolean translatingEnhancement = myScope
                .getDefiningElement() instanceof EnhancementBodyModuleDec;

        if (node.getLeft().getProgramType() instanceof PTGeneric
                || translatingEnhancement) {

            swapStmt = myGroup.getInstanceOf("unqualified_call").add("name",
                    "swap");
        }
        else {
            ...
            swapStmt = myGroup.getInstanceOf("qualified_call")
    ...
    public void preVariableNameExp(VariableNameExp node) {
        ...
        nameExp = (nonLocal)
                ? nameExp.add("name", "get" + node.getName().getName() + "()")
                : nameExp.add("name", node.getName().getName());
    ...
    protected ST getVariableTypeTemplate(PTType type) {
        ...
        if (type instanceof PTGeneric || type instanceof PTElement) {
            result = myGroup.getInstanceOf("unqualified_type").add("name",
                    "RType");
        }
        else if (type instanceof PTRepresentation) {

            result = myGroup.getInstanceOf("qualified_type").add("name",
                    getTypeName(type));
            ...
            if (getDefiningFacilityEntry(type) != null) {
                concept = getDefiningFacilityEntry(type).getFacility()
                        .getSpecification().getModuleIdentifier().toString();
            }
            else if (myScope
                    .getDefiningElement() instanceof EnhancementModuleDec) {
                concept = ((EnhancementModuleDec) myScope.getDefiningElement())
                        .getConceptName().getName();
            }
            else if (myScope
                    .getDefiningElement() instanceof EnhancementBodyModuleDec) {
                concept = ((EnhancementBodyModuleDec) myScope
    ...
    private ST getOperationArgItemTemplate(OperationDec operation,
        ...
        int parameterNum = 0;
        ST result = myGroup.getInstanceOf("operation_argument_item")
        ...
        try {
            OperationEntry o = myScope
                    .queryForOne(new NameQuery(qualifier, name,
            ...
            for (ProgramParameterEntry p : o.getParameters()) {
                ...
                ST castedArg = myGroup.getInstanceOf("parameter")
                        .add("type", castedType)
            ...
            PTType returnType = (operation.getReturnTy() != null)
                    ? operation.getReturnTy().getProgramTypeValue()
                    : null;
     ...
     * <code>enhancementBodyModuleDec</code>s. It is used to construct and add a
     * 'dummy method' that
     * simply uses 'con' to call the actual method.
     * </p>
     *
     * <p>
     * For example, given <code>type</code> = null, <code>name</code> = 'Pop',
     * and
     * <code>parameters</code> = [R, S]; is method returns :
     * 
     * <pre>
     * 
    ...
    private void addEnhancementConceptualFunction(PTType type, String name,
        ...
        ST singleLine = myGroup.getInstanceOf("enhanced_stmt")
    ...
    /**
     * <p>
     * Binds <em>every</em> actual parameter of a <code>FacilityDec</code> to
     * its formal counterpart,
     * as defined in a concept, enhancement, or realization.
     * </p>
     *
     * @param spec A <code>ModuleParameterization</code> referencing a concept
     *        or enhancement.
     *
     * @param realiz A <code>ModuleParameterization</code> referencing a
     *        realization (enhancementbody
     *        or conceptbody).
     */
    public void constructFacilityArgBindings(ModuleParameterization spec,
        ...
        try {
            ...
            AbstractParameterizedModuleDec specModule =
                    (AbstractParameterizedModuleDec) myBuilder
                            .getModuleScope(spec.getModuleIdentifier())
                            .getDefiningElement();

            AbstractParameterizedModuleDec realizModule =
                    (AbstractParameterizedModuleDec) myBuilder
                            .getModuleScope(realiz.getModuleIdentifier())
                            .getDefiningElement();

            List<ModuleParameterDec> joinedFormals =
                    new LinkedList<ModuleParameterDec>(
                            specModule.getParameters());
     ...
     * <code>directives</code> attribute of the
     * outermost <code>module</code> template defined in <tt>Base.stg</tt>.
     *
    ...
    public void addPackageTemplate(ModuleDec node) {
        ...
        ST pkg = myGroup.getInstanceOf("package").add("directories",
                pkgDirectories);
