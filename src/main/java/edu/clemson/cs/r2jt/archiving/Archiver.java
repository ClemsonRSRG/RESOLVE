...
public class Archiver {
    ...
    /**
     * <p>
     * The main archiver flag. Tells the compiler to attempt to create an
     * executable jar from a
     * RESOLVE facility.
     * </p>
     */
    ...
    /**
     * <p>
     * The alternate archiver flag. Can be used to print the output from the
     * javac and jar misc when
     * there is a problem creating the executable jar.
     * </p>
     */
    public static final Flag FLAG_VERBOSE_ARCHIVE = new Flag(FLAG_SECTION_NAME,
            "verboseJar", FLAG_DESC_VERBOSE_ARCHIVE);
    ...
    private String[] stdResolve = { "RESOLVE_BASE.java",
            "RESOLVE_BASE_EXT.java", "RESOLVE_INTERFACE.java", "RType.java",
    ...
    public void addFileToArchive(File inputFile) {
        ...
        if (ext.equals("co") || ext.equals("rb") || ext.equals("en")
            ...
            /*
             * if(onNoCompileList(inFile)){ return; }
             */
            ...
            if (compiler != null) {
                ...
                if (ret == 0) {
                    ...
                    if (!webOutput) {
                        ...
                        myInstanceEnvironment.getCompileReport()
                                .addBugReport(msg);
                    }
                }
            }
    ...
    /**
     * <p>
     * Method to invoke the jar utility and create the jar file.
     * </p>
     *
     * @return 0 if successful
     */
    public int createArchive(boolean compileSuccess) {
        ...
        try {
            /*
             * String regex = "(?<=\\.).*$"; //the file extension String
             * extRegex = "(?<=\\.)class$";
             * //match .class files Pattern extPattern =
             * Pattern.compile(extRegex); Matcher extMatcher =
             * null;
             */
                ...
                if (modulePathDir.isDirectory()) {
                    ...
                    for (int i = 0; i < files.length; i++) {
                        ...
                        if (nameMatcher.find()) {
                            ...
                            createdFiles
                                    .add(new File(files[i].getAbsolutePath()));
                        }
                    }
                }
            }
    ...
    /**
     * <p>
     * Returns a string containing a jar-appropriate path. Jar appropriate
     * simply means making sure
     * that all paths are rooted in a directory named <code>RESOLVE</code>
     * (capitalization schemes
     * ignored) -- stripping any excess directories.</p
     *
     * <p>
     * For example, given:
     * </p>
     * 
     * <pre>
     * ~Documents / RESOLVE - Workspace / RESOLVE / Main / Facilities / f.fa
     * </pre>
     *
     * <p>
     * This method returns a path string that looks like:
     * </p>
     * 
     * <pre>
     * RESOLVE / Main / Facilities / f.fa
     * </pre>
     * 
     * .
     *
     * @param file A file object.
     * @return A string containing the a properly <code>RESOLVE</code> workspace
     *         root directory headed
     *         path.
     */
    ...
    /**
     * <p>
     * Method to delete all the files that were created by the archiver.
     * </p>
     */
    ...
    /**
     * <p>
     * Method used to print everything on the list to be translated (used for
     * testing).
     * </p>
     */
    ...
    /**
     * <p>
     * Method to generate the names of the standard imports to add. (need to
     * check the OS to find out
     * what kind of slash to use first).
     * </p>
     */
    private void createStandardImports() {
        ...
        stdImports[3] = "Char_Str_Template" + File.separator
                + "Std_Char_Str_Realiz.java";
        stdImports[4] = "Character_Template" + File.separator
                + "Character_Template.java";
        stdImports[5] = "Character_Template" + File.separator
        ...
        stdImports[12] = "Static_Array_Template" + File.separator
                + "Std_Array_Realiz.java";
        stdImports[13] = "Location_Linking_Template_1" + File.separator
    ...
    private void createGuiWrapper(File file) {
        GuiWrapper gui = new GuiWrapper("gui", file.getPath(), workspaceDir,
    ...
    private void printDiagnostics(
        ...
        for (Diagnostic<?> diagnostic : diagnosticListener.getDiagnostics()) {
            if (diagnostic.getKind().equals(Diagnostic.Kind.ERROR)) {
                String errMsg = diagnostic.getMessage(null)
                        .replaceAll(Pattern.quote(workspaceDir), "");
