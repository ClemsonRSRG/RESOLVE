...
/**
 * <p>
 * <code>java.util.LinkedList</code>> returns an iterator with the (excellent,
 * useful) feature of
 * throwing a <code>ConcurrentModificationException</code> if the underlying
 * list is changed while
 * iteration is still in progress. As it happens, the prover is set up such
 * that, while the
 * underlying list may change, if further iteration is required, it will always
 * be restored to its
 * original state before next() is called. Since, the java runtime library is
 * open source, this is
 * simply an exact copy of <code>java.util.LinkedList</code> that
 * <em>doesn't</em> perform this
 * modification check. It is otherwise identical.
 * </p>
 */
...
public class UnsafeIteratorLinkedList<E> extends AbstractSequentialList<E>
     ...
     * formally, returns
     * {@code true} if and only if this list contains at least one element
     * {@code e} such that
     ...
     * it is present. If this
     * list does not contain the element, it is unchanged. More formally,
     * removes the element with the
     * lowest index {@code i} such that
     * <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o.equals(get(i)))</tt>
     * (if such an element
     * exists). Returns {@code true} if this list contained the specified
     * element (or equivalently, if
     ...
     * the left (subtracts one from their indices). Returns the element that was
     * removed from the
     * list.
     *
     ...
     * possible refactorings of
     * the error handling code, this "outlining" performs best with both server
     * and client VMs.
     */
     ...
     * this list, or -1 if this
     * list does not contain the element. More formally, returns the lowest
     * index {@code i} such that
     * <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o.equals(get(i)))</tt>,
     * or -1 if there is no
     * such index.
     *
     ...
     * list, or -1 if this
     * list does not contain the element. More formally, returns the highest
     * index {@code i} such that
     * <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o.equals(get(i)))</tt>,
     * or -1 if there is no
     * such index.
     ...
     * traversing the list
     * from head to tail). If the list does not contain the element, it is
     * unchanged.
     *
     * @param o element to be removed from this list, if present
     ...
     * the Iterator is created, in any way except through the list-iterator's
     * own {@code remove} or
     ...
     * maintained by this list. (In
     * other words, this method must allocate a new array). The caller is thus
     * free to modify the
     * returned array.
     *
     * <p>
     ...
     * Returns an array containing all of the elements in this list in proper
     * sequence (from first to
     * last element); the runtime type of the returned array is that of the
     * specified array. If the
     * list fits in the specified array, it is returned therein. Otherwise, a
     ...
     * than the list), the element in the array immediately following the end of
     * the list is set to
     * {@code null}. (This is useful in determining the length of the list
     * <i>only</i> if the caller
     ...
     * output array, and may, under certain circumstances, be used to save
     * allocation costs.
     *
     * <p>
