...
import edu.clemson.cs.r2jt.rewriteprover.gui.JProverFrame;
import edu.clemson.cs.r2jt.rewriteprover.justifications.Library;
...
public class AlgebraicProver {
    ...
    /**
     * <p>
     * Makes the prover start in interactive mode by default.
     * </p>
     */
    public static final Flag FLAG_INTERACTIVE = new Flag(
            Prover.FLAG_SECTION_NAME, "interactive", FLAG_DESC_INTERACTIVE);
    ...
    public AlgebraicProver(TypeGraph g, List<VC> vcs, ModuleScope scope,
        ...
        myModels[0] = new PerVCProverModel(g, vcs.get(0).getName(), vcs.get(0),
                myTheoremLibrary);
        myAutomatedProvers[0] = new AutomatedProver(myModels[0],
                myTheoremLibrary, scope, myTimeout);
        ...
        while (myRunningFlag) {
            ...
            if (myModels[myVCIndex].noConsequents()
                ...
                if (!myModels[myVCIndex].getProofSteps().isEmpty()
                        && myModels[myVCIndex]
                                .getLastProofStep() instanceof LabelStep) {
    ...
    private void outputProofFile() throws IOException {
        ...
        for (int i = 0; i < myModels.length; i++) {
            ...
            if (myModels[i].noConsequents()) {
                ...
                PerVCProverModel workingModel = new PerVCProverModel(
                        myTypeGraph, myVCs.get(i).getName(), myVCs.get(i),
                        myTheoremLibrary);
                ...
                for (ProofStep step : steps) {
                    ...
                    if (step.getApplication() != lastApplication) {
                        ...
                        if (doneWithAntecedentDevelopment && !(step
                                .getTransformation() instanceof EliminateTrueConjunctInConsequent)
                                && !(step
                                        .getTransformation() instanceof ReplaceSymmetricEqualityWithTrueInConsequent)
                                && !(step
                        ...
                        if (stepTransformation instanceof NoOpLabel) {
                            doneWithAntecedentDevelopment =
                                    doneWithAntecedentDevelopment
                                            || stepTransformation.toString()
                                                    .equals(AutomatedProver.SEARCH_START_LABEL);
            ...
            if (myModels[i].noConsequents()) {
                w.write("proved in "
                        + myAutomatedProvers[i].getLastStartLength() + "ms via "
                        + stepCount[i] + " steps (" + searchStepCount[i]
                        + " search)\n");
            }
            else {
    ...
    private void setVCIndex(int index) {
        ...
        if (myModels[myVCIndex] == null) {
            myModels[myVCIndex] = new PerVCProverModel(myTypeGraph,
                    myVCs.get(myVCIndex).getName(), myVCs.get(myVCIndex),
                    myTheoremLibrary);
    ...
    private class AlphabeticalByTheoremName
            implements
                Comparator<TheoremEntry> {
