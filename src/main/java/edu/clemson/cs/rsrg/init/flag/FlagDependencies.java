...
public class FlagDependencies {
    ...
    static void introduceFlag(Flag f) {
        ...
        if (mySealedFlag) {
            throw new IllegalStateException("Cannot create a new flag after "
                    ...
                    + "method in setUpFlagDependencies()?");
        }
     ...
     *
     * @throws NoSuchElementException If no flag with the given name exists.
     *
    ...
    /**
     * <p>
     * Creates an <em>implies</em> relationship between two flags. If the user
     * turns on the
     * <code>ifOn</code> flag, the <code>impliesOn</code> flag will be
     * automatically turned on too.
     * </p>
     *
    ...
    private static void checkFlagHasBeenIntroduced(Flag f) {
        if (!myFlags.containsKey(f)) {
            throw new MiscErrorException(f.getName(),
