...
/**
 * <p>
 * This is the class for all the "old" mathematical expression objects that the
 * compiler builds
 * using the ANTLR4 AST nodes.
 * </p>
 *
 * <p>
 * An "old" expression is an expression that refers to the incoming value of the
 * expression.
 * </p>
 *
 * @version 2.0
    ...
    protected final Exp substituteChildren(Map<Exp, Exp> substitutions) {
            ...
            // up to the name, then we need to apply the substitution.
                ...
                if (myOrigExp instanceof DotExp) {
                    ...
                    if (lastSegment instanceof FunctionExp) {
                        ...
                        FunctionExp lastSegmentAsFunctionExp =
                                (FunctionExp) lastSegment;

                        // Construct a new DotExp to compare
                        List<Exp> newSegments =
                        ...
                        OldExp newOldExp = new OldExp(myLoc,
                                new DotExp(
                                        myOrigExpAsDotExp.getLocation().clone(),
                                        newSegments));
                    ...
                    OldExp newOldExp =
                            new OldExp(myLoc, myOrigExpAsFunctionExp.getName());
                    if (nextKey.equivalent(newOldExp)) {
                        substitutionKey = nextKey;
                        oldExpWithFuncNameOnly = newOldExp;
        ...
        if (substitutionKey == null) {
            ...
            if (oldExpWithFuncNameOnly != null) {
                return substituteFunctionExp(innerFunctionExp, replacementExp,
                        substitutions);
            }
