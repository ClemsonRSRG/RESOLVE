...
/**
 * <p>
 * This is the abstract base class for all the expression objects that the
...
public abstract class Exp extends ResolveConceptualElement {
    ...
    /**
     * <p>
     * An object that contains additional information on where this expression
     * came from. This should
     * be added by the {@code VCGenerator} when applying the various different
     * {@code proof rules}.
     * </p>
     *
     * <p>
     * Note: We were trying to hard to make things immutable. There are things
     * that simply can't be
     * immutable until we are done creating objects. This is probably the best
     * compromise given the
     * design decision of the {@code Exp} hierarchy.
     * </p>
     */
     ...
     *
     * @return A {@link VarExp} containing {@code true} if it is exactly the
     *         same, otherwise just
     *         return a deep copy our ourselves.
     */
    ...
    /**
     * <p>
     * This method must be implemented by all inherited classes to attempt to
     * find the provided
     * expression in our sub-expressions.
     * </p>
     *
     * @param exp The expression we wish to locate.
     *
     * @return {@code true} if there is an instance of {@code exp} within this
     *         object's
     *         sub-expressions. {@code false} otherwise.
     */
     ...
     * method is only invoked by a mathematical expression, but since we could
     * have either
     * mathematical or programming expressions, the default behavior is to
     * return {@code false}.
     * </p>
     *
     * <p>
     * Any inherited mathematical expressions must override this method to make
     * this method work.
     * </p>
     *
    ...
    public boolean equals(Object o) {
        ...
        if (myLocationDetailModel != null
                ? !myLocationDetailModel.equals(exp.myLocationDetailModel)
                : exp.myLocationDetailModel != null)
            return false;
        if (myMathType != null ? !myMathType.equals(exp.myMathType)
                : exp.myMathType != null)
            return false;
        return myMathTypeValue != null
                ? myMathTypeValue.equals(exp.myMathTypeValue)
                : exp.myMathTypeValue == null;
    }
     ...
     * This method returns
     * {@code true} <strong>iff</strong> this expression and the provided
     * expression, {@code e}, are
     * equivalent with respect to structure and all function and variable names.
     * </p>
     *
    ...
    /**
     * <p>
     * Helper method to deal with {@link Exp}s that need to be compared but
     * might be {@code null}.
     * Returns {@code true} <strong>iff</strong> {@code e1} and {@code e2} are
     * both {@code null} or
     * both are not {@code null} and equivalent.
     * </p>
     *
     * @param e1 The first {@link Exp}.
     * @param e2 The second {@link Exp}.
     *
     * @return {@code true} <strong>iff</strong> both {@link Exp}s are
     *         {@code null}; or both are not
     *         {@code null} and are equivalent.
     */
    ...
    /**
     * <p>
     * This method must be implemented by all inherited classes to return the
    ...
    public int hashCode() {
        int result =
                myLocationDetailModel != null ? myLocationDetailModel.hashCode()
                        : 0;
    ...
    /**
     * <p>
     * Helper method to deal with {@link PosSymbol}s that need to be compared
     * but might be
     * {@code null}. Returns {@code true} <strong>iff</strong> {@code s1} and
     * {@code s2} are both
     * {@code null} or both are not {@code null} and have names that are
     * equivalent strings (see
     * {@link Exp#stringEquivalent(String, String)})
     * </p>
     *
     * @param s1 The first {@link PosSymbol}.
     * @param s2 The second {@link PosSymbol}.
     *
     * @return {@code true} <strong>iff</strong> both {@link PosSymbol}s are
     *         {@code null}; or both are
     *         not {@code null} and have names that are equivalent strings (see
     *         {@link Exp#stringEquivalent(String, String)}).
     */
     ...
     */
    public final void
            setLocationDetailModel(LocationDetailModel locationDetailModel) {
     ...
     * {@code null}. Returns
     * {@code true} <strong>iff</strong> {@code s1} and {@code s2} are both
     * {@code null} or both are
     * not {@code null} and represent the same string (case sensitive).
     * </p>
     *
     ...
     *         or both are not
     *         {@code null} and represent the same string.
     */
     ...
     * {@link Exp}s that occur as keys
     * in {@code substitutions} replaced with their corresponding values.
     * </p>
     * 
     * <p>
     * In general, a key {@link Exp} "occurs" in this {@link Exp} if either this
     * {@link Exp} or some
     * sub-expression is {@link Exp#equivalent(Exp)}. However, if the key is a
     * {@link VarExp} function
     * names are additionally matched, even though they would not ordinarily
     * match via
     * {@link Exp#equivalent(Exp)}, so function names can be substituted without
     * affecting their
     * arguments.
     * </p>
     * 
     ...
     *
     * @return A new {@link Exp} that is a deep copy of the original with the
    ...
    /**
     * <p>
     * Implemented by concrete subclasses of {@link Exp} to manufacture a copy
     * of themselves.
     * </p>
     *
    ...
    protected Exp copy() {
        throw new MiscErrorException(
                "Shouldn't be calling copy() from type " + this.getClass(),
     ...
     * <p>
     * Implemented by concrete subclasses of {@link Exp} to manufacture a copy
     * of themselves where all
     * sub-expressions have been appropriately substituted. The concrete
     * subclass may assume that
     * {@code this} does not match any key in {@code substitutions} and thus
     * need only concern itself
     * with performing substitutions in its children.
     * </p>
     * 
     * @param substitutions A mapping from {@link Exp}s that should be
     *        substituted out to the
