...
public class TreeBuildingListener extends ResolveParserBaseListener {
     ...
     */
    @Override
    public void exitProgVarDotNameExp(ResolveParser.ProgVarDotNameExpContext ctx) {
        // Create a dot expression
        List<ResolveParser.ProgNamedExpContext> progNamedExp = ctx.progDotExp().progNamedExp();
     ...
     */
    @Override
    public void enterProgVarDotArrayExp(
    ...
    public void exitProgVarDotArrayExp(
            ResolveParser.ProgVarDotArrayExpContext ctx) {
        // Create a dot expression
        List<ResolveParser.ProgNamedExpContext> progNamedExp = ctx.progDotArrayExp().progNamedExp();
        List<ProgramVariableExp> dotExps = new ArrayList<>();
        for (ResolveParser.ProgNamedExpContext context : progNamedExp) {
            dotExps.add((ProgramVariableExp) myNodes.removeFrom(context));
        }
        ...
        ResolveParser.ProgArrayExpContext arrayContext = ctx.progDotArrayExp().progArrayExp();
        dotExps.add(new ProgramArrayExp(createLocation(arrayContext),
                (ProgramVariableExp) myNodes.removeFrom(arrayContext.progNamedExp()),
                (ProgramExp) myNodes.removeFrom(arrayContext.progExp())));
    ...
    public void exitProgVarArrayExp(ResolveParser.ProgVarArrayExpContext ctx) {
        myNodes.put(ctx, new ProgramArrayExp(createLocation(ctx),
                (ProgramVariableExp) myNodes.removeFrom(ctx.progArrayExp()
                        .progNamedExp()), (ProgramExp) myNodes.removeFrom(ctx
                        .progArrayExp().progExp())));
    }

    /**
     * {@inheritDoc}
     * <p>
     * <p>This method stores a program variable name expression representation
     * generated by its child rules.</p>
     *
    ...
    public void exitProgVarNameExp(ResolveParser.ProgVarNameExpContext ctx) {
        myNodes.put(ctx, myNodes.removeFrom(ctx.progNamedExp()));
    }
