...
public class FacilityEntry extends SymbolTableEntry {
    ...
    /**
     * <p>
     * A flag that indicates whether or not this entry instantiates a
    ...
    /**
     * <p>
     * The module specification/realization that this facility entry is
     * instantiating.
     * </p>
     */
     ...
     * </p>
     */
    private final Map<ModuleParameterization, ModuleParameterization> myEnhancementRealizations =
            new HashMap<>();
     ...
     *        in.
     */
    public FacilityEntry(FacilityDec facility,
        ...
        if (facility.getConceptRealizName() != null) {
            realization = new ModuleParameterization(
                    new ModuleIdentifier(
                            facility.getConceptRealizName().getName()),
                    facility.getConceptParams(), this, mySourceRepository);
        }

        myType = new SpecRealizationPairing(facility.getLocation(), spec,
                realization);

        // These are realized by the concept realization
        for (EnhancementSpecItem realizationEnhancement : facility
                .getEnhancements()) {
        ...
        for (EnhancementSpecRealizItem enhancement : facility
                .getEnhancementRealizPairs()) {
            spec = new ModuleParameterization(
                    new ModuleIdentifier(
                            enhancement.getEnhancementName().getName()),
                    enhancement.getEnhancementParams(), this,
                    mySourceRepository);
            realization = new ModuleParameterization(
                    new ModuleIdentifier(
                            enhancement.getEnhancementRealizName().getName()),
                    enhancement.getEnhancementRealizParams(), this,
                    mySourceRepository);
     ...
     * </p>
     *
     * @param enhancement The enhancement that we are searching for its
     *        realization.
     *
     * @return The module that is realizing the specified enhancement.
     */
    public final ModuleParameterization
    ...
    /**
     * <p>
     * This returns the module specification and realization pairing for this
     * facility entry.
     * </p>
     *
     ...
     */
    @Override
    public final FacilityEntry instantiateGenerics(
    ...
    /**
     * <p>
     * This method returns a boolean that indicates whether or not this entry
     * instantiates a
     * {@code sharing concept}.
     * </p>
     *
