...
/**
 * <p>
 * The <code>HardCoded</code> class defines all mathematical symbols and
 * relationships that are
 * built into the compiler and acts as the base for where types, definitions,
 * theorems, etc.
 * </p>
 *
 * @version 2.0
...
public class HardCoded {
    ...
    public static void addBuiltInRelationships(TypeGraph g,
        ...
        try {
            ...
            Location classTheoryLoc = new Location(
                    new ResolveFile(new ResolveFileBasicInfo("Cls_Theory", ""),
                            ModuleType.THEORY,
            ...
            // Various binding
            typeTheorem1Scope.addBinding("D1", Quantification.UNIVERSAL, v,
                    g.CLS);
            typeTheorem1Scope.addBinding("R1", Quantification.UNIVERSAL, v,
                    g.CLS);
            ...
            typeTheorem1Scope.addBinding("f", Quantification.UNIVERSAL, v,
                    new MTFunction(g, new MTNamed(g, "R2"),
                            new MTNamed(g, "D2")));

            // VarExp refering to function f
            VarExp f = new VarExp(classTheoryLoc.clone(), null,
            ...
            // Various binding
            typeTheorem2Scope.addBinding("T1", Quantification.UNIVERSAL, v,
                    g.CLS);
            typeTheorem2Scope.addBinding("T2", Quantification.UNIVERSAL, v,
                    g.CLS);
            ...
            bindingTypeElements
                    .add(new MTCartesian.Element(new MTNamed(g, "T1")));
            bindingTypeElements
                    .add(new MTCartesian.Element(new MTNamed(g, "T2")));
    ...
    public static void addBuiltInSymbols(TypeGraph g, ScopeBuilder b) {
        try {
            ...
            // Since adding a binding require something of ResolveConceptualElement,
            // we associate everything with a VarExp.
            Location globalSpaceLoc = new Location(new ResolveFile(
                    new ResolveFileBasicInfo("Global", ""), ModuleType.THEORY,
                    new UnbufferedCharStream(new StringReader("")), null,
                    new ArrayList<String>(), ""), 0, 0);
            VarExp v = new VarExp(globalSpaceLoc, null,
                    new PosSymbol(globalSpaceLoc, "Global"));
    ...
    /**
     * <p>
     * This method returns the mathematical function used to represent an
     * expression's meta-segment.
     * </p>
     *
     * @param g The current type graph.
    ...
    public static MTType getMetaFieldType(TypeGraph g, Exp lastExp,
        ...
        if (lastExp.getMathTypeValue() != null) {
            ...
            case "Is_Initial":
                result = new MTFunction(g, g.BOOLEAN,
                        lastExp.getMathTypeValue());
                break;
            case "Val_in":
                result = new MTFunction(g, lastExp.getMathTypeValue(),
                        g.RECEPTACLES);
                break;
            }
        }
