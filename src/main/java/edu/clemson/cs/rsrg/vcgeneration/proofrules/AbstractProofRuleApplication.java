...
public abstract class AbstractProofRuleApplication
    ...
    /**
     * <p>
     * The current {@link AssertiveCodeBlock} that the {@code Proof Rules} will
     * operate on.
     * </p>
     */
    ...
    /**
     * <p>
     * A map that stores string template models for generated
    ...
    /**
     * <p>
     * A double ended queue that contains all the assertive code blocks that was
     * either passed in or
     * generated by the {@code Proof Rule}.
     * </p>
     */
    ...
    /**
     * <p>
     * An helper constructor that creates a double ended queue to store the
     * {@link AssertiveCodeBlock}
     * that was passed in as well as any generated from a class that inherits
     * from
     * {@code AbstractProofRuleApplication}.
     * </p>
     *
    ...
    public final Deque<AssertiveCodeBlock> getAssertiveCodeBlocks() {
        Deque<AssertiveCodeBlock> blocks =
                new LinkedList<>(myResultingAssertiveCodeBlocks);
     ...
     * @param affectsClause The {@code affects} clause associated with the
     *        ensures clause.
     *
     * @return The modified {@code ensures} clause expression.
     */
    protected final Exp createEnsuresExpWithModifiedSharedVars(Location loc,
            Exp originalExp, PosSymbol facQualifier,
        ...
        if (affectsClause != null) {
            for (Exp affectedExp : affectsClause.getAffectedExps()) {
                ...
                VarExp modifiedAffectsExp = Utilities.createVarExp(loc.clone(),
                        facQualifier, originalAffectsExp.getName(),
                        affectedExp.getMathType(),
                        affectedExp.getMathTypeValue());
                substitutions.put(new OldExp(originalAffectsExp.getLocation(),
    ...
    /**
     * <p>
     * An helper method that performs the substitution on all the {@link Exp} in
     * each
    ...
    /**
     * <p>
     * An helper method for generating a conjunction of {@code Def Var} with
     * their definitions.
     * </p>
     *
     ...
     *
     * @return If there are {@code Def Vars}, it returns a conjuncted expression
     *         containing the
     *         {@code Def Vars} with their definitions. Otherwise, it returns an
     *         expression
     *         representing {@code true}.
     */
    protected final Exp generateDefVarExps(PosSymbol facilityName,
        ...
        Exp retExp = VarExp.getTrueVarExp(typeFamilyDec.getLocation(),
        ...
        for (MathDefVariableDec defVariableDec : typeFamilyDec
            ...
            VarExp defVarAsVarExp =
                    Utilities.createVarExp(defVariableDec.getLocation().clone(),
                            null, defVariableDec.getName(),
                            defVariableDec.getMathType(), null);
            ...
            if (defVariableDec.getDefinitionAsExp() != null) {
                EqualsExp definitionExp = new EqualsExp(
                        defVariableDec.getLocation().clone(),
                        qualifiedDefVarAsVarExp, null, EqualsExp.Operator.EQUAL,
                        defVariableDec.getDefinitionAsExp().clone());
                definitionExp.setMathType(
                ...
                definitionExp.setLocationDetailModel(new LocationDetailModel(
                        ...
                        definitionExp.getLocation().clone(),
                        "Def Var: " + defVariableDec.getVariable().getName()
                                .getName()));
                ...
                if (VarExp.isLiteralTrue(retExp)) {
                    ...
                    retExp = MathExp.formConjunct(retExp.getLocation().clone(),
                            retExp, definitionExp);
                }
            }
        ...
        // containing the proper facility qualifier.
        VarExp typeAsVarExp = Utilities.createVarExp(
                typeFamilyDec.getLocation().clone(), null,
                typeFamilyDec.getName(), typeFamilyDec.getModel().getMathType(),
        ...
        for (SharedStateDec stateDec : instantiatedFacilityDecl
