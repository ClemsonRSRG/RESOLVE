...
public class CallStmtRule extends AbstractProofRuleApplication
    ...
    /**
     * <p>
     * The list of {@link ConfirmStmt ConfirmStmts} generated by some nested
     * function in our
     * {@link CallStmt} argument list.
     * </p>
     */
    ...
    /**
     * <p>
     * The list of {@code requires} clauses generated by some nested function in
     * our {@link CallStmt}
     * argument list.
     * </p>
     */
     ...
     * </p>
     *
     * @param callStmt The {@link CallStmt} we are applying the rule to.
    ...
    public final void applyRule() {
        ...
        List<VarExp> operationParamAsVarExps = Utilities
                .createOperationParamExpList(operationDec.getParameters());
        ...
        Exp requiresExp = createModifiedReqExp(operationEntry,
        ...
        Exp ensuresExp = createModifiedEnsExp(operationEntry, callArgs,
                modifiedArguments);
        AssumeStmt assumeStmt = new AssumeStmt(myCallStmt.getLocation().clone(),
                ensuresExp, false);
    ...
    private void createDurationEnsExp() {
        /*
         * // NY YS // Duration for CallStmt if
         * (myInstanceEnvironment.flags.isFlagSet(FLAG_ALTPVCS_VC))
         * { Location loc = (Location) stmt.getLocation().clone(); ConfirmStmt
         * finalConfirm =
         * myCurrentAssertiveCode.getFinalConfirm(); Exp finalConfirmExp =
         ...
         * stmt.getName(), argTypes,
         * myCurrentModuleScope);
         * 
         * // Add the profile ensures as additional assume Exp profileEnsures =
         * ope.getEnsuresClause();
         * if (profileEnsures != null) { profileEnsures =
         * replaceFormalWithActualEns(profileEnsures,
         * opDec .getParameters(), opDec.getStateVars(), replaceArgs, false);
         * 
         * // Obtain the current location if (stmt.getName().getLocation() !=
         * null) { // Set the details
         * of the current location Location ensuresLoc = (Location) loc.clone();
         * ensuresLoc.setDetails("Ensures Clause of " + opDec.getName() +
         * " from Profile " +
         * ope.getName()); Utilities.setLocation(profileEnsures, ensuresLoc); }
         * 
         * ensures = myTypeGraph.formConjunct(ensures, profileEnsures); }
         * 
         * // Construct the Duration Clause Exp opDur =
         * Exp.copy(ope.getDurationClause());
         * 
         * // Replace PostCondition variables in the duration clause opDur =
         ...
         * Utilities.createPosSymbol(Utilities .getCumDur(finalConfirmExp)),
         * myTypeGraph.R, null); Exp
         * durCallExp = Utilities.createDurCallExp((Location) loc.clone(),
         * Integer
         * .toString(opDec.getParameters().size()), Z, myTypeGraph.R); InfixExp
         * sumEvalDur = new
         * InfixExp((Location) loc.clone(), opDur, Utilities
         * .createPosSymbol("+"), durCallExp);
         * sumEvalDur.setMathType(myTypeGraph.R); sumEvalDur = new
         * InfixExp((Location) loc.clone(),
         * Exp.copy(cumDur), Utilities.createPosSymbol("+"), sumEvalDur);
         * sumEvalDur.setMathType(myTypeGraph.R);
         * 
         * // For any evaluates mode expression, we need to finalize the
         * variable
         * edu.clemson.cs.r2jt.collections.List<ProgramExp> assignExpList =
         * stmt.getArguments(); for
         * (int i = 0; i < assignExpList.size(); i++) { ParameterVarDec p =
         * opDec.getParameters().get(i); VariableExp pExp = (VariableExp)
         * assignExpList.get(i); if
         * (p.getMode() == Mode.EVALUATES) { VarDec v = new
         * VarDec(Utilities.getVarName(pExp),
         * p.getTy()); FunctionExp finalDur = Utilities.createFinalizAnyDur(v,
         * myTypeGraph.R);
         * sumEvalDur = new InfixExp((Location) loc.clone(), sumEvalDur,
         * Utilities.createPosSymbol("+"),
         * finalDur); sumEvalDur.setMathType(myTypeGraph.R); } }
         * 
         * // Replace Cum_Dur in our final ensures clause finalConfirmExp =
    ...
    /**
     * <p>
     * An helper method for creating the modified {@code ensures} clause that
     * contains the modified
     * {@code ensures} clause from the call statement as well as any parameter
     * {@code ensures}
     * clauses.
     * </p>
     *
     * <p>
     * Note that this helper method also does all the appropriate substitutions
     * to the {@code VCs} in
     * the assertive code block.
     * </p>
     *
    ...
    private Exp createModifiedEnsExp(OperationEntry operationEntry,
        ...
        InstantiatedFacilityDecl instantiatedFacilityDecl =
                Utilities.getInstantiatingFacility(operationEntry,
        ...
        Exp ensuresExp = Utilities.formConjunct(ensuresClause.getLocation(),
                null, ensuresClause,
                new LocationDetailModel(ensuresClause.getLocation().clone(),
        ...
        for (int i = 0; i < paramList.size(); i++) {
            ...
            VarExp parameterExp =
                    Utilities.createVarExp(varDec.getLocation().clone(), null,
                            varDec.getName().clone(),
                            varDec.getTy().getMathTypeValue(), null);
            OldExp oldParameterExp = new OldExp(varDec.getLocation().clone(),
                    parameterExp.clone());
            ...
            VarExp tempParamExp =
                    Utilities.createVarExp(varDec.getLocation().clone(), null,
                            new PosSymbol(parameterExp.getLocation().clone(),
                                    "_" + parameterExp.getName().getName()),
                            exp.getMathType(), exp.getMathTypeValue());
            OldExp tempOldParamExp = new OldExp(varDec.getLocation().clone(),
                    tempParamExp.clone());
            ...
            if (nameTy.getProgramType() instanceof PTGeneric) {
                ...
                if (callingArg.getProgramType() instanceof PTFamily) {
                    PTFamily callingType =
                            (PTFamily) callingArg.getProgramType();
                    ste = Utilities.searchProgramType(callingArg.getLocation(),
                            null,
                            new PosSymbol(callingArg.getLocation(),
                                    callingType.getName()),
                            myCurrentModuleScope);
                }
                // Note 2: Both parameters must be generic, so look
                else {
                    PTGeneric callingType =
                            (PTGeneric) callingArg.getProgramType();
            ...
            if (varDec.getMode().equals(ParameterMode.ALTERS)) {
                // NQV(exp)
                VCVarExp nqvExp = Utilities
                        .createVCVarExp(myCurrentAssertiveCodeBlock, exp);
                ...
                if (typeEntry.getDefiningElement() instanceof TypeFamilyDec) {
                    ...
                    if (!VarExp.isLiteralTrue(
                        ...
                        varDecEnsures =
                                modifiedConstraint.getAssertionExp().clone();
                ...
                substitutionParamToTemp.put(oldParameterExp,
                        tempOldParamExp.clone());
            ...
            // 2) CLEARS Mode
                ...
                if (typeEntry.getDefiningElement() instanceof TypeFamilyDec) {
                    ...
                    AssertionClause modifiedInitEnsures =
                            Utilities.getTypeInitEnsuresClause(initEnsures,
                                    exp.getLocation(), null, varDec.getName(),
                                    type.getExemplar(),
                                    typeEntry.getModelType(), null);
                    varDecEnsures =
                ...
                // - T7.Is_Initial( NQV(RS, h) )
                    ...
                    varDecEnsures = Utilities.createInitExp(
                            new VarDec(varDec.getName(), nameTy),
                            myTypeGraph.BOOLEAN);
                }
                ...
                // Store the new location detail.
                varDecEnsures.setLocationDetailModel(new LocationDetailModel(
                        varDec.getLocation().clone(), exp.getLocation().clone(),
                        "Ensures Clause of " + operationDec.getName()
                                + " (Condition from \""
            ...
            if (varDecEnsures != null && !VarExp.isLiteralTrue(varDecEnsures)) {
                if (parameterEnsures == null) {
                    ...
                    parameterEnsures = MathExp.formConjunct(
                            myCallStmt.getLocation().clone(), parameterEnsures,
                            varDecEnsures);
                }
            }
        }
                ...
                for (Exp affectedExp : clause.getAffectedExps()) {
                    ...
                    VarExp modifiedAffectsExp =
                            Utilities.createVarExp(clause.getLocation().clone(),
                                    ...
                                    affectedExp.getMathType(),
                                    affectedExp.getMathTypeValue());
                    sharedVarSubstitution
                            .put(new OldExp(originalAffectsExp.getLocation(),
                                    originalAffectsExp), modifiedAffectsExp);

                    // 2) originalAffectsExp ~> NQV(modifiedAffectsExp)
                    VCVarExp vcVarExp = Utilities.createVCVarExp(
                            myCurrentAssertiveCodeBlock, modifiedAffectsExp);
                    ...
                    substitutionsForSeq.put(modifiedAffectsExp.clone(),
                            vcVarExp.clone());
                }
            ...
            // For non-facility instantiated operations, we simply generate the proper NQV substitutions
                ...
                for (Exp affectedExp : clause.getAffectedExps()) {
                    ...
                    OldExp oldAffectedExp =
                            new OldExp(originalAffectsExp.getLocation().clone(),
                                    originalAffectsExp.clone());
                    oldAffectedExp
                            .setMathType(originalAffectsExp.getMathType());

                    // NQV(affectedExp)
                    VCVarExp nqvAffectsExp = Utilities.createVCVarExp(
                            myCurrentAssertiveCodeBlock, originalAffectsExp);
                    ...
                    // A temporary VarExp that avoids any formal with the same name as the actual.
                    VarExp tempAffectsExp = Utilities.createVarExp(
                            originalAffectsExp.getLocation().clone(),
                            originalAffectsExp.getQualifier(),
                            new PosSymbol(
                                    originalAffectsExp.getLocation().clone(),
                                    "_" + originalAffectsExp.getName()
                                            .getName()),
                            originalAffectsExp.getMathType(),
                            originalAffectsExp.getMathTypeValue());
                    ...
                    substitutionParamToTemp.put(affectedExp,
                            tempAffectsExp.clone());
                    ...
                    substitutionParamToTemp.put(oldAffectedExp,
                            tempOldAffectsExp.clone());
                    substitutionTempToActual.put(tempOldAffectsExp,
                            affectedExp);

                    // Substitutions for sequents in VCs
                    // 1) affectedExp ~> NQV(affectedExp)
                    substitutionsForSeq.put(affectedExp.clone(),
                            nqvAffectsExp.clone());
                }
            }
        }
        ...
        List<VerificationCondition> newVCs = createReplacementVCs(
                myCurrentAssertiveCodeBlock.getVCs(), substitutionsForSeq);
    ...
    /**
     * <p>
     * An helper method for creating the modified {@code requires} clause that
     * contains all invoking
     * conditions for nested function calls as well as the modified
     * {@code requires} clause from the
     * call statement.
     * </p>
     *
     * <p>
     * Note that if any of the function or regular calls happen to be recursive,
     * then this will also
     * generate the appropriate termination clauses and add it to our current
     * {@link AssertiveCodeBlock}.
     * </p>
     *
     * @param operationEntry Calling statement's {@link OperationEntry}.
     * @param operationParamAsVarExps List of operation parameters as
     *        {@link VarExp VarExps}.
     * @param modifiedArguments List of modified calling arguments.
     *
    ...
    private Exp createModifiedReqExp(OperationEntry operationEntry,
        ...
        if (!VarExp.isLiteralTrue(requiresExp)) {
            ...
            if (operationDec.getAffectedVars() != null) {
                ...
                // facility instantiated version.
                if (instantiatedFacilityDecl != null) {
                    PosSymbol facQualifier = instantiatedFacilityDecl
                    ...
                    for (Exp affectedExp : clause.getAffectedExps()) {
                        // 1) originalAffectsExp ~> facility qualified modifiedAffectsExp
                        VarExp originalAffectsExp = (VarExp) affectedExp;
                        VarExp modifiedAffectsExp = Utilities.createVarExp(
                                clause.getLocation().clone(), facQualifier,
                                originalAffectsExp.getName(),
                                affectedExp.getMathType(),
                                affectedExp.getMathTypeValue());
                        sharedVarSubstitution.put(originalAffectsExp,
                                modifiedAffectsExp);
                    }
        ...
        Exp conjunctRequiresExp = VarExp
                .getTrueVarExp(myCallStmt.getLocation().clone(), myTypeGraph);
    ...
    /**
     * <p>
     * An helper method for generating a termination clause if our current
     * {@link CallStmt} is a
     * recursive call to our current recursive {@code procedure}.
     * </p>
     *
    ...
    private Exp createTerminationReqExp() {
        ...
        IntegerExp oneExp = new IntegerExp(
                myCurrentProcedureDecreasingExp.getLocation().clone(), null, 1);
        ...
        InfixExp sumExp =
                new InfixExp(
                        myCurrentProcedureDecreasingExp.getLocation().clone(),
                        oneExp, null,
                        new PosSymbol(myCurrentProcedureDecreasingExp
                                .getLocation().clone(), "+"),
        ...
        InfixExp terminationExp = new InfixExp(
                myCurrentProcedureDecreasingExp.getLocation().clone(), sumExp,
                null,
                new PosSymbol(
                        myCurrentProcedureDecreasingExp.getLocation().clone(),
                        "<="),
    ...
    /**
     * <p>
     * An helper method for modifying the argument expression list if we have a
    ...
    private List<Exp> modifyArgumentList(List<ProgramExp> callArgs) {
        ...
        for (ProgramExp exp : callArgs) {
            ...
            if (exp instanceof ProgramFunctionExp) {
                ...
                ProgramFunctionExp expAsProgramFunctionexp =
                ...
                Exp generatedRequires = walker.getRequiresClause(
                        expAsProgramFunctionexp.getLocation().clone());
