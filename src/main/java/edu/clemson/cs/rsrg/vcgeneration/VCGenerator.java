...
/**
 * <p>
 * This class generates verification conditions (VCs) using the provided RESOLVE
 * abstract syntax
 * tree. This visitor logic is implemented as a {@link TreeWalkerVisitor}.
 * </p>
 *
...
public class VCGenerator extends TreeWalkerVisitor {
    ...
    /**
     * <p>
     * While walking a {@link SharedStateRealizationDec}, this will be set to
     * its corresponding shared
     * state declaration in the concept.
     * </p>
     */
    ...
    /**
     * <p>
     * An outer declaration that has {@link RealizInitFinalItem
     * RealizInitFinalItems} that needs to be
     * processed.
     * </p>
     */
    ...
    /**
     * <p>
     * The current {@link AssertiveCodeBlock} that the inner declarations will
     * operate on.
     * </p>
     */
    ...
    /**
     * <p>
     * All the completed {@link AssertiveCodeBlock AssertiveCodeBlocks} that
     * only contain the final
    ...
    /**
     * <p>
     * All the {@link AssertiveCodeBlock AssertiveCodeBlocks} generated by the
     * various different
     * declaration and statements that still needs more proof rule applications.
     * </p>
     */
     ...
     * </p>
     */
    public static final Flag FLAG_ADD_CONSTRAINT = new Flag(FLAG_SECTION_NAME,
            "addConstraints", FLAG_DESC_ADD_CONSTRAINT);
    ...
    /**
     * <p>
     * This creates an object that overrides methods to generate VCs from a
     * {@link ModuleDec}.
     * </p>
     *
    ...
    public final void preModuleDec(ModuleDec dec) {
        ...
        try {
            ...
            List<FacilityEntry> results =
                    myCurrentModuleScope.query(new EntryTypeQuery<>(
                            FacilityEntry.class, ImportStrategy.IMPORT_NAMED,
            ...
            for (SymbolTableEntry s : results) {
                ...
                if (s.getSourceModuleIdentifier().compareTo(
                    ...
                    FacilityDeclRule ruleApplication = new FacilityDeclRule(
                            facDec, false, myBuilder, myCurrentModuleScope,
                            new AssertiveCodeBlock(facDec.getName(), facDec,
                                    myTypeGraph),
                    ...
                    if (!facDec.getExternallyRealizedFlag()) {
                        PosSymbol conceptRealizName =
                                facDec.getConceptRealizName();
                        ModuleIdentifier coRealizId = new ModuleIdentifier(
                                conceptRealizName.getName());
                        myCurrentVerificationContext
                                .storeConceptRealizAssertionClauses(
                                        conceptRealizName.getLocation(),
                                        coRealizId, true);
                    }

                    for (EnhancementSpecRealizItem specRealizItem : facDec
                        ...
                        ModuleIdentifier enId =
                                new ModuleIdentifier(enhancementName.getName());
                        myCurrentVerificationContext
                                .storeEnhancementAssertionClauses(
                                        enhancementName.getLocation(), enId,
                                        true);

                        // Store all requires/constraint from the imported enhancement realization(s)
                        PosSymbol enhancementRealizName =
                                specRealizItem.getEnhancementRealizName();
                        ModuleIdentifier enRealizId = new ModuleIdentifier(
                                enhancementRealizName.getName());
                        myCurrentVerificationContext
                                .storeEnhancementRealizAssertionClauses(
                                        enhancementRealizName.getLocation(),
                                        enRealizId, true);
                    }
                }
            }
    ...
    public final void postModuleDec(ModuleDec dec) {
        ...
        while (!myIncompleteAssertiveCodeBlocks.isEmpty()) {
            ...
            ST blockModel = myAssertiveCodeBlockModels
        ...
        for (AssertiveCodeBlock block : myFinalAssertiveCodeBlocks) {
            ...
            for (VerificationCondition vc : vcs) {
                namedVCs.add(new VerificationCondition(vc.getLocation(),
                        blockCount + "_" + vcCount, vc.getSequent(),
    ...
    public final void preConceptModuleDec(ConceptModuleDec concept) {
        ...
        myCurrentVerificationContext.storeConceptAssertionClauses(
                conceptName.getLocation(),
                new ModuleIdentifier(conceptName.getName()), false);

        // Add to VC detail model
        ST header = mySTGroup.getInstanceOf("outputConceptHeader")
    ...
    public final void preConceptRealizModuleDec(
        ...
        myCurrentVerificationContext.storeConceptAssertionClauses(
                conceptName.getLocation(), coId, false);

        // Store all the shared states declared in the concept
        myCurrentVerificationContext
                .storeConceptSharedStateDecs(conceptName.getLocation(), coId);
        ...
        // Add to VC detail model
        ST header = mySTGroup.getInstanceOf("outputConceptRealizHeader")
                .add("realizName", conceptRealizName.getName())
                .add("conceptName", conceptName.getName());
        myVCGenDetailsModel.add("fileHeader", header.render());
    }
    ...
    public final void
        ...
        // Add to VC detail model
        ST header = mySTGroup.getInstanceOf("outputEnhancementHeader")
                .add("enhancementName", enhancementName.getName())
                .add("conceptName", conceptName.getName());
        myVCGenDetailsModel.add("fileHeader", header.render());
    }
    ...
    public final void preEnhancementRealizModuleDec(
        ...
        ST header = mySTGroup.getInstanceOf("outputEnhancementRealizHeader")
                .add("realizName", enhancementRealizName.getName())
                .add("enhancementName", enhancementName.getName())
                .add("conceptName", conceptName.getName());
        myVCGenDetailsModel.add("fileHeader", header.render());
    }
    ...
    public final void preFacilityModuleDec(FacilityModuleDec facility) {
        ...
        myCurrentVerificationContext.storeFacilityModuleAssertionClauses(
                facilityName.getLocation(),
                new ModuleIdentifier(facilityName.getName()));

        // Add to VC detail model
        ST header = mySTGroup.getInstanceOf("outputFacilityHeader")
                .add("facilityName", facilityName.getName());
        myVCGenDetailsModel.add("fileHeader", header.render());
    }
    ...
    public final void postFacilityDec(FacilityDec dec) {
        ...
        AssumeStmt topLevelAssumeStmt = new AssumeStmt(
                dec.getLocation().clone(),
                myCurrentVerificationContext.createTopLevelAssumeExpFromContext(
                        dec.getLocation(), false, false),
                false);
        ...
        FacilityDeclRule declRule = new FacilityDeclRule(dec, true, myBuilder,
        ...
        // Store this facility's InstantiatedFacilityDecl for future use
        myCurrentVerificationContext.storeInstantiatedFacilityDecl(
                declRule.getInstantiatedFacilityDecl());
    ...
    public final void preOperationProcedureDec(OperationProcedureDec dec) {
        ...
        OperationEntry correspondingOperation =
                Utilities.searchOperation(dec.getLocation(), null,
                        dec.getName(), argTypes, ImportStrategy.IMPORT_NAMED,
        ...
        if (dec.getRecursive()) {
            ...
            myCurrentAssertiveCodeBlock = new AssertiveCodeBlock(dec.getName(),
                    dec, correspondingOperation,
                    dec.getDecreasing().getAssertionExp(), myTypeGraph);
        }
        else {
            myCurrentAssertiveCodeBlock = new AssertiveCodeBlock(dec.getName(),
                    dec, correspondingOperation, myTypeGraph);
        }
        ...
        Exp topLevelAssumeExp = createTopLevelAssumeExpForProcedureDec(
                dec.getLocation(), myCurrentAssertiveCodeBlock,
                correspondingOperation, false, false, inConceptRealiz, true);
        AssumeStmt topLevelAssumeStmt = new AssumeStmt(
                dec.getLocation().clone(), topLevelAssumeExp, false);
        myCurrentAssertiveCodeBlock.addStatement(topLevelAssumeStmt);

        // Create Remember statement
        MemoryStmt rememberStmt = new MemoryStmt(dec.getLocation().clone(),
        ...
        stepModel.add("proofRuleName", "Procedure Declaration Rule (Part 1)")
    ...
    public final void postOperationProcedureDec(OperationProcedureDec dec) {
        ...
        ProcedureDec procedureDec = new ProcedureDec(dec.getName(),
                wrappedOpDec.getParameters(), wrappedOpDec.getReturnTy(),
                wrappedOpDec.getAffectedVars(), dec.getDecreasing(),
                dec.getFacilities(), dec.getVariables(), dec.getStatements(),
                dec.getRecursive());
        ProcedureDeclRule declRule = new ProcedureDeclRule(procedureDec,
                myVariableTypeEntries, myBuilder, myCurrentModuleScope,
                myCurrentAssertiveCodeBlock, myCurrentVerificationContext,
                mySTGroup,
    ...
    public final void postProcedureDec(ProcedureDec dec) {
        // Apply procedure declaration rule
        ProcedureDeclRule declRule = new ProcedureDeclRule(dec,
                myVariableTypeEntries, myBuilder, myCurrentModuleScope,
                myCurrentAssertiveCodeBlock, myCurrentVerificationContext,
                mySTGroup,
                myAssertiveCodeBlockModels.remove(myCurrentAssertiveCodeBlock));
    ...
    @Override
    public final void
            preSharedStateRealizationDec(SharedStateRealizationDec dec) {
        ...
        try {
            ...
            ConceptModuleDec conceptModuleDec = (ConceptModuleDec) myBuilder
                    .getModuleScope(new ModuleIdentifier(
                            conceptRealizModuleDec.getConceptName().getName()))
                    .getDefiningElement();
            ...
            Utilities.noSuchModule(
                    conceptRealizModuleDec.getConceptName().getLocation());
        }
    ...
    public final void midSharedStateRealizationDec(
        ...
        if (prevChild instanceof AssertionClause
                && ((AssertionClause) prevChild).getClauseType()
            ...
            SharedStateCorrRule declRule = new SharedStateCorrRule(dec,
                    myCorrespondingSharedStateDec, myBuilder, block,
                    myCurrentVerificationContext, mySTGroup, blockModel);
            declRule.applyRule();
    ...
    @Override
    public final void
            postSharedStateRealizationDec(SharedStateRealizationDec dec) {
    ...
    public final void preRealizInitFinalItem(RealizInitFinalItem item) {
        // Create a new assertive code block
        myCurrentAssertiveCodeBlock = new AssertiveCodeBlock(
                myRealizInitFinalOuterDec.getName(), item, myTypeGraph);
        ...
        if (myRealizInitFinalOuterDec instanceof TypeRepresentationDec) {
            if (item.getItemType()
                    .equals(AbstractInitFinalItem.ItemType.INITIALIZATION)) {
                // Create the top most level assume statement and
                // add it to the assertive code block as the first statement
                Exp topLevelAssumeExp =
                        createTopLevelAssumeExpForRealizInitFinalItem(
                                item.getLocation(), false);
                AssumeStmt topLevelAssumeStmt = new AssumeStmt(
                        item.getLocation().clone(), topLevelAssumeExp, false);
                myCurrentAssertiveCodeBlock.addStatement(topLevelAssumeStmt);

                // Create Remember statement
                MemoryStmt rememberStmt = new MemoryStmt(
                        item.getLocation().clone(), StatementType.REMEMBER);
                myCurrentAssertiveCodeBlock.addStatement(rememberStmt);
                ...
                TypeFamilyDec typeFamilyDec = Utilities
                        .getAssociatedTypeFamilyDec(typeRepresentationDec,
                                myCurrentVerificationContext);

                // Query for the type representation entry in the symbol table.
                SymbolTableEntry symbolTableEntry = Utilities.searchProgramType(
                        typeRepresentationDec.getLocation(), null,
                        typeRepresentationDec.getName(), myCurrentModuleScope);
                ...
                myCurrentAssertiveCodeBlock
                        .addStatement(new InitializeVarStmt(
            ...
            if (item.getItemType()
                ...
                for (InstantiatedFacilityDecl decl : instantiatedFacilityDecls) {
                    ...
                    if (decl.isLocalFacility()) {
                        myCurrentAssertiveCodeBlock
                                .addStatement(new FacilityInitStmt(
                                        decl.getInstantiatedFacilityDec()));
                    }
                }
                ...
                for (VarDec varDec : sharedStateRealizationDec.getStateVars()) {
                    ...
                    SymbolTableEntry symbolTableEntry = Utilities
                            .searchProgramType(varDec.getLocation(), null,
        ...
        // Create a new model for this assertive code block
        ST blockModel = mySTGroup.getInstanceOf("outputAssertiveCodeBlock");
        blockModel.add("blockName",
                blockName + myRealizInitFinalOuterDec.getName());
        ST stepModel = mySTGroup.getInstanceOf("outputVCGenStep");
        stepModel
                .add("proofRuleName",
                        blockName + myRealizInitFinalOuterDec.getName()
    ...
    public final void postRealizInitFinalItem(RealizInitFinalItem item) {
        ...
        if (myRealizInitFinalOuterDec instanceof TypeRepresentationDec) {
            ...
            if (item.getItemType()
                    .equals(AbstractInitFinalItem.ItemType.INITIALIZATION)) {
                ruleApplication = new TypeRepresentationInitRule(
                        typeRepresentationDec, myVariableTypeEntries, myBuilder,
                        myCurrentModuleScope, myCurrentAssertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup,
            ...
            // Generate a new type finalization rule application.
                ...
                ruleApplication = new TypeRepresentationFinalRule(
                        typeRepresentationDec, myVariableTypeEntries, myBuilder,
                        myCurrentModuleScope, myCurrentAssertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup,
                        myAssertiveCodeBlockModels
            ...
            ruleApplication = new SharedStateRepresentationInitRule(
                    sharedStateRealizationDec, myCorrespondingSharedStateDec,
                    myVariableTypeEntries, myBuilder, myCurrentModuleScope,
                    myCurrentAssertiveCodeBlock, myCurrentVerificationContext,
    ...
    public final void postVarDec(VarDec dec) {
        ...
        if (dec.getTy() instanceof NameTy) {
            ...
            SymbolTableEntry ste = Utilities.searchProgramType(
                    nameTy.getLocation(), nameTy.getQualifier(),
                    nameTy.getName(), myCurrentModuleScope);
            ...
            if ((typeEntry.getDefiningElement() instanceof TypeFamilyDec)
                    || (typeEntry
                            .getDefiningElement() instanceof FacilityTypeRepresentationDec)) {
            ...
            myCurrentAssertiveCodeBlock.addStatement(
                    new InitializeVarStmt(dec, ste, isGenericVar));

            // Add this as a free variable
            myCurrentAssertiveCodeBlock
                    .addFreeVar(Utilities.createVarExp(dec.getLocation(), null,
                            dec.getName(), dec.getMathType(), null));
    ...
    public final void postAssertionClause(AssertionClause clause) {
        if (clause.getWhichEntailsExp() != null) {
            // Create a new assertive code block
            PosSymbol name = new PosSymbol(
                    clause.getWhichEntailsExp().getLocation().clone(),
            ...
            WhichEntailsRule entailsRule = new WhichEntailsRule(clause, block,
                    myCurrentVerificationContext, mySTGroup, blockModel);
    ...
    /**
     * <p>
     * This method returns the final {@link AssertiveCodeBlock
     * AssertiveCodeBlocks} containing the
     * generated {@link Sequent Sequents}.
     * </p>
     *
    ...
    /**
     * <p>
     * This method returns the verbose mode output with how we generated the
     * {@code VCs} for this
     * {@link ModuleDec}.
     * </p>
     *
     ...
     * @param substitutionsMap A map of substitutions for any parameter
     *        constraints we generate.
     * @param scope The module scope to start our search.
     * @param currentBlock The current {@link AssertiveCodeBlock} we are
     *        currently generating.
     * @param entries List of operation's parameter entries.
     *
     * @return The original {@code exp} plus any operation parameter's type
     *         constraints.
     */
    private Exp addParamTypeConstraints(Location loc, Exp exp,
        ...
        for (ProgramParameterEntry entry : entries) {
            ...
            if (!(declaredType instanceof PTGeneric)) {
                ...
                SymbolTableEntry ste = Utilities.searchProgramType(loc,
                        nameTy.getQualifier(), nameTy.getName(), scope);
                ...
                if (!(typeEntry
                    ...
                    if (parameterMode != ProgramParameterEntry.ParameterMode.REPLACES) {
                        if (!VarExp.isLiteralTrue(typeFamilyDec.getConstraint()
                            ...
                            Exp whichEntailsExp = modifiedConstraintClause
                                    .getWhichEntailsExp();
                            if (whichEntailsExp != null) {
                                whichEntailsExp = Utilities
                                ...
                                // Apply any pre-defined substitutions
                                whichEntailsExp = whichEntailsExp
                                        .substitute(substitutionsMap);
                            }

                            modifiedConstraintClause = new AssertionClause(
                                    modifiedConstraintClause.getLocation()
                                            .clone(),
                                    modifiedConstraintClause.getClauseType(),
                            ...
                            Location constraintLoc = modifiedConstraintClause
            ...
            currentBlock.addFreeVar(Utilities.createVarExp(
                    parameterVarDec.getLocation(), null,
    ...
    /**
     * <p>
     * An helper method that adds the all the {@code Shared Variables} to the
     * assertive code block's
     * free variables list.
     * </p>
     *
    ...
    private void addSharedVarsToFreeVariableList(AssertiveCodeBlock block) {
        ...
        for (InstantiatedFacilityDecl facilityDecl : facilityDecls) {
            for (SharedStateDec stateDec : facilityDecl
                ...
                for (MathVarDec varDec : stateDec.getAbstractStateVars()) {
                    block.addFreeVar(Utilities.createVarExp(
                            varDec.getLocation(),
                            facilityDecl.getInstantiatedFacilityName(),
                            varDec.getName(), varDec.getMathType(), null));
                }
            }
        }
    }
    ...
    /**
     * <p>
     * Applies each of the statement proof rules. After this call, we are done
     * processing
     * {@code assertiveCodeBlock}.
     * </p>
     *
     * @param assertiveCodeBlock An assertive block that we are trying apply the
     *        proof rules to the
     *        various {@link Statement Statements}.
     */
    private void applyStatementRules(AssertiveCodeBlock assertiveCodeBlock) {
        ...
        while (assertiveCodeBlock.hasMoreStatements()) {
            ...
            if (statement instanceof AssumeStmt) {
                ...
                ruleApplication = new AssumeStmtRule((AssumeStmt) statement,
                        assertiveCodeBlock, myCurrentVerificationContext,
                        mySTGroup, blockModel);
            }
            else if (statement instanceof CallStmt) {
                // Generate a new call rule application.
                ruleApplication = new CallStmtRule((CallStmt) statement,
                        myBuilder, myCurrentModuleScope, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else if (statement instanceof ChangeStmt) {
                // Generate a new change rule application.
                ruleApplication = new ChangeStmtRule((ChangeStmt) statement,
                        assertiveCodeBlock, myCurrentVerificationContext,
                        mySTGroup, blockModel);
            }
            else if (statement instanceof ConfirmStmt) {
                // Generate a new confirm rule application.
                ruleApplication = new ConfirmStmtRule((ConfirmStmt) statement,
                        assertiveCodeBlock, myCurrentVerificationContext,
                        mySTGroup, blockModel);
            }
            else if (statement instanceof FacilityInitStmt) {
                // Generate a new facility initialization rule application.
                ruleApplication = new FacilityInitStmtRule(
                        (FacilityInitStmt) statement, myBuilder,
                        assertiveCodeBlock, myCurrentVerificationContext,
                        mySTGroup, blockModel);
            }
            else if (statement instanceof FinalizeVarStmt) {
                // Generate a new variable finalization rule application.
                ruleApplication = new FinalizeVarStmtRule(
                        (FinalizeVarStmt) statement, myBuilder,
                ...
                ruleApplication = new FuncAssignStmtRule(
                        (FuncAssignStmt) statement, myBuilder,
                        myCurrentModuleScope, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else if (statement instanceof IfStmt) {
                // Generate a new if-else rule application.
                ruleApplication = new IfStmtRule((IfStmt) statement, myBuilder,
                        myCurrentModuleScope, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else if (statement instanceof InitializeVarStmt) {
                // Generate a new variable declaration/initialization rule application.
                ruleApplication = new InitializeVarStmtRule(
                        (InitializeVarStmt) statement, myBuilder,
                        myCurrentModuleScope, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else if (statement instanceof MemoryStmt) {
                if (((MemoryStmt) statement)
                        .getStatementType() == StatementType.REMEMBER) {
                    // Generate a new remember rule application.
                    ruleApplication = new RememberStmtRule(assertiveCodeBlock,
                            myCurrentVerificationContext, mySTGroup,
                            blockModel);
                }
                else {
                ...
                ruleApplication = new PresumeStmtRule((PresumeStmt) statement,
                        assertiveCodeBlock, myCurrentVerificationContext,
                        mySTGroup, blockModel);
            }
            else if (statement instanceof SwapStmt) {
                // Generate a new swap rule application.
                ruleApplication = new SwapStmtRule((SwapStmt) statement,
                        myCurrentModuleScope, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else if (statement instanceof VCConfirmStmt) {
                // Generate a new VCConfirm rule application.
                ruleApplication = new VCConfirmStmtRule(
                        (VCConfirmStmt) statement, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else if (statement instanceof WhileStmt) {
                // Generate a new while rule application
                ruleApplication = new WhileStmtRule((WhileStmt) statement,
                        myCurrentModuleScope, myTypeGraph, assertiveCodeBlock,
                        myCurrentVerificationContext, mySTGroup, blockModel);
            }
            else {
                throw new SourceErrorException(
            ...
            while (!resultingBlocks.isEmpty()) {
                myIncompleteAssertiveCodeBlocks
                        .addFirst(resultingBlocks.removeLast());
            }

            // Store any new block models
            myAssertiveCodeBlockModels
                    .putAll(ruleApplication.getNewAssertiveCodeBlockModels());
    ...
    /**
     * <p>
     * An helper method that uses all the {@code requires} and
     * {@code constraint} clauses from the
     * various different sources (see below for complete list) and builds the
     * appropriate
     * {@code assume} clause that goes at the beginning an
     ...
     * <li>Type {@code convention} clause (if we are are in a
     * {@link TypeRepresentationDec} and
     * processing type finalization block).</li>
     * <li>Type {@code correspondence} clause (if we are in a
     ...
     *
     * <p>
     * See the {@code Shared Variable Initialization},
     * {@code Type Initialization} and
     * {@code Type Finalization} rule for more detail.
     * </p>
     *
     * @param loc The location in the AST that we are currently visiting.
    ...
    private Exp createTopLevelAssumeExpForRealizInitFinalItem(Location loc,
        ...
        if (inTypeRepresentationDec) {
            ...
            if (isTypeFinalItem) {
                ...
                retExp = Utilities
                        .formConjunct(loc.clone(), retExp, typeConventionClause,
                                new LocationDetailModel(
                                        typeConventionClause
                                                .getLocation().clone(),
                                        loc.clone(),
                                        "Type " + typeRepresentationDec
                                                .getName().getName()
                                                + "'s Convention"));
            }
            ...
            retExp = Utilities.formConjunct(loc.clone(), retExp,
                    typeCorrespondenceClause,
                    new LocationDetailModel(
                            typeCorrespondenceClause.getLocation().clone(),
                            loc.clone(),
                            "Type " + typeRepresentationDec.getName().getName()
                                    + "'s Correspondence"));
        }
     ...
     * change if it is an
     * implementation for a {@code concept}'s operation:</li>
     * <li>
     * <ul>
     * <li>Substitute the parameter name with {@code Conc.<name>} if this is the
     * type we are
     * implementing in a {@code concept realization}.</li>
     * </ul>
     * </li>
     * <li>Any {@code which_entails} expressions that originated from any of the
     * clauses above.</li>
     ...
     * @param currentBlock The current {@link AssertiveCodeBlock} we are
     *        currently generating.
     * @param correspondingOperationEntry The corresponding
     *        {@link OperationEntry}.
    ...
    private Exp createTopLevelAssumeExpForProcedureDec(Location loc,
            ...
            boolean addSharedCorrespondenceFlag, boolean inConceptRealiz,
            boolean isLocal) {
        // Add all the expressions we can assume from the current context
        Exp retExp = myCurrentVerificationContext
                .createTopLevelAssumeExpFromContext(loc,
                        addSharedConventionFlag, addSharedCorrespondenceFlag);
        ...
        if (inConceptRealiz && !isLocal) {
            ...
            for (SharedStateDec stateDec : sharedStateDecs) {
                for (MathVarDec mathVarDec : stateDec.getAbstractStateVars()) {
                    ...
                    VarExp varExp = Utilities.createVarExp(loc.clone(), null,
        ...
        Exp aggConventionCorrespondenceExp =
                VarExp.getTrueVarExp(loc, myTypeGraph);
        for (ParameterVarDec parameterVarDec : correspondingOperationEntry
                .getOperationDec().getParameters()) {
            // Query for the type entry in the symbol table
            NameTy nameTy = (NameTy) parameterVarDec.getTy();
            ...
            if (ste.getDefiningElement() instanceof TypeRepresentationDec
                ...
                AssertionClause conventionClause =
                        Utilities.getTypeConventionClause(
                                ((TypeRepresentationDec) ste
                                        .getDefiningElement()).getConvention(),
                ...
                if (VarExp.isLiteralTrue(aggConventionCorrespondenceExp)) {
                    aggConventionCorrespondenceExp = Utilities.formConjunct(loc,
                            null, conventionClause, conventionDetailModel);
                }
                else {
                    aggConventionCorrespondenceExp = Utilities.formConjunct(loc,
                            aggConventionCorrespondenceExp, conventionClause,
                ...
                AssertionClause correspondenceClause =
                        Utilities.getTypeCorrespondenceClause(
                                ((TypeRepresentationDec) ste
                                        .getDefiningElement())
                                                .getCorrespondence(),
                                loc.clone(), parameterVarDec.getName(), nameTy,
                                new PosSymbol(loc.clone(),
                                        familyType.getExemplarName()),
        ...
        if (inConceptRealiz && !isLocal) {
            if (!VarExp.isLiteralTrue(aggConventionCorrespondenceExp)) {
                retExp = MathExp.formConjunct(loc.clone(), retExp,
                        aggConventionCorrespondenceExp);
            }
        }
        ...
        if (!VarExp.isLiteralTrue(requiresExp)) {
            ...
            if (whichEntailsExp != null) {
                whichEntailsExp =
                        whichEntailsExp.substitute(substitutionParamToConc);
            }

            // Form a conjunct with the requires clause and add
            // the location detail associated with it.
            AssertionClause modifiedRequiresClause = new AssertionClause(
                    requiresClause.getLocation().clone(),
                    requiresClause.getClauseType(), requiresExp,
                    whichEntailsExp, requiresClause.getInvolvedSharedVars());
        ...
        if (myCompileEnvironment.flags.isFlagSet(FLAG_ADD_CONSTRAINT)) {
            retExp = addParamTypeConstraints(loc, retExp,
                    substitutionParamToConc, myCurrentModuleScope, currentBlock,
