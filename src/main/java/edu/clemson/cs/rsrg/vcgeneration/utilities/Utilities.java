...
/**
 * <p>
 * This class contains a bunch of utilities methods used by the
 * {@link VCGenerator} and all of its
 * associated {@link TreeWalkerVisitor TreeWalkerVisitors}.
 * </p>
 *
 * @author Yu-Shan Sun
 * @version 2.0
...
public class Utilities {
    ...
    /**
     * <p>
     * This method to check if an equivalent expression is in the specified
     * collection.
     * </p>
     *
     * <p>
     * <em>Note:</em> We can't use {@link Collection#contains(Object)} because
     * it will use the strict
     * {@link Exp#equals(Object)} method rather than
     * {@link Exp#equivalent(Exp)}.
     * </p>
     *
    ...
    /**
     * <p>
     * Converts the different types of {@link Exp} to the ones used by the VC
     * Generator.
     * </p>
     *
    ...
    public static Exp convertExp(Exp oldExp, ModuleScope scope) {
        ...
        if (oldExp instanceof ProgramIntegerExp) {
            IntegerExp exp = new IntegerExp(oldExp.getLocation(), null,
        ...
        // Case #2: ProgramCharacterExp
            ...
            CharExp exp = new CharExp(oldExp.getLocation(),
        ...
        // Case #3: ProgramStringExp
            ...
            StringExp exp = new StringExp(oldExp.getLocation(),
        ...
        // Case #5: VariableNameExp
            ...
            VarExp exp = new VarExp(oldExp.getLocation(),
                    ((ProgramVariableNameExp) oldExp).getQualifier(),
     ...
     */
    public static DotExp createConcVarExp(VarDec varDec, MTType varDecType,
            MTType booleanType) {
        // Convert the declared variable into a VarExp
        VarExp varExp = Utilities.createVarExp(varDec.getLocation(), null,
                varDec.getName(), varDecType, null);
    ...
    /**
     * <p>
     * This method returns a {@link FunctionExp} with the specified name and
     * arguments.
     * </p>
     *
     * @param loc Location that wants to create this function expression.
    ...
    public static FunctionExp createFunctionExp(Location loc,
        ...
        VarExp functionName = Utilities.createVarExp(loc, qualifier, name,
    ...
    /**
     * <p>
     * This method returns a {@link DotExp} with the {@link VarDec} and its
    ...
    public static DotExp createInitExp(VarDec varDec, MTType booleanType) {
        // Convert the declared variable into a VarExp
        VarExp varExp = Utilities.createVarExp(varDec.getLocation(), null,
                varDec.getName(), varDec.getTy().getMathTypeValue(), null);

        // Create a VarExp using the type
        VarExp typeNameExp = null;
        if (varDec.getTy() instanceof NameTy) {
            NameTy ty = (NameTy) varDec.getTy();
            typeNameExp = Utilities.createVarExp(ty.getLocation(),
                    ty.getQualifier(), ty.getName(), ty.getMathType(),
                    ty.getMathTypeValue());
        }
        else {
            Utilities.tyNotHandled(varDec.getTy(),
                    varDec.getTy().getLocation());
        }
    ...
    /**
     * <p>
     * This method creates a list of {@link VarExp VarExps} representing each of
     * the
     * {@code Operation's} {@link ParameterVarDec ParameterVarDecs}.
     * </p>
     *
     ...
     *         operation parameter.
     */
    public static List<VarExp> createOperationParamExpList(
        ...
        for (ParameterVarDec dec : parameterVarDecs) {
            retExpList.add(Utilities.createVarExp(dec.getLocation(), null,
    ...
    /**
     * <p>
     * This method returns a newly created {@link VarExp} with {@code P_Val} as
     * the name and {@code N}
     * as its math type.
     * </p>
     *
    ...
    public static VarExp createPValExp(Location loc, ModuleScope scope) {
        ...
        try {
            ...
            retExp = createVarExp(loc.clone(), null,
                    new PosSymbol(loc.clone(), "P_Val"), mse.getTypeValue(),
                    null);
        }
        catch (SymbolNotOfKindTypeException e) {
            notAType(mse, loc);
    ...
    /**
     * <p>
     * This method returns a newly created {@link VarExp} with the
     * {@link PosSymbol} and math type
     * provided.
     * </p>
     *
     * @param loc New {@link VarExp VarExp's} {@link Location}.
    ...
    /**
     * <p>
     * An helper method that throws the appropriate message that the expression
     * type that we found
     * isn't handled.
     * </p>
     *
    ...
    /**
     * <p>
     * An helper method that uses the assertion expression and any
     * {@code which_entails} expressions
     * to {@code exp} to form a new conjuncted expression.
     * </p>
     *
     ...
     * @param clauseDetailModel The {@link LocationDetailModel} associated with
     *        {@code clause}.
     *
    ...
    /**
     * <p>
     * An helper method for locating the associated {@code Type Family} from a
    ...
    public static TypeFamilyDec getAssociatedTypeFamilyDec(
        ...
        if (typeFamilyDec == null) {
            ...
            // by accident.
            Utilities.noSuchSymbol(null, dec.getName().getName(),
                    dec.getLocation());
        }
    ...
    /**
     * <p>
     * An helper method for locating a facility qualifier (if any) from a raw
     * program type.
     * </p>
     *
        ...
        if (facQualifier == null) {
            ...
            while (it.hasNext() && facQualifier == null) {
                ...
                for (TypeFamilyDec dec : decl.getConceptDeclaredTypes()) {
                    if (dec.getName().getName()
                            .equals(ty.getName().getName())) {
    ...
    /**
     * <p>
     * An helper method for locating the instantiating facility (if any) from an
     * operation entry.
     * </p>
     *
     ...
     * @param context The current {@link VerificationContext}.
     *
     * @return A {@link InstantiatedFacilityDecl} if {@code operationEntry} is
     *         from a module that has
     *         been instantiated, {@code null} otherwise.
     */
    public static InstantiatedFacilityDecl getInstantiatingFacility(
        ...
        while (it.hasNext() && instantiatedFacilityDecl == null) {
            ...
            if (sourceModuleIdentifier.equals(new ModuleIdentifier(
                    facilityDec.getConceptName().getName()))) {
    ...
    /**
     * <p>
     * An helper method that returns {@link ProgramFunctionExp
     * ProgramFunctionExp's} corresponding
     * {@link OperationEntry}.
     * </p>
     *
     ...
     */
    public static OperationEntry getOperationEntry(
            ProgramFunctionExp functionExp, ModuleScope scope) {
        ...
        return Utilities.searchOperation(functionExp.getLocation(),
                ...
                FacilityStrategy.FACILITY_INSTANTIATE, scope);
    }

    /**
     * <p>
     * Given the original {@code constraint} clause, use the provided
     * information on the actual
     * parameter variable to substitute the {@code exemplar} in the
     * {@code constraint} clause and
     * create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalConstraintClause The {@link AssertionClause} containing
     *        the original
     ...
     *         {@code constraint} clause.
     */
    public static AssertionClause getTypeConstraintClause(
            AssertionClause originalConstraintClause, Location loc,
            PosSymbol qualifier, PosSymbol name, PosSymbol exemplarName,
            MTType type, MTType typeValue) {
        // Create a variable expression from the declared variable
        VarExp varDecExp =
                Utilities.createVarExp(loc, qualifier, name, type, typeValue);

        // Create a variable expression from the type exemplar
        VarExp exemplar = Utilities.createVarExp(loc, null, exemplarName, type,
                typeValue);
        ...
        Exp constraintWithReplacements = originalConstraintClause
        ...
        if (originalConstraintClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalConstraintClause
                    .getWhichEntailsExp().substitute(substitutions);
        }

        return new AssertionClause(newLoc,
    ...
    /**
     * <p>
     * Given the original {@code convention} clause, use the provided
     * information on the actual
     * parameter variable to substitute the {@code exemplar} in the
     * {@code convention} clause and
     * create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalConventionClause The {@link AssertionClause} containing
     *        the original
     ...
     *         {@code convention} clause.
     */
    public static AssertionClause getTypeConventionClause(
            AssertionClause originalConventionClause, Location loc,
            PosSymbol name, PosSymbol exemplarName, MTType type,
            MTType typeValue) {
        // Create a variable expression from the declared variable
        VarExp varDecExp =
                Utilities.createVarExp(loc, null, name, type, typeValue);
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp conventionWithReplacements = originalConventionClause
                .getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalConventionClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalConventionClause
                    .getWhichEntailsExp().substitute(substitutions);
        }
    ...
    /**
     * <p>
     * Given the original {@code correspondence} clause, use the provided
     * information on the actual
     * parameter variable to substitute the {@code exemplar} in the
     * {@code correspondence} clause and
     * create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalCorrespondenceClause The {@link AssertionClause}
     ...
     *         {@code correspondence} clause.
     */
    public static AssertionClause getTypeCorrespondenceClause(
            AssertionClause originalCorrespondenceClause, Location loc,
            PosSymbol name, Ty ty, PosSymbol exemplarName, Ty exemplarTy,
        ...
        DotExp concVarDecExp = Utilities.createConcVarExp(new VarDec(name, ty),
                type, booleanType);
        ...
        DotExp concExemplarExp = Utilities.createConcVarExp(
                new VarDec(exemplarName, exemplarTy), type, booleanType);
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp correspondenceWithReplacements = originalCorrespondenceClause
                .getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalCorrespondenceClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalCorrespondenceClause
                    .getWhichEntailsExp().substitute(substitutions);
        }
    ...
    /**
     * <p>
     * Given the original {@code finalization ensures} clause, use the provided
     * information on a
     * program variable to substitute the {@code #exemplar} in the
     * {@code finalization ensures} clause
     * and create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalFinalEnsuresClause The {@link AssertionClause} containing
     *        the original
     *        {@code finalization ensures} clause.
     * @param loc The location in the AST that we are currently visiting.
     ...
     *
     * @return A modified {@link AssertionClause} containing the new
     *         {@code finalization ensures}
     *         clause.
     */
    public static AssertionClause getTypeFinalEnsuresClause(
            AssertionClause originalFinalEnsuresClause, Location loc,
            PosSymbol qualifier, PosSymbol name, PosSymbol exemplarName,
            MTType type, MTType typeValue) {
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp finalEnsuresWithReplacements = originalFinalEnsuresClause
                .getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalFinalEnsuresClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalFinalEnsuresClause
                    .getWhichEntailsExp().substitute(substitutions);
        }
    ...
    /**
     * <p>
     * Given the original {@code initialization ensures} clause, use the
     * provided information on a
     * program variable to substitute the {@code exemplar} in the
     * {@code initialization ensures}
     * clause and create a new {@link AssertionClause}.
     * </p>
     *
     * @param originalInitEnsuresClause The {@link AssertionClause} containing
     *        the original
     ...
     *         clause.
     */
    public static AssertionClause getTypeInitEnsuresClause(
            AssertionClause originalInitEnsuresClause, Location loc,
            PosSymbol qualifier, PosSymbol name, PosSymbol exemplarName,
            MTType type, MTType typeValue) {
        // Create a variable expression from the declared variable
        VarExp varDecExp =
        ...
        // Create new assertion clause by replacing the exemplar with the actual
        Location newLoc = loc.clone();
        Exp initEnsuresWithReplacements = originalInitEnsuresClause
                .getAssertionExp().substitute(substitutions);
        Exp whichEntailsWithReplacements = null;
        if (originalInitEnsuresClause.getWhichEntailsExp() != null) {
            whichEntailsWithReplacements = originalInitEnsuresClause
                    .getWhichEntailsExp().substitute(substitutions);
        }
    ...
    /**
     * <p>
     * Given a {@link ProgramVariableExp}, return the associated
     * {@link PosSymbol} name.
     * </p>
     *
    ...
    public static boolean isLocationOperation(String name, ModuleScope scope) {
        ...
        List<SymbolTableEntry> entries = scope
                .query(new NameQuery(null, name, ImportStrategy.IMPORT_NONE,
    ...
    public static Exp negateExp(Exp exp, MTType booleanType) {
        ...
        // Case 3: All other kinds of expressions.
            ...
            retExp = new PrefixExp(exp.getLocation().clone(), null,
    ...
    /**
     * <p>
     * An helper method that throws the appropriate message that the symbol
     * table entry that we found
     * isn't a type.
     * </p>
     *
    ...
    /**
     * <p>
     * This method is used to check for there are paths from
     * {@code originalSequent} to each sequent
     * in {@code resultSequents} in the reduction tree.
     * </p>
     *
     * @param g The reduction tree.
     * @param originalSequent The original {@link Sequent}.
     * @param resultSequents The {@link Sequent} that resulted from the sequent
     *        reduction
     *        applications.
     *
     * @return {@code true} if all the {@link Sequent} in {@code resultSequents}
     *         have a path from
     *         {@code originalSequent} in the reduction tree, {@code false}
     *         otherwise.
     */
    public static boolean pathsExist(Graph<Sequent, DefaultEdge> g,
        ...
        ShortestPathAlgorithm<Sequent, DefaultEdge> pathAlgorithm =
    ...
    /**
     * <p>
     * This method is used to replace any parameter declarations whose program
     * types come from
     * instantiated facility declarations. This will replace any the
     * instantiated type's formal with
     * its actual instantiation expression in {@code clauseExp}.
     * </p>
     *
            ...
            if ((dec.getTy() instanceof NameTy)
                ...
                if (decTyAsNameTy.getQualifier() == null) {
                    ...
                    while (it.hasNext() && isInstantiatedType) {
                        ...
                        if (decTyAsNameTy.getName().getName()
                                .equals(it.next().getName().getName())) {
                    ...
                    while (it2.hasNext() && isInstantiatedType) {
                        ...
                        if (decTyAsNameTy.getName().getName()
                                .equals(it2.next().getName().getName())) {
                            isInstantiatedType = false;
                        }
                    }
                }
                    ...
                    if (decTyAsNameTy.getQualifier().getName()
                            .equals(currentModuleName.getName())) {
                        isInstantiatedType = false;
                    }
                }
                ...
                if (isInstantiatedType) {
                    ...
                    if (decTyAsNameTy.getQualifier() == null) {
                        while (it.hasNext()
                            ...
                            Iterator<TypeFamilyDec> it2 = nextDec
                                    .getConceptDeclaredTypes().iterator();
                            while (it2.hasNext()
                                    && instantiatedFacilityDecl == null) {
                    ...
                    if (instantiatedFacilityDecl == null) {
                        ...
                        for (SharedStateDec sharedStateDec : sharedStateDecs) {
                            ...
                            for (MathVarDec mathVarDec : sharedStateDec
                                ...
                                qualifiedSharedVars.add(Utilities.createVarExp(
                                        mathVarDec.getLocation().clone(),
                                        facName.clone(),
                                        mathVarDec.getName().clone(),
                                        mathVarDec.getMathType(), null));
                            }
                            ...
                            // Replace in clause
                            modifiedClauseExp = replaceFormalWithActual(
                                    modifiedClauseExp, sharedVarsAsVarExps,
                                    qualifiedSharedVars);
                        }
                        ...
                        for (TypeFamilyDec typeFamilyDec : typeFamilyDecs) {
                            ...
                            for (MathDefVariableDec mathDefVariableDec : typeFamilyDec
                                ...
                                MathVarDec mathVarDec =
                                        mathDefVariableDec.getVariable();

                                // Convert to VarExp
                                defVarsExp.add(Utilities.createVarExp(
                                        mathVarDec.getLocation().clone(), null,
                                        mathVarDec.getName().clone(),
                                        mathVarDec.getMathType(), null));

                                // Convert to VarExp with the facility qualifier name.
                                qualifiedDefVars.add(Utilities.createVarExp(
                                        mathVarDec.getLocation().clone(),
                                        facName.clone(),
                                        mathVarDec.getName().clone(),
                                        mathVarDec.getMathType(), null));
    ...
    /**
     * <p>
     * This method is used to replace the module parameters with the actual
     * instantiated arguments.
     * Note that both of these have been converted to mathematical expressions.
     * </p>
     *
     ...
     *
     * @throws MiscErrorException This exception is thrown when we pass two
     *         lists that aren't equal in
     *         size.
     */
    public static Exp replaceFormalWithActual(Exp exp,
            List<VarExp> formalParams, List<Exp> actualArgs) {
        ...
        if (formalParams.size() == actualArgs.size()) {
            ...
            for (int i = 0; i < formalParams.size(); i++) {
                ...
                VarExp tempExp = Utilities.createVarExp(
                        formalParam.getLocation(), null,
                        new PosSymbol(formalParam.getLocation(),
                                "_" + formalParam.getName().getName()),
    ...
    /**
     * <p>
     * Given a math symbol name, locate and return the {@link MathSymbolEntry}
     ...
     * return the
     * {@link OperationEntry} stored in the symbol table.
     * </p>
     *
     * @param loc The location in the AST that we are currently visiting.
    ...
    public static OperationEntry searchOperation(Location loc,
        ...
        try {
            op = scope.queryForOne(new OperationQuery(qualifier, name, argTypes,
                    importStrategy, facilityStrategy));
        }
        catch (NoSuchSymbolException nsse) {
    ...
    /**
     * <p>
     * Given the name of the type locate and return the {@link SymbolTableEntry}
     * stored in the symbol
     * table.
     * </p>
     *
    ...
    public static SymbolTableEntry searchProgramType(Location loc,
        ...
        List<SymbolTableEntry> entries = scope.query(
                new NameQuery(qualifier, name, ImportStrategy.IMPORT_NAMED,
    ...
    /**
     * <p>
     * An helper method that throws the appropriate raw type not handled
     * message.
     * </p>
     *
     * @param ty A raw type.
    ...
    public static void tyNotHandled(Ty ty, Location loc) {
        throw new SourceErrorException(
                "[VCGenerator] Ty not handled: " + ty.toString(), loc);
    }
