...
/**
 * <p>
 * This class extracts ensures clauses (with the appropriate substitutions) from
 * walking potentially
 * nested {@link ProgramFunctionExp}. This visitor logic is implemented as a
 * {@link TreeWalkerVisitor}.
 * </p>
 *
 * <p>
 * <strong>Note:</strong> We don't have to worry about any shared variables
 * being affected. By
 * definition a function operation can't have side effects, therefore modifying
 * a shared variables
...
public class ProgramFunctionExpWalker extends TreeWalkerVisitor {
    ...
    /**
     * <p>
     * The {@link VerificationContext} where all the information for the current
     * {@code Assertive Code Block} is located.
     * </p>
     */
    ...
    /**
     * <p>
     * If this is a {@code Recursive Procedure}, then this will contain the
     * {@code decreasing} clause.
     * </p>
     */
    ...
    /**
     * <p>
     * A map that contains the modified ensures clause with the formal replaced
     * with the actuals for
     * each of the nested function calls.
     * </p>
     */
    ...
    /**
     * <p>
     * A list that contains the modified requires clauses with the formal
     * replaced with the actuals
     * for each of the nested function calls.
     * </p>
     */
    ...
    /**
     * <p>
     * A list that contains any generated termination {@code Confirm}
     * statements.
     * </p>
     */
    ...
    /**
     * <p>
     * This creates a {@link TreeWalkerVisitor} that visits and generates
     * {@code requires} and
     * {@code ensures} clauses for potentially nested function calls.
     * </p>
     *
     * <p>
     * Note that this constructor is used by non-recursive declarations, where
     * there isn't a
     * {@code decreasing} clause.
     * </p>
     *
    ...
    public final void postProgramFunctionExp(ProgramFunctionExp exp) {
        ...
        if (exp.getQualifier() != null
                && operationDec.getParameters().isEmpty()) {
            PosSymbol qualifier = exp.getQualifier().clone();
            for (InstantiatedFacilityDecl decl : myCurrentVerificationContext
                    .getProcessedInstFacilityDecls()) {
                if (decl.getInstantiatedFacilityName().getName()
                        .equals(qualifier.getName())) {
                    // Replace any shared variables
                    for (SharedStateDec stateDec : decl
                            .getConceptSharedStates()) {
                        for (MathVarDec varDec : stateDec
                                .getAbstractStateVars()) {
                            // Construct the qualified and not qualified version of varDec
                            VarExp varDecAsVarExp = Utilities.createVarExp(
                                    exp.getLocation().clone(), null,
                                    varDec.getName(), varDec.getMathType(),
                                    null);
                            VarExp qualifiedVarExp =
                                    (VarExp) varDecAsVarExp.clone();
                            qualifiedVarExp.setQualifier(qualifier);

                            // Add it to our substitution map
                            substitutionFacSharedVars.put(varDecAsVarExp,
                                    qualifiedVarExp);
                        }
                    }

                    // Replace any definition variables
                    for (TypeFamilyDec typeFamilyDec : decl
                        ...
                        for (MathDefVariableDec mathDefVariableDec : typeFamilyDec
                            ...
                            VarExp defVarAsVarExp = Utilities.createVarExp(
                                    exp.getLocation().clone(), null,
                                    mathDefVariableDec.getName(),
                                    mathDefVariableDec.getMathType(), null);
                            VarExp qualifiedDefVarExp =
                                    (VarExp) defVarAsVarExp.clone();
                            qualifiedDefVarExp.setQualifier(qualifier);

                            // Add it to our substitution map
                            substitutionFacSharedVars.put(defVarAsVarExp,
                                    qualifiedDefVarExp);
                        }
                    }
                }
        ...
        if (!VarExp
                .isLiteralTrue(operationDec.getRequires().getAssertionExp())) {
            // Replace formals in the original requires clause with the
            // actuals from the function call.
            Exp requiresExp = replaceFormalWithActualReq(
                    operationDec.getRequires().getAssertionExp(),
            ...
            requiresExp.setLocationDetailModel(new LocationDetailModel(
                    operationDec.getRequires().getLocation().clone(),
                    exp.getLocation().clone(),
                    "Requires Clause of " + fullOperationName));

            // Replace any facility declaration instantiation arguments
            // in the requires clause.
            requiresExp = Utilities.replaceFacilityFormalWithActual(requiresExp,
        ...
        Exp ensuresExp = replaceFormalWithActualEns(
                operationDec.getEnsures().getAssertionExp(),
                operationDec.getParameters(), exp.getArguments());
        ...
        ensuresExp.setLocationDetailModel(new LocationDetailModel(
                operationDec.getEnsures().getLocation().clone(),
                exp.getLocation().clone(),
                "Ensures Clause of " + fullOperationName));

        // Replace any facility declaration instantiation arguments
        // in the ensures clause.
        ensuresExp = Utilities.replaceFacilityFormalWithActual(ensuresExp,
                operationDec.getParameters(),
                myCurrentModuleScope.getDefiningElement().getName(),
                myCurrentVerificationContext);
    ...
    /**
     * <p>
     * This method returns the final modified ensures clause after all the
     * necessary
     * replacement/substitutions have been made.
     * </p>
     *
     * <p>
     * Note that the return value is removed from the class and subsequent calls
     * to this method with
     * the same {@code exp} will result in an exception.
     * </p>
     *
     * @param exp The outermost {@link ProgramFunctionExp} that we wish to
     *        extract an {@code ensures}
     *        clause for.
     *
    ...
    /**
     * <p>
     * This method returns the final modified requires clause after all the
     * necessary
     * replacement/substitutions have been made.
     * </p>
     *
     * @param loc The location to be stored inside the requires clause generated
     *        by this walker.
     *
    ...
    public final Exp getRequiresClause(Location loc) {
        ...
        for (Exp exp : myRequiresClauseList) {
            ...
            // Else form a conjunct.
                ...
                allRequiresExp = InfixExp.formConjunct(loc.clone(),
                        allRequiresExp, exp.clone());
            }
        }
    ...
    /**
     * <p>
     * This method returns the list of termination {@code Confirm} clauses (if
     * any).
     * </p>
     *
     * @return {@code Confirm} clauses generated from recursive calls to our
    ...
    /**
     * <p>
     * An helper method for sanity checking the generated expression from the
     * {@link ProgramFunctionExpWalker}.
     * </p>
     *
     * @param generatedExp The generated {@code ensures} clause expression.
     *
    ...
    private void
        ...
        if (myDecreasingExp == null) {
            ...
            IntegerExp oneExp = new IntegerExp(
                    myDecreasingExp.getLocation().clone(), null, 1);
            oneExp.setMathType(myDecreasingExp.getMathType());

            InfixExp sumExp = new InfixExp(
                    myDecreasingExp.getLocation().clone(), oneExp, null,
                    new PosSymbol(myDecreasingExp.getLocation().clone(), "+"),
                    myDecreasingExp.clone());
            sumExp.setMathType(myDecreasingExp.getMathType());

            InfixExp terminationExp = new InfixExp(
                    myDecreasingExp.getLocation().clone(), sumExp, null,
                    new PosSymbol(myDecreasingExp.getLocation().clone(), "<="),
                    pValExp.clone());
    ...
    /**
     * <p>
     * An helper method that replaces the formal with the actual variables
     * inside the {@code ensures}
     * clause.
     * </p>
     *
     ...
     *
     * @return A potentially modified {@code ensures} clause represented using
     *         an {@link Exp}.
     */
        ...
        for (int i = 0; i < argList.size(); i++) {
            ...
            if (exp instanceof ProgramFunctionExp) {
                ...
                if (myEnsuresClauseMap.containsKey(exp)) {
                    ...
                    throw new SourceErrorException(
                            "[VCGenerator] Could not find the modified ensures clause of: "
                                    + exp.toString(),
                            exp.getLocation());
                }
            }
            ...
            VarExp paramExpAsVarExp = Utilities.createVarExp(
            ...
            VarExp tempExp = Utilities.createVarExp(varDec.getLocation(), null,
                    new PosSymbol(varDec.getLocation(),
                            "_" + varDec.getName().getName()),
     ...
     * @param argList The list of arguments from the operation call.
     *
     * @return A potentially modified {@code requires} clause represented using
     *         an {@link Exp}.
     */
