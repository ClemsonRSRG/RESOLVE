...
public class SequentReductionTest {
...

    {
        try {
            FAKE_LOCATION =
                    new Location(new ResolveFile(
                            new ResolveFileBasicInfo("SequentReductionTest",
                                    ""),
                            ModuleType.THEORY,
    ...
    /**
     * <p>
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a complex
     * {@link Sequent} that needs multiple reductions.
     * </p>
     */
    @Test
    public final void testComplexExample() {
        // Create sequent: "|- ((p implies r) or (q implies r)) implies ((p and q) implies r)"
        VarExp p = Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "p"), FAKE_TYPEGRAPH.BOOLEAN,
                null);
        VarExp q = Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "q"), FAKE_TYPEGRAPH.BOOLEAN,
                null);
        VarExp r = Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "r"), FAKE_TYPEGRAPH.BOOLEAN,
                null);
        Exp pImpliesr =
                MathExp.formImplies(FAKE_LOCATION, p.clone(), r.clone());
        Exp qImpliesr =
                MathExp.formImplies(FAKE_LOCATION, q.clone(), r.clone());
        ...
        Exp pAndqImpliesr =
                MathExp.formImplies(FAKE_LOCATION, pAndq, r.clone());
        Exp complexExp =
                MathExp.formImplies(FAKE_LOCATION, orExp, pAndqImpliesr);
        ...
        for (Sequent resultSequent : resultSequents) {
            ...
            assertTrue(Utilities.pathsExist(reduction.getReductionTree(),
                    originalSequent, resultSequents));
        }
    }
     ...
     * <p>
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link LeftAndRule}.
     * </p>
     */
    @Test
    public final void testLeftAndRule() {
        // Create sequent: "H, (A and B) |- G"
        VarExp A = Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "A"), FAKE_TYPEGRAPH.BOOLEAN,
                null);
        VarExp B = Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "B"), FAKE_TYPEGRAPH.BOOLEAN,
                null);
        Exp AandB = MathExp.formConjunct(FAKE_LOCATION, A, B);

        List<Exp> antecedents = new ArrayList<>();
        antecedents.add(Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "H"), FAKE_TYPEGRAPH.BOOLEAN,
                null));
        antecedents.add(AandB);

        List<Exp> consequents = new ArrayList<>();
        consequents.add(Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "G"), FAKE_TYPEGRAPH.BOOLEAN,
                null));
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link LeftImpliesRule}.
     * </p>
     */
    @Test
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link LeftNotRule}.
     * </p>
     */
    @Test
    public final void testLeftNotRule() {
        ...
        VarExp A = Utilities.createVarExp(FAKE_LOCATION, null,
                new PosSymbol(FAKE_LOCATION, "A"), FAKE_TYPEGRAPH.BOOLEAN,
                null);
        Exp notA = new PrefixExp(FAKE_LOCATION, null,
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link LeftOrRule}.
     * </p>
     */
    @Test
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link RightAndRule}.
     * </p>
     */
    @Test
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link RightImpliesRule}.
     * </p>
     */
    @Test
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link RightNotRule}.
     * </p>
     */
    @Test
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only needs the {@link RightOrRule}.
     * </p>
     */
    @Test
     ...
     * <p>
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on an empty
     * {@link Sequent}.
     * </p>
     */
    @Test
    public final void testSequentReductionOnEmptySequent() {
        ...
        Sequent originalSequent = new Sequent(FAKE_LOCATION,
     ...
     * This tests what happens when we call
     * {@link SequentReduction#applyReduction()} on a
     * {@link Sequent} that only has atomic formulas.
     * </p>
     */
    @Test
    ...
    /**
     * <p>
     * This method checks to see if the given expression is in the antecedent
     * expression list.
     * </p>
     *
    ...
    /**
     * <p>
     * This method checks to see if the given expression is in the consequent
     * expression list.
     * </p>
     *
     * @param sequent A {@link Sequent} to be checked.
     ...
     * @param exp The {@link Exp} we are searching for.
     *
     * @return {@code true} if it is in the {@code sequent}, {@code false}
     *         otherwise.
     */
